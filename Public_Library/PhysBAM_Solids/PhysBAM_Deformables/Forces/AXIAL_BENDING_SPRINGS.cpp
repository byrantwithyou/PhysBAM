...
Prepare_Energy_Correction_Force()
{
    ...
    if(verbose) for(int i=1;i<=particles.array_collection->Size();i++) LOG::cout<<"Initial body "<<i<<" velocity: "<<particles.V(i)<<std::endl;
...
Compute_Energy_Correction_Force(ARRAY_VIEW<const TV> velocity_save,const int max_particle_degree,const T time,const T dt)
{
    ...
    for(SPRING_ITERATOR iterator(force_springs);iterator.Valid();iterator.Next()){int s=iterator.Data();
        ...
        if(verbose) LOG::cout<<"AXIAL BENDING SPRING "<<s<<" ****************"<<std::endl;
        T current_work=Compute_Spring_Work(s,velocity_save,time,dt);
        if(verbose) LOG::cout<<"Direction "<<optimization_direction(s)<<" Total work done: "<<current_work<<std::endl;
        
        if(current_work+delta_pe<0)
            continue;
        
        if(verbose) LOG::cout<<"Saved potential energy: "<<potential_energy_save(s)<<" current potential energy: "<<current_potential_energy<<" delta pe: "<<delta_pe<<std::endl;
        if(verbose) LOG::cout<<"Current total energy: "<<current_potential_energy+current_kinetic_energy<<std::endl;
        ...
        if(use_kinetic_energy_fix){
            ...
            root2=quadratic.root2;}
        T force_magnitude;
        if(verbose) LOG::cout<<"ROOTS "<<roots<<std::endl;
        if(roots==2){
        ...
        if(verbose) LOG::cout<<"Force I want to apply: "<<energy_correction_forces(s)*optimization_direction(s)<<std::endl;
        if(!allow_kd_direction_flip){
            if(energy_correction_sign(s)==1){
                ...
                if(force_correction(s)+energy_correction_forces(s)>0)
                    energy_correction_forces(s)=-force_correction(s);}}
        if(verbose) LOG::cout<<"Correction force: "<<energy_correction_forces(s)*optimization_direction(s)<<std::endl;
...
{        
    ...
    if(verbose) for(int i=1;i<=particles.array_collection->Size();i++) LOG::cout<<"Body "<<i<<" velocity: "<<particles.V(i)<<std::endl;

    if(verbose) 
        for(SPRING_ITERATOR iterator(force_springs);iterator.Valid();iterator.Next()){int s=iterator.Data();
            LOG::cout<<"Force correction for spring "<<s<<": "<<force_correction(s)<<std::endl;
            LOG::cout<<"Final total energy: "<<Potential_Energy(s,time)+Endpoint_Kinetic_Energy(s)<<std::endl;}
}
//#####################################################################
...
Compute_Spring_Work(int s,ARRAY_VIEW<const TV> velocity_save,const T time,const T dt) const
{
    ...
    if(verbose) LOG::cout<<"Work computation: force: "<<force<<" average v1: "<<average_vel1<<" average v2: "<<average_vel2<<std::endl;
