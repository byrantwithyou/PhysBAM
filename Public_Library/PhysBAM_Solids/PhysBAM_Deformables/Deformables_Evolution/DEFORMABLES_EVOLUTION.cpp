...
DEFORMABLES_EVOLUTION(DEFORMABLES_PARAMETERS<TV>& deformables_parameters_input,DEFORMABLE_BODY_COLLECTION<TV>& deformable_body_collection_input,RIGID_GEOMETRY_COLLECTION<TV>& rigid_geometry_collection_input)
    ...
    kinematic_evolution(rigid_geometry_collection,deformables_parameters_input.rigid_geometry_evolution_parameters.use_kinematic_keyframes),
...
Backward_Euler_Step_Velocity_Helper(const T dt,const T current_velocity_time,const T current_position_time,const bool velocity_update)
{
    ...
    DEFORMABLES_VELOCITY<TV> V(particles.V,rigid_geometry_particles.twist,deformable_body_collection,rigid_geometry_collection),F(F_full,rigid_F_full,deformable_body_collection,rigid_geometry_collection),
...
Trapezoidal_Step_Velocity(const T dt,const T time)
{
    ...
    kinematic_evolution.Set_External_Velocities(rigid_geometry_collection.particles.twist,time+dt,time+dt/2);
...
Backward_Euler_Step_Velocity(const T dt,const T time)
{
    ...
    kinematic_evolution.Set_External_Velocities(rigid_geometry_collection.particles.twist,time+dt,time+dt);
...
Print_Maximum_Velocities(const T time) const
{
    ...
    if(max_index){
        ...
        if(deformable_body_collection.mpi_solids) LOG::cout<<", local = "<<max_magnitude;
        LOG::cout<<" ("<<p<<")"<<std::endl;}
}
//#####################################################################
...
Save_Velocity()
{
    ...
    for(int i=0;i<rigid_geometry_collection.particles.array_collection->Size();i++) if(rigid_geometry_collection.Is_Active(i)){
        rigid_velocity_save(i)=rigid_geometry_collection.particles.twist(i).linear;}
}
//#####################################################################
...
Exchange_Velocity()
{
    ...
    for(int i=0;i<rigid_geometry_particles.array_collection->Size();i++) if(rigid_geometry_collection.Is_Active(i)){
...
Adjust_Velocity_For_Self_Repulsion_And_Self_Collisions(const T dt,const T time,int& repulsions_found,int& collisions_found,const bool exit_early)
{
    ...
    if(collisions_found<0) // failed to resolve collisions (should only happen when exit_early=true) - otherwise any collisions that were found have been resolved
        ...
        else for(int p=0;p<particles.array_collection->Size();p++) if(!modified(p)) particles.V(p)=V_save(p);}
