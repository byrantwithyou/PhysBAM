...
Advance_To_Target_Time(const T target_time)
{
    ...
    bool done=false;for(int substep=1;!done;substep++){
        ...
        if(rigids_parameters.rigid_body_evolution_parameters.simulate_rigid_bodies) example.rigid_body_collection.Reset_Impulse_Accumulators();
        rigids_evolution_callbacks->Preprocess_Solids_Substep(time,substep);
        if(example.mpi_rigids || rigids_parameters.use_spatial_partition_for_levelset_collision_objects) // TODO - ANDY - why is this needed??? TODO: move this to the right places inside solids evolution 
