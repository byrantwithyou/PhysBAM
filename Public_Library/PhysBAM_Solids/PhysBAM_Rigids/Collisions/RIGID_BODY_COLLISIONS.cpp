        ...
        for(int i=0;i<contact_graph.directed_graph.Nodes_In_Level(level).m;i++){
            ...
            for(int j=0;j<contact_graph.directed_graph.Parents(body_id).m;j++){
                ...
                if(other_body_level!=level || !contact_graph.directed_graph.Parents(other_body_id).Contains(body_id) || body_id<other_body_id)
                    if(!parameters.perform_collisions || pairs_processed_by_collisions.Contains(VECTOR<int,2>(body_id,other_body_id).Sorted()))
...
{
    LOG::SCOPE scope("rigid body push out legacy");
...
{
    LOG::SCOPE scope("rigid body push out new");
...
Compute_Contact_Frequency()
{
    ...
    for(int i=0;i<adj.m;i++) if(depths(i)==-1) Get_Rigid_Body_Depth(i,depths);
...
Construct_Stacks()
{
    ...
    for(int i=0;i<visited.m;i++) if(!visited(i)){
        ...
        for(int j=0;j<list.m;j++) contact_stack.Last().Append(list(j));}
}
//#####################################################################
            ...
            for(int t=0;t<object_indices.m;t++) if(object_indices(t)>=COLLISION_GEOMETRY_ID(0)){
                ...
                if(id>=0 && Either_Body_Collides_With_The_Other(rigid_body->particle_index,id) &&
                    ...
                    LOG::cout<<"!!!! Interpenetration detected: id:"<<particle_body<<" \""<<rigid_body_collection.Rigid_Body(particle_body).name<<"\" point inside id: "<<levelset_body<<" \""
                             <<rigid_body_collection.Rigid_Body(levelset_body).name<<"\" levelset"<<std::endl;
                    return true;}}}}}
    return false;
}
//#####################################################################
...
Print_Interpenetration_Statistics()
{
    ...
    if(num_interpenetration_points>0){
        ...
        LOG::cout<<"Interpenetration statistics: num interpenetrating points = "<<num_interpenetration_points<<", smallest phi = "
           <<smallest_phi<<"(\""<<rigid_body_collection.Rigid_Body(worst_id_1).name<<"\" and \""<<rigid_body_collection.Rigid_Body(worst_id_2).name<<"\")"
           <<", avg phi = "<<avg_phi<<std::endl;}
    else LOG::cout<<"No interpenetration"<<std::endl;
}
//#####################################################################
...
Register_Analytic_Collisions()
{
    ...
    analytic_collision_registry.Set(VECTOR<std::string,2>(box,box),Update_Box_Box_Collision);
    analytic_collision_registry.Set(VECTOR<std::string,2>(box,plane).Sorted(),Update_Box_Plane_Collision);
...
Set_Level_Temporarily_Static(const int level)
{
    ...
    for(int i=0;i<nodes.m;i++) if(rigid_body_collection.Is_Active(nodes(i))) rigid_body_collection.Rigid_Body(nodes(i)).is_temporarily_static=true;
}
//#####################################################################
...
{
    for(int i=0;i<contact_joints.m;i++) rigid_body_collection.articulated_rigid_body.joint_mesh.Remove_Articulation(contact_joints(i));
...
Create_Contact_Joint(const RIGID_BODY<TV>& parent,const RIGID_BODY<TV>& child,const TV& location)
{
    ...
    FRAME<TV> J(location,ROTATION<TV>::From_Rotated_Vector(TV::Axis_Vector(0),normal));
