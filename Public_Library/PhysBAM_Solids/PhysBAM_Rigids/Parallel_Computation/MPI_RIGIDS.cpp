    ...
    :current_tag(0),recompute(false)
{
    ...
    LOG::cout<<"number of ranks = "<<number_of_processors<<std::endl;
    ...
    COLLISION_GEOMETRY_SPATIAL_PARTITION<COLLISION_GEOMETRY<TV>,const ARRAY<COLLISION_GEOMETRY<TV>*,COLLISION_GEOMETRY_ID>,COLLISION_GEOMETRY_ID>& spatial_partition)
{
    ...
    if(!rigid_body_collection_input.rigid_body_particle.store_id){
        if(ATTRIBUTE_INDEX index=rigid_body_collection_input.rigid_body_particle.Get_Attribute_Index(ATTRIBUTE_ID_ID)){
            rigid_body_collection_input.rigid_body_particle.store_id=true;
            ARRAY_VIEW<int>* id=rigid_body_collection_input.rigid_body_particle.template Get_Array_From_Index<int>(index);
            ...
            for(int i=0;i<rigid_body_collection_input.rigid_body_particle.Size();i++){
...
Broadcast_Positions(RIGID_BODY_COLLECTION<TV>& rigid_body_collection_input)
{
    ...
    int buffer_size=rigid_body_collection_input.rigid_body_particle.Pack_Size()*particles_of_partition(id).m+1;
    send_buffer.Resize(buffer_size);int position=0;
    for(int b=0;b<particles_of_partition(id).m;b++){
        rigid_body_collection_input.rigid_body_particle.Pack(send_buffer,position,particles_of_partition(id)(b));}
    ...
    for(int i=0;i<number_of_processors;i++){
        if(i-1!=rank){
            ...
            for(int b=0;b<particles_of_partition(PARTITION_ID(i)).m;b++){
                ...
                rigid_body_collection_input.rigid_body_particle.Unpack(recv_buffers(i),position,rigid_body_index);
...
Split_Range(const VECTOR<T,3>& global_range,const VECTOR<int,3>& processes_per_dimension)
{
    ...
    if(processes_per_dimension!=VECTOR<int,3>()) count=processes_per_dimension;
        ...
        for(test_count.z=0;test_count.z<number_of_processors;test_count.z++) if(number_of_processors%test_count.z==0){
    ...
    COLLISION_GEOMETRY_SPATIAL_PARTITION<COLLISION_GEOMETRY<TV>,const ARRAY<COLLISION_GEOMETRY<TV>*,COLLISION_GEOMETRY_ID>,COLLISION_GEOMETRY_ID>& spatial_partition)
{
    ...
    partition_id_from_particle_index.Resize(rigid_body_collection_input.rigid_body_particle.Size());
...
{
    for(int j=0;j<rigid_body_collection_input.rigid_body_particle.Size();j++) rigid_body_collection_input.Rigid_Body(j).impulse_accumulator->Reset();
}
//#####################################################################
...
{
    ARRAY<bool> need_to_reevolve(rigid_body_collection.rigid_body_particle.Size());
    ...
    for(int i=0;i<rigid_body_collection.rigid_body_particle.Size();i++){
        if(need_to_reevolve(i)){
            ...
            // Restore Velocities
            rigid_body.Twist().linear=mpi_rigid_velocity_save(p).linear;
            ...
            rigid_body.Twist().linear+=impulse_accumulator->accumulated_impulse.linear/rigid_body.Mass();
...
//#####################################################################
namespace PhysBAM{
template<> void MPI_RIGIDS<VECTOR<float,1> >::Prune_And_Exchange_Impulses(RIGID_BODY_COLLECTION<VECTOR<float,1> >& rigid_body_collection,ARRAY<bool>& need_to_reevolve)
{PHYSBAM_NOT_IMPLEMENTED();}
template<> void MPI_RIGIDS<VECTOR<double,1> >::Prune_And_Exchange_Impulses(RIGID_BODY_COLLECTION<VECTOR<double,1> >& rigid_body_collection,ARRAY<bool>& need_to_reevolve)
{PHYSBAM_NOT_IMPLEMENTED();}
}
template<class TV> void MPI_RIGIDS<TV>::
...
template<class TV> void MPI_RIGIDS<TV>::Initialize(RIGID_BODY_COLLECTION<TV>& rigid_body_collection_input,TV_INT processes_per_dimension_input,
...
template<class TV> void MPI_RIGIDS<TV>::Broadcast_Positions(RIGID_BODY_COLLECTION<TV>& rigid_body_collection_input){PHYSBAM_NOT_IMPLEMENTED();}
template<class TV> void MPI_RIGIDS<TV>::Update_Partitions(RIGID_BODY_COLLECTION<TV>& rigid_body_collection_input,
    COLLISION_GEOMETRY_SPATIAL_PARTITION<COLLISION_GEOMETRY<TV>,const ARRAY<COLLISION_GEOMETRY<TV>*,COLLISION_GEOMETRY_ID>,COLLISION_GEOMETRY_ID>& spatial_partition){PHYSBAM_NOT_IMPLEMENTED();}
template<class TV> RANGE<TV> MPI_RIGIDS<TV>::Split_Range(TV_INT& processes_per_dimension,const RANGE<TV>& global_range){PHYSBAM_NOT_IMPLEMENTED();}
template<class TV> VECTOR<int,1> MPI_RIGIDS<TV>::Split_Range(const VECTOR<T,1>& global_range,const VECTOR<int,1>& processes_per_dimension){PHYSBAM_NOT_IMPLEMENTED();}
template<class TV> VECTOR<int,2> MPI_RIGIDS<TV>::Split_Range(const VECTOR<T,2>& global_range,const VECTOR<int,2>& processes_per_dimension){PHYSBAM_NOT_IMPLEMENTED();}
template<class TV> VECTOR<int,3> MPI_RIGIDS<TV>::Split_Range(const VECTOR<T,3>& global_range,const VECTOR<int,3>& processes_per_dimension){PHYSBAM_NOT_IMPLEMENTED();}
template<class TV> void MPI_RIGIDS<TV>::Split_Dimension(const T x,const int processes,ARRAY<T>& boundaries){PHYSBAM_NOT_IMPLEMENTED();}
template<class TV> void MPI_RIGIDS<TV>::Simple_Partition(RIGID_BODY_COLLECTION<TV>& rigid_body_collection_input,
    COLLISION_GEOMETRY_SPATIAL_PARTITION<COLLISION_GEOMETRY<TV>,const ARRAY<COLLISION_GEOMETRY<TV>*,COLLISION_GEOMETRY_ID>,COLLISION_GEOMETRY_ID>& spatial_partition){PHYSBAM_NOT_IMPLEMENTED();}
template<class TV> void MPI_RIGIDS<TV>::Synchronize_Dt(T& dt) const {PHYSBAM_NOT_IMPLEMENTED();}
...
template<class TV> template<class T_DATA> T_DATA MPI_RIGIDS<TV>::Reduce_Max(const T_DATA local_value) const {PHYSBAM_NOT_IMPLEMENTED();}
template<class TV> template<class T_DATA> T_DATA MPI_RIGIDS<TV>::Reduce_Add(const T_DATA local_value) const {PHYSBAM_NOT_IMPLEMENTED();}
//#####################################################################
...
#define INSTANTIATION_HELPER(TV,T) \
    ...
    template T MPI_RIGIDS<TV>::Reduce_Min(const T) const; \
    template T MPI_RIGIDS<TV>::Reduce_Max(const T) const; \
    template int MPI_RIGIDS<TV>::Reduce_Max(const int) const; \
    template T MPI_RIGIDS<TV>::Reduce_Add(const T) const;
