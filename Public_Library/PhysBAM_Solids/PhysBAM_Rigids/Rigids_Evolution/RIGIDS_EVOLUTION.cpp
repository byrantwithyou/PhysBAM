...
Prepare_Backward_Euler_System(RIGIDS_BACKWARD_EULER_SYSTEM<TV>& system,const T dt,const T current_velocity_time,const T current_position_time,const bool velocity_update)
{
    ...
    if(articulated_rigid_body.constrain_pd_directions){
        for(int i=0;i<rigid_body_collection.dynamic_rigid_body_particles.m;i++){int p=rigid_body_collection.dynamic_rigid_body_particles(i);rigid_B_full(p)=world_space_rigid_mass_inverse(p)*rigid_B_full(p);}
        articulated_rigid_body.Poststabilization_Projection(rigid_B_full,true);
        for(int i=0;i<rigid_body_collection.dynamic_rigid_body_particles.m;i++){int p=rigid_body_collection.dynamic_rigid_body_particles(i);rigid_B_full(p)=world_space_rigid_mass(p)*rigid_B_full(p);}}
...
Finish_Backward_Euler_Step(const T dt,const T current_position_time,const bool velocity_update)
{
    ...
    if(velocity_update && rigids_parameters.use_post_cg_constraints){ // return rhs + dt Fd V^n+1 for friction processing
        ...
        if(rigid_body_collection.articulated_rigid_body.Has_Actuators() && rigid_body_collection.articulated_rigid_body.constrain_pd_directions){
            rigid_body_collection.articulated_rigid_body.Poststabilization_Projection(rigid_body_particles.twist,true);
...
{
    LOG::SCOPE scope("backward euler step velocity helper");
    RIGID_BODY_PARTICLES<TV>& rigid_body_particles=rigid_body_collection.rigid_body_particle;
    ...
    RIGIDS_VELOCITY<TV> V(rigid_body_particles.twist,rigid_body_collection),F(rigid_F_full,rigid_body_collection),
...
{
    LOG::SCOPE scope("trapezoidal step velocity");
    // save V at time
    Save_Velocity();
    ...
    RIGIDS_VELOCITY<TV> V_n(rigid_V_save,rigid_body_collection),V(rigid_body_collection.rigid_body_particle.twist,rigid_body_collection);
    V*=(T)2;V-=V_n;
...
{
    LOG::SCOPE scope("Apply_Constraints");
    ARTICULATED_RIGID_BODY<TV>& articulated_rigid_body=rigid_body_collection.articulated_rigid_body;
...
Update_Velocity_Using_Stored_Differences(const T dt,const T time,const int p)
{
    ...
    if(rigid_body_collection.rigid_body_particle.kinematic(p)) kinematic_evolution.Set_External_Velocities(rigid_body.Twist(),time+dt,p);
...
{
    LOG::SCOPE scope("update velocity using stored differences");
    ...
    for(int i=0;i<rigid_body_collection.kinematic_rigid_bodies.m;i++){int p=rigid_body_collection.kinematic_rigid_bodies(i);
        kinematic_evolution.Set_External_Velocities(rigid_body_collection.rigid_body_particle.twist(p),time+dt,p);}
}
//#####################################################################
...
Compute_Momentum_Differences()
{
    LOG::SCOPE scope("compute momentum difference");
...
Exchange_Velocity()
{
    ...
    for(int i=0;i<rigid_body_particles.array_collection->Size();i++) if(rigid_body_collection.Is_Active(i)){RIGID_BODY<TV>& body=rigid_body_collection.Rigid_Body(i);
        if(!body.Is_Simulated()){
...
Clamp_Velocities()
{
    ...
    for(int i=0;i<dynamic_rigid_body_particles.m;i++){int p=dynamic_rigid_body_particles(i);
        ...
        if(magnitude_squared>max_linear_velocity_squared) rigid_body_particles.twist(p).linear*=rigids_parameters.rigid_body_evolution_parameters.max_rigid_body_linear_velocity/sqrt(magnitude_squared);
        ...
        if(magnitude_squared>max_angular_velocity_squared){
            rigid_body_particles.twist(p).angular*=rigids_parameters.rigid_body_evolution_parameters.max_rigid_body_angular_velocity/sqrt(magnitude_squared);
