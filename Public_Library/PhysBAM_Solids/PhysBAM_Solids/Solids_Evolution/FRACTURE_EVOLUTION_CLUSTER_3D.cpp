...
Process_Cluster_Fracture(const T dt,const T time,SOLIDS_EVOLUTION<TV>* solids_evolution,SOLIDS_EVOLUTION_CALLBACKS<TV>* solids_evolution_callbacks,bool force)
{
    ...
    if(force){
        ...
        for(int i=0;i<rigid_body_particles.Size();i++) if(rigid_body_particles.Is_Active(i)){
        ...
        if(clusters_to_check.m>0){
            ...
            for(int i=0;i<rigid_body_particles.Size();i++) if(rigid_body_particles.Is_Active(i)) rigid_body_particles.Rigid_Body(i).Restore_State(COLLISION_BODY<TV>::SOLIDS_EVOLUTION_RIGID_BODY_OLD_STATE);
        ...
        if(clusters_to_check.m>0){
            ...
            for(int i=0;i<rigid_body_particles.Size();i++) if(rigid_body_particles.Is_Active(i)) rigid_body_particles.Rigid_Body(i).Save_State(COLLISION_BODY<TV>::SOLIDS_EVOLUTION_RIGID_BODY_OLD_STATE);
            // remove all tagged clusters -- ones that had a greater impulse
            for(int c=0;c<clusters_to_check.m;c++){
                clusters_to_check(c)->Save_Constituent_Bodies_State(COLLISION_BODY<TV>::SOLIDS_EVOLUTION_RIGID_BODY_OLD_STATE,time);
                rigid_body_particles.Remove_Cluster_Body(clusters_to_check(c)->Id_Number(),false);
