...
Get_Active_Forces_Contributions(const T dt,const T time,ARRAY<TV>& force_on_particles,ARRAY<TWIST<TV> >& force_on_rigid_body_particles)
{
    ...
    GENERALIZED_VELOCITY<TV> F(F_full,rigid_F_full,solid_body_collection),B(B_full,rigid_B_full,solid_body_collection),
        V(solid_body_collection.deformable_body_collection.particles.V,solid_body_collection.rigid_body_collection.rigid_body_particle.twist,solid_body_collection);
...
Set_Asynchronous_Within_Distance(const T distance)
{
    ...
    LOG::cout<<"Updated adaptive asynchronous particles number: "<<async_list.m<<std::endl;
}
//#####################################################################
...
{
    LOG::cout<<"Adding blob of "<<blob_particles.m<<" particles"<<std::endl;
