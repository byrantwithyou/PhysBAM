...
Advance_One_Time_Step(const T dt,const T time)
{
    ...
    if(solids_parameters.perform_collision_body_collisions){
        ...
        if(interactions) LOG::cout<<"collision body collisions = "<<interactions<<std::endl;}
    ...
    // output
    LOG::cout<<"maximum velocity = "<<ARRAY<TV>::Maximum_Magnitude(particles.V.Subset(dynamic_particles))<<std::endl;
}
//#####################################################################
