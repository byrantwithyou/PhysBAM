...
Rebuild_Topology()
{
    ...
    LOG::cout<<"number of elements = "<<fracture_object->embedded_object.simplicial_object.mesh.elements.m;
    LOG::cout<<"   ("<<fracture_object->embedded_object.Fraction_Of_Elements_With_Embedded_Subelements()<<")  ";
    LOG::cout<<"building embedded_object: ";
    ...
    LOG::cout<<"building boundary surface:";
    ...
    if(fracture_object->embedded_object.simplicial_object.Minimum_Signed_Volume(&index)<0)
        LOG::cout<<"MINIMUM SIGNED VOLUME IS NEGATIVE JUST AFTER BULID CURRENT ETV\n\n"<<std::endl;
...
Fracture_Where_High_Stress(const T small_number)
{
#if 0
    FINITE_VOLUME<TV,3>& finite_volume=solids_parameters.solid_body_collection.template Find_Force<FINITE_VOLUME<TV,3>&>();
    LOG::cout<<"computing sigma"<<std::endl;
    ...
    LOG::cout<<"max_eigenvalue_seen = "<<max_eigenvalue_seen<<"   ------   "<<"min_eigenvalue_seen = "<<min_eigenvalue_seen<<std::endl;
    LOG::cout<<"max:("<<max_eigenvalue_mean<<","<<max_eigenvalue_variance<<","<<max_eigenvalue_standard_deviation<<")"<<std::endl;
    LOG::cout<<"min:("<<min_eigenvalue_mean<<","<<min_eigenvalue_variance<<","<<min_eigenvalue_standard_deviation<<")"<<std::endl;
...
Adjust_Nodes_For_Segment_Triangle_Intersections(T threshhold)
{
    ...
    for(int i=0;i<particles.Size();i++){
        ...
        if(edge_triangle_perturbation.x){
            ...
            LOG::cout<<"p "<<p<<" with edge "<<e<<" triangle "<<t<<" and distance "<<edge_triangle_perturbation.z<<std::endl;
            if(edge_triangle_perturbation.z>1e-5){
                LOG::cout<<"skipping big perturbation here"<<std::endl;continue;}
            ...
            if(INTERSECTION::Intersects(SEGMENT_3D<T>(particles.X(a),particles.X(b)),triangle,solids_parameters.solid_body_collection.deformable_body_collection.triangle_collisions.geometry.small_number)){
                ...
                processed_nodes++;}
            else LOG::cout<<"no longer an intersection with node "<<p<<std::endl;}}
...
{
#if 0
    LOG::cout<<"CREATE NEW BODIES ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^"<<std::endl;
    DEFORMABLE_BODY_COLLECTION<TV>& deformable_body_collection=solids_parameters.solid_body_collection.deformable_body_collection;
...
Run_Quasistatics_And_Fracture(const T time,const int max_number_of_fracture_iterations)
{
    ...
    LOG::cout<<"fracture_iterations "<<number_of_fracture_iterations<<std::endl;
