...
public:
    ...
    {for(int t=0;t<mesh.elements.m;t++) fracture_object.fracture_bias_direction(t)=random_numbers.template Get_Direction<VECTOR<T,d> >();}
    ...
    int number_marked=0;
    for(int t=0;t<mesh.elements.m;t++) if(!regions(t) && to_be_marked(t)){regions(t)=region;number_marked++;}
    ...
    for(int r=0;r<regions.m;r++) fracture_object.fracture_bias_direction(r)=random_vectors(regions(r));}
    ...
    for(int r=0;r<regions.m;r++) spatial_fracture_bias_direction(r)=random_vectors(regions(r));}
    ...
    {int count=0;for(int t=0;t<mesh.elements.m;t++) if(regions(t)==region) count++;return count;}
    ...
    void Print_Regions()
    {for(int t=0;t<mesh.elements.m;t++) LOG::cout<<"region("<<t<<")="<<regions(t)<<std::endl;}
