...
public:
    ...
    AVERAGING_COLLIDABLE_BINARY_UNIFORM(const GRID_BASED_COLLISION_GEOMETRY_UNIFORM<TV>& body_list_input,const T default_replacement_value_input)
    ...
    TV Face_To_Face_Vector(const GRID<TV>& grid,const int side,const int axis,const TV_INT& face_index,const T_FACE_LOOKUP& u_face) const
    {FACE_ITERATOR<TV> iterator(grid,axis,face_index);
