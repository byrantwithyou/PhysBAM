...
Advance_One_Time_Step(const T_FACE_ARRAYS_SCALAR& V,const T dt,const T time,const int number_of_ghost_cells)
{
    ...
    if(indices_interface.m>0){
        ...
        LOG::cout<<"Dn_dot ("<<Ddot_min<<","<<Ddot_max<<")"<<std::endl;}
    if(indices_interface.m>0){
        ...
        LOG::cout<<"Mean Dn "<<mean_Dn<<" "<<mean_Dn_dot<<std::endl;
        LOG::cout<<"Mean curvature "<<mean_curvature<<std::endl;
        LOG::cout<<"Mean coefficients "<<mean_A<<" "<<mean_B<<" "<<mean_C<<" "<<mean_D<<std::endl;}
}
//#####################################################################
