...
Initialize_Grid(const T_GRID& mac_grid)
{
    ...
    fine_grid=mac_grid;
    coarse_grid.Initialize(fine_grid.counts/coarse_scale,fine_grid.domain,fine_grid.Is_MAC_Grid());
...
{
    for(UNIFORM_GRID_ITERATOR_FACE<TV> iterator(coarse_grid);iterator.Valid();iterator.Next()) coarse_face_velocities(iterator.Full_Index())=0;
    FACE_INDEX<TV::dimension> fine_index;
    for(UNIFORM_GRID_ITERATOR_CELL<TV> iterator(coarse_grid);iterator.Valid();iterator.Next()){
        for(UNIFORM_GRID_ITERATOR_FACE<TV> local_iterator(local_grid,0,GRID<TV>::BOUNDARY_REGION);local_iterator.Valid();local_iterator.Next()){int axis=local_iterator.Axis();
    ...
    for(UNIFORM_GRID_ITERATOR_FACE<TV> iterator(coarse_grid);iterator.Valid();iterator.Next()){int axis=iterator.Axis();
        T factor=0;
...
{ 
    for(UNIFORM_GRID_ITERATOR_FACE<TV> iterator(coarse_grid);iterator.Valid();iterator.Next()) beta_face.Component(iterator.Axis())(iterator.Face_Index())=0;
    FACE_INDEX<TV::dimension> fine_index;
    for(UNIFORM_GRID_ITERATOR_CELL<TV> iterator(coarse_grid);iterator.Valid();iterator.Next()){
        for(UNIFORM_GRID_ITERATOR_FACE<TV> local_iterator(local_grid,0,GRID<TV>::BOUNDARY_REGION);local_iterator.Valid();local_iterator.Next()){int axis=local_iterator.Axis();
    ...
    for(UNIFORM_GRID_ITERATOR_FACE<TV> iterator(coarse_grid);iterator.Valid();iterator.Next()){
...
Set_Coarse_Boundary_Conditions(T_FACE_ARRAYS_SCALAR& coarse_face_velocities)
{
    for(int axis=0;axis<TV::dimension;axis++) for(int axis_side=0;axis_side<2;axis_side++){int side=2*axis+axis_side;
        if(domain_boundary(axis)(axis_side)){ //Need to check mpi as smaller solves are never using mpi (for now)
            ...
            for(UNIFORM_GRID_ITERATOR_FACE<TV> local_iterator(coarse_grid,1,GRID<TV>::BOUNDARY_REGION,side);local_iterator.Valid();local_iterator.Next()){
...
Map_Fine_To_Coarse(T_FACE_ARRAYS_SCALAR& coarse_face_velocities,const T_FACE_ARRAYS_SCALAR& face_velocities)
{
    ...
    for(UNIFORM_GRID_ITERATOR_FACE<TV> iterator(coarse_grid);iterator.Valid();iterator.Next()) coarse_face_velocities_save(iterator.Full_Index())=coarse_face_velocities(iterator.Full_Index());
    for(UNIFORM_GRID_ITERATOR_FACE<TV> iterator(fine_grid);iterator.Valid();iterator.Next()) face_velocities_save(iterator.Full_Index())=face_velocities(iterator.Full_Index());
}
//#####################################################################
...
{
    for(UNIFORM_GRID_ITERATOR_CELL<TV> iterator(coarse_grid);iterator.Valid();iterator.Next()){TV_INT cell=iterator.Cell_Index();
...
{
    for(UNIFORM_GRID_ITERATOR_FACE<TV> local_iterator(local_mac_grid,0,GRID<TV>::BOUNDARY_REGION);local_iterator.Valid();local_iterator.Next()){
...
{
    for(UNIFORM_GRID_ITERATOR_FACE<TV> local_iterator(local_mac_grid,0,GRID<TV>::INTERIOR_REGION);local_iterator.Valid();local_iterator.Next()){
        FACE_INDEX<TV::dimension> fine_index=FACE_INDEX<TV::dimension>(local_iterator.Axis(),coarse_cell_index*coarse_scale+local_iterator.Face_Index()-coarse_scale*TV_INT::All_Ones_Vector());
...
Map_Fine_To_Local_Boundaries_For_Cell(GRID<TV>& local_mac_grid,ARRAY<bool,FACE_INDEX<TV::dimension> >& local_psi_N,TV_INT cell_index)
{
    ...
    for(UNIFORM_GRID_ITERATOR_FACE<TV> local_iterator(local_mac_grid);local_iterator.Valid();local_iterator.Next()){
...
{ 
    ...
    for(UNIFORM_GRID_ITERATOR_CELL<TV> local_iterator(local_grid);local_iterator.Valid();local_iterator.Next()){
...
Threaded_Local_Projection_PCG(T_FACE_ARRAYS_SCALAR& fine_face_velocities,const T dt,const T time)
{
    for(UNIFORM_GRID_ITERATOR_CELL<TV> iterator(coarse_grid);iterator.Valid();iterator.Next()){
        ...
        refinement_task->projection=this;refinement_task->cell_index=iterator.Cell_Index();refinement_task->dt=dt;refinement_task->time=time;refinement_task->fine_face_velocities=&fine_face_velocities;
...
{
    for(UNIFORM_GRID_ITERATOR_CELL<TV> iterator(coarse_grid);iterator.Valid();iterator.Next()) Local_Projection_PCG(fine_face_velocities,local_grid,local_face_velocities,fast_local_projection,dt,time,iterator.Cell_Index());
}
//#####################################################################
// Function Map_Coarse_To_Fine
//#####################################################################
...
Map_Coarse_To_Fine(const T_FACE_ARRAYS_SCALAR& coarse_face_velocities,T_FACE_ARRAYS_SCALAR& face_velocities,const T dt,const T time)
{
    ...
    if(thread_queue) Threaded_Local_Projection_PCG(face_velocities,dt,time);
