...
Set_Coarse_Boundary_Conditions(T_FACE_ARRAYS_SCALAR& coarse_face_velocities)
{
    ...
    for(int axis=0;axis<T_GRID::dimension;axis++) for(int axis_side=0;axis_side<2;axis_side++){
        ...
        if(solid_wall(axis)(axis_side)){
            for(UNIFORM_GRID_ITERATOR_FACE<TV> iterator(coarse_grid,1,T_GRID::BOUNDARY_REGION,side);iterator.Valid();iterator.Next()){
            ...
            for(UNIFORM_GRID_ITERATOR_FACE<TV> iterator(coarse_grid,1,T_GRID::BOUNDARY_REGION,side);iterator.Valid();iterator.Next()){TV_INT cell=iterator.Face_Index()+interior_cell_offset;
                elliptic_solver->psi_D(cell)=true;p(cell)=0;}}
    ...
    if(false) if(POISSON_COLLIDABLE_UNIFORM<T_GRID>* poisson_collidable=dynamic_cast<POISSON_COLLIDABLE_UNIFORM<T_GRID>*>(poisson)){
        ...
        for(UNIFORM_GRID_ITERATOR_FACE<TV> iterator(coarse_grid);iterator.Valid();iterator.Next()){FACE_INDEX<TV::dimension> index=iterator.Full_Index();poisson->beta_face(index)*=beta_face_new(index);}}
    for(UNIFORM_GRID_ITERATOR_CELL<TV> iterator(coarse_grid);iterator.Valid();iterator.Next()) if(Contains_Outside(iterator.Cell_Index(),levelset.phi,0)){
...
Set_Local_Boundary_Conditions(GRID<TV>& local_grid,PROJECTION_UNIFORM<GRID<TV> >& local_projection,TV_INT coarse_index)
{
    ...
    for(UNIFORM_GRID_ITERATOR_CELL<TV> iterator(local_grid);iterator.Valid();iterator.Next()){
...
{
    for(UNIFORM_GRID_ITERATOR_CELL<TV> iterator(local_mac_grid,1);iterator.Valid();iterator.Next()){
...
{ 
    ...
    for(UNIFORM_GRID_ITERATOR_CELL<TV> local_iterator(local_grid,domain);local_iterator.Valid();local_iterator.Next()){
...
{
    for(UNIFORM_GRID_ITERATOR_CELL<TV> iterator(coarse_grid,1);iterator.Valid();iterator.Next())
...
Set_Levelset_Boundary_Conditions(const GRID<TV>& levelset_grid,ARRAY<T,FACE_INDEX<TV::dimension> >& levelset_velocities,const ARRAY<T,TV_INT>& levelset_phi,const T time)
{
    ...
    for(int axis=0;axis<TV::dimension;axis++) for(int axis_side=0;axis_side<2;axis_side++){int side=2*axis+axis_side;
        ...
        TV_INT boundary_face_offset=axis_side==0?TV_INT::Axis_Vector(axis):-TV_INT::Axis_Vector(axis);
        if(solid_wall(axis)(axis_side)){
            for(UNIFORM_GRID_ITERATOR_FACE<TV> iterator(levelset_grid,1,GRID<TV>::BOUNDARY_REGION,side);iterator.Valid();iterator.Next()){
                TV_INT face=iterator.Face_Index()+boundary_face_offset;
        ...
        else for(UNIFORM_GRID_ITERATOR_FACE<TV> iterator(levelset_grid,1,GRID<TV>::BOUNDARY_REGION,side);iterator.Valid();iterator.Next()){TV_INT cell=iterator.Face_Index()+interior_cell_offset;
            levelset_projection.elliptic_solver->psi_D(cell)=true;levelset_projection.p(cell)=0;}}
    for(UNIFORM_GRID_ITERATOR_CELL<TV> iterator(levelset_grid);iterator.Valid();iterator.Next()){
    ...
    for(UNIFORM_GRID_ITERATOR_CELL<TV> iterator(coarse_grid);iterator.Valid();iterator.Next()){ 
        if(!Contains_Outside(iterator.Cell_Index(),levelset_phi,buffer)) for(int axis=0;axis<TV::dimension;axis++) for(int axis_side=0;axis_side<2;axis_side++){
            ...
            if(adjacent_outside) for(UNIFORM_GRID_ITERATOR_CELL<TV> local_iterator(local_grid,domain);local_iterator.Valid();local_iterator.Next()){
...
{
    for(UNIFORM_GRID_ITERATOR_FACE<TV> iterator(fine_grid);iterator.Valid();iterator.Next()){FACE_INDEX<TV::dimension> face=iterator.Full_Index();
