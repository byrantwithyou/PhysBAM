        ...
        if(!eigensystem.All_Eigenvalues_Same()){
            ...
            for(int ii=i+1-eno_order;ii<=i+eno_order;ii++){
                for(int k=0;k<d;k++){
                    ...
                    if(ii<1 && outflow_boundaries(1) && lambda(k)<0) for(int kk=0;kk<d;kk++){DLU(k,ii)(1)+=L(k,kk)*U(1)(kk);DLF(k,ii)(1)+=L(k,kk)*F(1)(kk);}
                    else if(ii>m && outflow_boundaries(2) && lambda(k)>0) for(int kk=0;kk<d;kk++){DLU(k,ii)(1)+=L(k,kk)*U(m)(kk);DLF(k,ii)(1)+=L(k,kk)*F(m)(kk);}
                    else for(int kk=0;kk<d;kk++){DLU(k,ii)(1)+=L(k,kk)*U(ii)(kk);DLF(k,ii)(1)+=L(k,kk)*F(ii)(kk);}}}
