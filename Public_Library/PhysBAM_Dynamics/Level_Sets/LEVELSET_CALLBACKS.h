...
class LEVELSET_CALLBACKS
{    
    typedef typename T_GRID::VECTOR_T VECTOR_T;typedef typename VECTOR_T::SCALAR T;
...
//#####################################################################
    ...
    virtual void Adjust_Particle_For_Domain_Boundaries(PARTICLE_LEVELSET_PARTICLES<VECTOR_T>& particles,const int index,VECTOR_T& V,const PARTICLE_LEVELSET_PARTICLE_TYPE particle_type,
        const T dt,const T time){PHYSBAM_WARN_IF_NOT_OVERRIDDEN();}
    virtual bool Adjust_Particle_For_Objects(VECTOR_T& X,VECTOR_T& V,const T r, const PARTICLE_LEVELSET_PARTICLE_TYPE particle_type,const T dt,const T time)
