...
//#####################################################################
// Class LEVELSET_ADVECTION_3D
//##################################################################### 
#ifndef __LEVELSET_ADVECTION_3D__
#define __LEVELSET_ADVECTION_3D__
...
template<class T>
class LEVELSET_ADVECTION_3D:public LEVELSET_ADVECTION_UNIFORM<GRID<VECTOR<T,3> > >
{
    ...
    typedef typename REBIND<typename GRID_ARRAYS_POLICY<GRID<TV> >::FACE_ARRAYS,bool>::TYPE T_FACE_ARRAYS_BOOL;
...
public:
    ...
    void Euler_Step_Subset(const ARRAY<T,FACE_INDEX<TV::dimension> >& face_velocity,int m_start,int m_end,int n_start,int n_end,int mn_start,int mn_end,const T dt,const T time,const int number_of_ghost_cells);
    void Euler_Step_Cell(const ARRAY<T,FACE_INDEX<TV::dimension> >& face_velocity,int i,int j,int ij,const T dt,const T time,const int number_of_ghost_cells);
    void Reinitialize(const int time_steps=10,const T time=0);
private:
    void Euler_Step_Of_Reinitialization(const ARRAY<T,VECTOR<int,3> >& sign_phi,const T dt,const T time);
};
