...
Reinitialize(const int time_steps,const T time)
{
    ...
    ARRAY<T,VECTOR<int,3> > sign_phi(0,m,0,n,0,mn); // smeared out sign function
    T epsilon=sqr(grid.dX.Max());
    TV_INT i;
    for(i.x=0;i.x<m;i.x++) for(i.y=0;i.y<n;i.y++) for(i.z=0;i.z<mn;i.z++) sign_phi(i)=phi(i)/sqrt(sqr(phi(i))+epsilon);
...
Euler_Step_Of_Reinitialization(const ARRAY<T,VECTOR<int,3> >& sign_phi,const T dt,const T time)
{
    ...
    ARRAY<T,VECTOR<int,3> > rhs(0,m,0,n,0,mn);

    ARRAY<T,VECTOR<int,1> > phi_1d_x(-ghost_cells,m+ghost_cells),phix_minus(0,m),phix_plus(0,m);
    ...
    ARRAY<T,VECTOR<int,1> > phi_1d_y(-ghost_cells,n+ghost_cells),phiy_minus(0,n),phiy_plus(0,n);
    ...
    ARRAY<T,VECTOR<int,1> > phi_1d_z(-ghost_cells,mn+ghost_cells),phiz_minus(0,mn),phiz_plus(0,mn);
    ...
    for(i.x=0;i.x<m;i.x++) for(i.y=0;i.y<n;i.y++) for(i.z=0;i.z<mn;i.z++){
