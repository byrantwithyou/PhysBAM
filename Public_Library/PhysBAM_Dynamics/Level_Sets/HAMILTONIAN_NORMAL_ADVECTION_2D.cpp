...
#include <PhysBAM_Dynamics/Level_Sets/HAMILTONIAN_NORMAL_ADVECTION_2D.h>
using namespace PhysBAM;
//#####################################################################
// Function H
//#####################################################################
template<class T> T HAMILTONIAN_NORMAL_ADVECTION_2D<T>::
H(const T phi_x,const T phi_y,const int i,const int j,const T t)
{       
    return speed*sqrt(sqr(phi_x)+sqr(phi_y));
}
//#####################################################################
// Function Maxabs_H1
//#####################################################################
...
{       
    if(phi_y_1*phi_y_2 <= 0) return abs(speed); // use phi_y=0
    T phi_x=maxabs(phi_x_1,phi_x_2),phi_y=minabs(phi_y_1,phi_y_2);
    return abs(speed)*phi_x/sqrt(sqr(phi_x)+sqr(phi_y));
}
//#####################################################################
// Function Maxabs_H2
//#####################################################################
...
{       
    if(phi_x_1*phi_x_2 <= 0) return abs(speed); // use phi_x=0
    T phi_x=minabs(phi_x_1,phi_x_2),phi_y=maxabs(phi_y_1,phi_y_2);
    return abs(speed)*phi_y/sqrt(sqr(phi_x)+sqr(phi_y));
}
//#####################################################################
