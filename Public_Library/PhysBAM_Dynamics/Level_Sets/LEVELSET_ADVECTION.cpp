...
Use_Semi_Lagrangian_Collidable_Advection(const T_GRID_BASED_COLLISION_GEOMETRY& body_list,const SCALAR phi_replacement_value,const T_FACE_ARRAYS_BOOL& face_velocities_valid_mask_input)
{
    ...
    semi_lagrangian_collidable=new ADVECTION_WRAPPER_COLLIDABLE_CELL<T_GRID,SCALAR,T_FACE_LOOKUP,T_ADVECTION_SEMI_LAGRANGIAN_COLLIDABLE_CELL,T_FACE_LOOKUP_COLLIDABLE>(*nested_semi_lagrangian_collidable,body_list,
...
//#####################################################################
template<class T_GRID> void LEVELSET_ADVECTION<T_GRID>::
HJ_WENO(const int m,const SCALAR dx,const ARRAY<SCALAR,VECTOR<int,1> >& phi,ARRAY<SCALAR,VECTOR<int,1> >& phix_minus,ARRAY<SCALAR,VECTOR<int,1> >& phix_plus) const
{
...
//#####################################################################
template<class T_GRID> void LEVELSET_ADVECTION<T_GRID>::
HJ_ENO(const int order,const int m,const SCALAR dx,const ARRAY<SCALAR,VECTOR<int,1> >& phi,ARRAY<SCALAR,VECTOR<int,1> >& phix_minus,ARRAY<SCALAR,VECTOR<int,1> >& phix_plus) const
{
    ...
    if(order == 1) for(int i=1;i<=m;i++){phix_minus(i)=D1(i-1);phix_plus(i)=D1(i);}
        ...
        phix_minus(i)=ADVECTION_SEPARABLE_UNIFORM<GRID<TV>,SCALAR>::ENO(dx,D1(i-1),D2(i-2),D2(i-1));
        phix_plus(i)=ADVECTION_SEPARABLE_UNIFORM<GRID<TV>,SCALAR>::ENO(dx,D1(i),-D2(i),-D2(i-1));}
    else if(order == 3) for(int i=1;i<=m;i++){
        phix_minus(i)=ADVECTION_SEPARABLE_UNIFORM<GRID<TV>,SCALAR>::ENO(dx,D1(i-1),D2(i-2),D2(i-1),D3(i-3),D3(i-2),D3(i-1));
        phix_plus(i)=ADVECTION_SEPARABLE_UNIFORM<GRID<TV>,SCALAR>::ENO(dx,D1(i),-D2(i),-D2(i-1),D3(i),D3(i-1),D3(i-2));}
}
//#####################################################################
