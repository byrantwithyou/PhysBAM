...
//        assert(!example.fluids_parameters.surface_tension && !example.fluids_parameters.variable_surface_tension);
    ...
    if(example.restart){
        ...
        example.Update_Fluid_Parameters((T)1./example.frame_rate,time);
        T_ARRAYS_SCALAR phi_dirichlet_regions;FAST_LEVELSET<GRID<TV> > levelset_dirichlet_regions(grid,phi_dirichlet_regions);
...
Project_Fluid(const T dt_projection,const T time_projection,const int substep)
{
    ...
    if(!fluids_parameters.analytic_test){
        T_ARRAYS_SCALAR phi_for_dirichlet_regions;FAST_LEVELSET<GRID<TV> > levelset_for_dirichlet_regions(grid,phi_for_dirichlet_regions); // for Dirichlet boundaries, surface tension and extrapolation
        ...
        if(euler){
            ...
            T_ARRAYS_SCALAR phi_for_pseudo_dirichlet_regions;FAST_LEVELSET<GRID<TV> > levelset_for_pseudo_dirichlet_regions(grid,phi_for_pseudo_dirichlet_regions);
