...
{
    for(int i=0;i<fluids_parameters.number_of_regions;i++){
        FAST_LEVELSET<GRID<TV> >& levelset=fluids_parameters.particle_levelset_evolution->Levelset(i);
...
Get_Levelset_Velocity(const T_GRID& grid,LEVELSET_MULTIPLE<T_GRID>& levelset_multiple,T_FACE_ARRAYS_SCALAR& V_levelset,const T time) const
{
    ...
    if(fluids_parameters.pseudo_dirichlet_regions.Number_True()>0){
        T_ARRAYS_SCALAR phi_for_pseudo_dirichlet_regions;T_GRID grid_temp(grid);FAST_LEVELSET<GRID<TV> > levelset_for_pseudo_dirichlet_regions(grid_temp,phi_for_pseudo_dirichlet_regions);
