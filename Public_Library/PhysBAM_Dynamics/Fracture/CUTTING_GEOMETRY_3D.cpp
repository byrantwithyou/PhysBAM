...
Split_Existing_Polygon_Edges()
{
    ...
    for(int new_intersection=intersection_registry->index_for_last_old_intersection+1;new_intersection<=intersection_registry->Number_Of_Intersections();new_intersection++){
        ...
        for(int j=0;j<simplices.m;j++) for(int k=j+1;k<simplices.m;k++){
            ...
            if(verbose) LOG::cout<<"Split edge "<<closest_particle_left<<", "<<closest_particle_right<<" with particle "<<new_intersection<<std::endl;}}
}
//#####################################################################
...
{    
    ...
    robust_interactions.Triangle_Segment_Intersection_Weights(cutting_simplices->simplices(simplices(triangle_array_index)).weights,
        cutting_simplices->Weight_For_Nodes_In_Simplex(simplices(0==triangle_array_index?2:1),shared_edge),all_weights(triangle_array_index),segment_weight);
...
Intersect_Simplex_With_Old_Simplices_In_Embedding(const int tet,const int new_simplex)
{
    ...
    for(int i=0;i<old_simplices.m;i++) for(int j=i+1;j<old_simplices.m;j++){
        ...
        if(cutting_simplices->Simplex_Is_Fake(simplices(0)) && cutting_simplices->Simplex_Is_Fake(simplices(1)) && cutting_simplices->Simplex_Is_Fake(simplices(2)))
            continue;
        ...
        if(points_all_shared.m>=2) continue; // they all share at least an edge
        else if(points_all_shared.m==1){int shared_node=points_all_shared(0); // they all share 1 point
            ...
            const VECTOR<TV,3>& simplex_weights_wrt_tet=cutting_simplices->simplices(simplices(0)).weights;
            TV particle_weights_wrt_tet=all_weights(0)(0)*simplex_weights_wrt_tet(0)+all_weights(0)(1)*simplex_weights_wrt_tet(1)+
                ((T)1-all_weights(0).Sum())*simplex_weights_wrt_tet(2);
        ...
        for(int i=0;i<2;i++){
            ARRAY<int> shared_nodes;cutting_simplices->Shared_Nodes_On_Simplices(VECTOR<int,2>(converted_simplices(i),converted_simplices(2)),shared_nodes);
            if(shared_nodes.m==2 && !cutting_simplices->Simplex_Is_Fake(converted_simplices(2-i))){ // the simplex intersecting the edge shouldn't be fake (1 edges never intersect)
                ...
                bool intersects=SIMPLEX_INTERACTIONS<T>::Intersection(cutting_simplices->simplices(simplices(2-i)).weights,
        ...
        {ARRAY<int> shared_nodes;cutting_simplices->Shared_Nodes_On_Simplices(VECTOR<int,2>(converted_simplices(0),converted_simplices(1)),shared_nodes);
        if(shared_nodes.m==2 && !cutting_simplices->Simplex_Is_Fake(converted_simplices(2))){ // can't intersected shared edge on old simplices with fake new simplex
            VECTOR<int,2> shared_edge(shared_nodes(0),shared_nodes(1));
            // check through other pairs of simplices that share the same edge to see if it already exists
            if(intersection_registry->intersections_on_simplex.m>=converted_simplices(2)){ // TODO: why can this use intersections_on_simplex on the previous two don't?
            ...
            bool intersects=SIMPLEX_INTERACTIONS<T>::Intersection(cutting_simplices->simplices(simplices(2)).weights,
                cutting_simplices->Weight_For_Nodes_In_Simplex(simplices(0),shared_edge));
            if(intersects){
                VECTOR<VECTOR<T,2>,3> all_weights;
                Get_Simplex_Weights_For_Edge_Triangle_Intersection(simplices,3,shared_edge,all_weights);
                Register_Cut_Intersection(converted_simplices,all_weights,0);}
        ...
        VECTOR<VECTOR<T,2>,3> all_weights;
        bool intersects=SIMPLEX_INTERACTIONS<T>::Three_Triangle_Intersection_Barycentric_Coordinates(weights_for_simplices[0],weights_for_simplices[1],weights_for_simplices[2],
            all_weights[0],all_weights[1],all_weights[2]);
        ...
        for(int i=0;i<particles_for_polygon.m;i++){const ARRAY<int>& particles=particles_for_polygon(i);
            ...
            if(j>=0){ // TODO: this seems to be just a call to Point_Is_Inside_Unoriented_Polygon
                ...
                VECTOR<T,2> X1=intersection_registry->Get_Simplex_Weights_Of_Intersection(p1,simplex),
                    ...
                    X=intersection_registry->Get_Simplex_Weights_Of_Intersection(nodes[1],simplex),
