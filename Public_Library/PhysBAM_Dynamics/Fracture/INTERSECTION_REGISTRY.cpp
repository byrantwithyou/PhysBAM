...
Intersection_List_For_Cuts(const VECTOR<int,d2>& simplices,const VECTOR<int,d+1>& element_nodes,ARRAY<int>& intersection_list,typename ENABLE_IF<(d2<=d),UNUSABLE>::TYPE unusable)
{
    ...
    for(int i=1;i<d2;i++) if(intersections_on_simplex(simplices_copy[i]).m<intersections_on_simplex(simplices_copy[0]).m) exchange(simplices_copy[0],simplices_copy[i]);
    intersection_list=intersections_on_simplex(simplices_copy[0]);
    for(int i=1;i<d2;i++) for(int j=intersection_list.m-1;j>=0;j--) if(!simplices_on_intersection(intersection_list(j)).Contains(simplices_copy[i])) intersection_list.Remove_Index_Lazy(j);
...
Get_Simplex_Weights_Of_Intersection(const int intersection,const int simplex)
{
    ...
    if(index<0) index=simplices_on_intersection(intersection).Find(cutting_simplices.simplices(simplex).parent);
    assert(index>=0);
