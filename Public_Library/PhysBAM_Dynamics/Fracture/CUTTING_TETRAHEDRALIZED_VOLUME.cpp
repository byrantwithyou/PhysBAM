...
Initialize_Original_Embedding(const TETRAHEDRALIZED_VOLUME<T>& original_tetrahedralized_volume)
{
    ...
    current_tetrahedralized_volume->particles.Initialize(original_tetrahedralized_volume.particles);
    ...
    intersection_registry->Preallocate_Intersections(embedding_vertices.Size());
    intersection_registry->Preallocate_Simplices(original_embedding_faces_count);
    cutting_particles.Preallocate(embedding_vertices.Size());
    ...
    for(int face=0;face<embedding_faces.elements.m;face++){const VECTOR<int,3>& face_nodes=embedding_faces.elements(face);
        ...
        for(int i=0;i<tets_on_face.m;i++){int tet=tets_on_face(i);
            ...
            regions_per_tet(tet)(0).Append(current_cutting_polygons.m);}}
...
Find_Triangle_Tet_Intersections(const int first_new)
{
    ...
    for(int cutting_triangle_index=first_new;cutting_triangle_index<cutting_triangles.m;cutting_triangle_index++){
        ...
        for(int i=0;i<tet_intersection_list.m;i++){
    ...
    for(int cseg=0;cseg<cutting_segments.m;cseg++) if(cutting_edge_triangles(cseg).m==1 && cutting_edge_triangles(cseg)(0)>=first_new){
        // insert global fake cutting triangle
        const VECTOR<int,3>& real_bordering_triangle_nodes=cutting_triangles(cutting_edge_triangles(cseg)(0));
        ...
        if(Segment_Reversed_In_Triangle(real_bordering_triangle_nodes, cut_nodes))
            exchange(ordered_cut_nodes(0),ordered_cut_nodes(1));
...
Find_Triangle_Triangle_Intersections()
{
    ...
    for(int tet=0;tet<current_tetrahedralized_volume->mesh.elements.m;tet++){
        ...
        triangulated_surface.particles.Add_Elements(3*simplices.m);
        ...
        for(int i1=0;i1<simplices.m;i1++){
            ...
            if(verbose) LOG::cout<<"for simplex index="<<simplex1<<", simplices.Subset(intersection_list)="<<simplices.Subset(intersection_list);
            ...
            if(verbose)
                LOG::cout<<"    intersecting simplices: "<<intersecting_simplices_per_simplex(simplex1);}
        ...
        for(int i=last_old_simplex_index+1;i<simplices.m;++i){
            ...
            for(int j1=0;j1<intersecting_simplices.m;++j1){
                ...
                for(int j2=j1+1;j2<intersecting_simplices.m;++j2){
                    ...
                    if(verbose) LOG::cout<<" Checking old simplices "<<old_simplex1<<" and "<<old_simplex2<<" with new simplex "<<new_simplex<<std::endl;
...
{
    if(cutting_simplices->Simplex_Is_Fake(simplices[0]) && cutting_simplices->Simplex_Is_Fake(simplices[1]) && cutting_simplices->Simplex_Is_Fake(simplices[2])) return;
    ...
    if(shared_nodes.m>=2) return; // they are all at least on the same edge
        ...
        int shared_node=shared_nodes(0); // they share a single point
        ...
        for(int i=0;i<3;i++) all_weights[i]=Get_Node_Weights_From_Triangle(shared_node,cutting_simplices->simplices(simplices[i]).nodes);
        // check if the node is inside the tet
        const CUTTING_SIMPLEX<T,3>& simplex1=cutting_simplices->simplices(simplices[0]);
        ...
        const ARRAY<int>& intersecting_simplices=intersecting_simplices_per_simplex(new_simplex);
        for(int i=0;i<intersecting_simplices.m;i++){
    ...
    for(int i=0;i<2;i++){
        shared_nodes.Resize(0);
        cutting_simplices->Shared_Nodes_On_Simplices(VECTOR<int,2>(converted_simplices[i],converted_simplices[2]),shared_nodes);
        if(shared_nodes.m==2 && !cutting_simplices->Simplex_Is_Fake(converted_simplices[2-i])){ // the simplex intersecting the edge shouldn't be fake (2 edges never intersect)
            VECTOR<int,2> shared_edge(shared_nodes(0),shared_nodes(1));
            ...
            const ARRAY<int>& intersecting_simplices=intersecting_simplices_per_simplex(new_simplex);
            for(int j=0;j<intersecting_simplices.m;j++){
    ...
    shared_nodes.Resize(0);
    cutting_simplices->Shared_Nodes_On_Simplices(VECTOR<int,2>(converted_simplices[0],converted_simplices[1]),shared_nodes);
    if(shared_nodes.m==2&&!cutting_simplices->Simplex_Is_Fake(converted_simplices[2])){
        // the simplex intersecting the edge shouldn't be fake
        VECTOR<int,2> shared_edge(shared_nodes(0),shared_nodes(1));
        // check if it already exists
        if(intersection_registry->intersections_on_simplex.m>=converted_simplices[2]){
            const ARRAY<int>& intersections_on_new_simplex=intersection_registry->intersections_on_simplex(converted_simplices[2]);
            for(int i=0;i<intersections_on_new_simplex.m;i++){
        ...
        return;}
    // normal intersection?
    for(int i=0;i<3;i++) if(cutting_simplices->Simplex_Is_Fake(simplices[i])) return; // we know not all are fake from above, but if any are fake we cannot intersect
    typedef typename CUTTING_SIMPLEX<T,3>::GET_ADAPTIVE_WEIGHTS_RESULT_TYPE WEIGHT_TYPE;
    // 3 tris, 3 nodes per tri, 3 coordinates per node
    VECTOR<VECTOR<VECTOR<WEIGHT_TYPE,3>,3>,3> adaptive_weights_for_simplices;
    for(int i=0;i<3;i++) cutting_simplices->simplices(simplices[i]).Get_Adaptive_Weights(adaptive_weights_for_simplices[i]);
    VECTOR<TV,3> all_weights;
    bool is_degenerate;
    bool intersects=Intersects_And_Intersection_Coordinates<void>(adaptive_weights_for_simplices[0],adaptive_weights_for_simplices[1],adaptive_weights_for_simplices[2],
        all_weights[0],all_weights[1],all_weights[2],abs_tol_for_level2_barycentric_coordinates,&is_degenerate);
    ...
    for(int i=0;i<3;i++) for(int j=0;j<2;j++) final_weights[i][j]=all_weights[i][j];
...
Split_Existing_Polygon_Edges()
{
    ...
    for(int i=intersection_registry->index_for_last_old_intersection+1;i<intersection_registry->Number_Of_Intersections();i++){
        ...
        for(int j=0;j<simplices.m;j++) for(int k=j+1;k<simplices.m;k++){
            ...
            VECTOR<T,2> delta_weights=intersection_registry->Get_Simplex_Weights_Of_Intersection(all_particles(0),simplex_1)-
                intersection_registry->Get_Simplex_Weights_Of_Intersection(all_particles(1),simplex_1);
            ...
            for(int p=0;p<all_particles.m;p++) if(all_particles(p)<i){ // only consider particles that existed before or have already split polygon edges
            ...
            if(nodes_shared.m==2 && !edges_seen.Set(VECTOR<int,2>(nodes_shared(0),nodes_shared(1)).Sorted())) continue;
            polygon_mesh.Split_Polygon_Edge(closest_particle_left,closest_particle_right,i);
            if(verbose) LOG::cout<<"Split edge "<<closest_particle_left<<", "<<closest_particle_right<<" with particle "<<i<<std::endl;}}
}
//#####################################################################
...
Split_Existing_Polygons()
{
    ...
    for(int tet=0;tet<current_tetrahedralized_volume->mesh.elements.m;tet++){
        const ARRAY<int>& simplices=simplices_per_current_tet(tet);
        int last_old_simplex_index=last_old_simplex_index_per_current_tet(tet);
        for(int i=0;i<last_old_simplex_index;i++){
    ...
    if(verbose){LOG::cout<<std::endl;for(int i=0;i<current_cutting_polygons.m;i++)
        LOG::cout<<"Cutting polygon "<<i<<" has polygon element "<<current_cutting_polygons(i).polygon_index<<": "<<polygon_mesh.elements(current_cutting_polygons(i).polygon_index)
                <<std::endl;}
}
//#####################################################################
...
Add_Polygons_On_New_Simplices()
{
    for(int tet=0;tet<current_tetrahedralized_volume->mesh.elements.m;tet++){
        ...
        for(int i=last_old_simplex_index+1;i<simplices.m;++i){
            ...
            if(verbose) LOG::cout<<"unoriented_existing_segments="<<unoriented_existing_segments<<std::endl;
            ...
            for(int j=0;j<unoriented_existing_segments.m;j++){
                const VECTOR<int,2>& unoriented_segment=unoriented_existing_segments(j);
                VECTOR<int,2> reversed_segment(unoriented_segment(1),unoriented_segment(0));
                ...
                if(count==2){
                    ...
                    TV actual_edge_dir=(Get_Particle_Position_In_Local_Tet_Space(unoriented_segment(1),new_simplex)-
                        Get_Particle_Position_In_Local_Tet_Space(unoriented_segment(0),new_simplex)).Normalized();
            ...
            for(int j=0;j<intersecting_simplices.m;j++){
                ...
                if(shared_nodes.m==2){
                    ...
                    Get_Oriented_Segments_On_Edge_Of_Two_Simplices(new_simplex,other_simplex,shared_nodes(0),shared_nodes(1),new_oriented_segments_on_simplex);
            ...
            for(int j=new_unoriented_segments_to_examine_later.m-1;j>=0;--j)
            ...
            if(verbose){
                LOG::cout<<"Killer segments: "<<killer_segments<<std::endl;
                LOG::cout<<"Segments to examine later: "<<new_unoriented_segments_to_examine_later<<std::endl;
                LOG::cout<<"Performing 2D region finding on segments: "<<std::endl<<oriented_segments_on_material_and_simplex_boundaries<<std::endl;}
            ...
            if(verbose) LOG::cout<<"**** Original material boundary within simplex "<<new_simplex<<std::endl<<unconnected_polygonal_regions<<std::endl;
            // prune away regions with killer segments
            for(int j=0;j<killer_segments.m;j++) for(int k=unconnected_polygonal_regions.m-1;k>=0;--k)
            ...
            if(verbose) LOG::cout<<"**** Polygon before adding new edges: "<<std::endl<<final_polygon_element_particles<<std::endl;
            ...
            for(int k=0;k<final_final_polygon_element_particles.m;k++){
                ...
                for(int p=0;p<final_final_polygon_element_particles(k).m;p++)
                    for(int q=final_final_polygon_element_particles(k)(p).m-1;q>=0;--q)
                ...
                if(verbose){
                    LOG::cout<<"***** Made final polygon "<<final_final_polygon_element_particles(k)<<std::endl;
                    LOG::cout<<"***** Made final polygon "<<reversed_polygon_particles<<std::endl;}}}
        if(verbose) LOG::cout<<"polygons_per_element("<<tet<<")="<<polygons_per_element(tet)<<std::endl;}
    ...
    const int cutting_simplex,const bool flipped,ARRAY<ARRAY<ARRAY<int > > >& final_polygon_element_particles) const
{
    ...
    if(!added_any) // do nothing if no segments were added
        ...
        if(verbose) LOG::cout<<"Simplex "<<cutting_simplex<<" has unconnected polygon regions "<<std::endl<<unconnected_polygonal_regions<<std::endl;
        ...
        if(verbose){LOG::cout<<"***** Final polygon: "<<std::endl<<final_polygon_element_particles<<std::endl;}}
}
//#####################################################################
    ...
    if(nodes_on_boundary.Contains(false)){
        ...
        for(int i=0;i<particles_for_polygon.m;i++){const ARRAY<int>& particles=particles_for_polygon(i);
            ...
            if(j>=0){
                ...
                VECTOR<T,2> x0=intersection_registry->Get_Simplex_Weights_Of_Intersection(nodes[1],simplex);
                ...
                if(angle30==0||angle31==angle30) PHYSBAM_FATAL_ERROR("nodes[1] should not be collinear with either v12 or v32; probable roundoff error?");
    ...
    if(!survive_incomplete_regions){
        while(unused_segments.m){
            ...
            for(;;){
                ...
                for(int i=0;i<segments.m;i++){VECTOR<int,2> next_nodes=segments(i);
                    if(segments(i)[0]==nodes[1]){
                        T angle;
                        if(segments(i)[1]!=nodes[0]){
                            VECTOR<T,2> X1=intersection_registry->Get_Simplex_Weights_Of_Intersection(nodes[0],cutting_simplex);
                            VECTOR<T,2> X2=intersection_registry->Get_Simplex_Weights_Of_Intersection(nodes[1],cutting_simplex);
                            VECTOR<T,2> next_X1=intersection_registry->Get_Simplex_Weights_Of_Intersection(next_nodes[0],cutting_simplex);
                            VECTOR<T,2> next_X2=intersection_registry->Get_Simplex_Weights_Of_Intersection(next_nodes[1],cutting_simplex);
            ...
            if(nodes[1]!=start) PHYSBAM_FATAL_ERROR("did not arrive back at start node");
        ...
        while(unused_segments.m){
            ...
            for(;;){
                ...
                for(int i=0;i<segments.m;i++) if(segments(i)[0]==seg_to_examine[1]){T angle=0;
                    if(segments(i)[1]!=seg_to_examine[0]){
                        VECTOR<T,2> cus1=intersection_registry->Get_Simplex_Weights_Of_Intersection(seg_to_examine[0],cutting_simplex);
                        VECTOR<T,2> cus2=intersection_registry->Get_Simplex_Weights_Of_Intersection(seg_to_examine[1],cutting_simplex);
                        VECTOR<T,2> cas1=intersection_registry->Get_Simplex_Weights_Of_Intersection(segments(i)[0],cutting_simplex);
                ...
                for(int i=0;i<segments.m;i++) if(segments(i)[1]==seg_to_examine[1]){T angle=0;
                    if(segments(i)[0]!=seg_to_examine[0]){
                        VECTOR<T,2> cus1=intersection_registry->Get_Simplex_Weights_Of_Intersection(seg_to_examine[0],cutting_simplex);
                        VECTOR<T,2> cus2=intersection_registry->Get_Simplex_Weights_Of_Intersection(seg_to_examine[1],cutting_simplex);
                        VECTOR<T,2> cas1=intersection_registry->Get_Simplex_Weights_Of_Intersection(segments(i)[1],cutting_simplex);
                        VECTOR<T,2> cas2=intersection_registry->Get_Simplex_Weights_Of_Intersection(segments(i)[0],cutting_simplex);
                        VECTOR<T,2> seg_to_examine_direction=cus2-cus1,candidate_seg_direction=cas2-cas1;
                        //if(seg_to_examine_direction.Magnitude()<1e-12) PHYSBAM_FATAL_ERROR(); // TODO: remove debugging
                        //if(candidate_seg_direction.Magnitude()<1e-12) PHYSBAM_FATAL_ERROR();
                        angle=VECTOR<T,2>::Oriented_Angle_Between(candidate_seg_direction,seg_to_examine_direction);
...
{
    if(verbose){
        LOG::cout<<"Starting inside/outside on unconnected regions: \n"<<unconnected_polygonal_regions;
    ...
    if(!something_is_contained) // nothing is inside anything else...
        for(int i=0;i<unconnected_polygonal_regions.m;i++){
            ARRAY<int> particles(unconnected_polygonal_regions(i).Project(0)); // if nothing is contained, it's a closed region
        ...
        for(int i=0;i<unconnected_polygonal_regions.m;i++){
            ...
            for(int j=0;j<unconnected_polygonal_regions(i).m;j++){
                ...
                VECTOR<T,2> seg_11=intersection_registry->Get_Simplex_Weights_Of_Intersection(seg_1(0),simplex);
                VECTOR<T,2> seg_12=intersection_registry->Get_Simplex_Weights_Of_Intersection(seg_1(1),simplex);
                VECTOR<T,2> seg_21=intersection_registry->Get_Simplex_Weights_Of_Intersection(seg_2(0),simplex);
                VECTOR<T,2> seg_22=intersection_registry->Get_Simplex_Weights_Of_Intersection(seg_2(1),simplex);
        ...
        for(int i=regions_in_order.m-1;i>=0;i--){int region_id=regions_in_order(i);if(!used(region_id)){
            ...
            particles_on_polygon.Append(ARRAY<int>(unconnected_polygonal_regions(region_id).Project(0)));
            ...
            for(int j=0;j<in_out_graph.Children(region_id).m;j++){int child_region=in_out_graph.Children(region_id)(j);
                ...
                if(depths(region_id)+1==depths(child_region) &&
                    (unjoined_region_orientations(region_id)!=unjoined_region_orientations(child_region) || unjoined_region_has_no_area(child_region))){
                        ...
                        particles_on_polygon.Append(ARRAY<int>(unconnected_polygonal_regions(child_region).Project(0)));}}}}}
}
//#####################################################################
...
Find_Material_Regions()
{
    if(verbose) for(int i=0;i<current_cutting_polygons.m;i++){
        const CUTTING_POLYGON& cutting_polygon=current_cutting_polygons(i);
        LOG::cout<<"Cutting polygon "<<i<<" has polygon element index "<<cutting_polygon.polygon_index<<" and is flipped? "<<(cutting_polygon.flipped?"yes":"no")
        ...
        while(count<polygons.m){regions_for_tet.Append(ARRAY<int>());ARRAY<int>& polygons_for_new_region=regions_for_tet.Last();
            ...
            while(!polygons_to_examine.Empty()){
                ...
                for(int i=0;i<edges_for_polygon.m;i++) for(int j=0;j<edges_for_polygon(i).m;j++){
                    ...
                    int node_1=(segment_flipped_in_mesh?polygon_segment_mesh.elements(segment_index)(1):polygon_segment_mesh.elements(segment_index)(0));
                    int node_2=(segment_flipped_in_mesh?polygon_segment_mesh.elements(segment_index)(0):polygon_segment_mesh.elements(segment_index)(1));
            ...
            if(polygons_for_new_region.m<=3){
                LOG::cout<<" Count is "<<count<<" of "<<polygons.m<<", wants to add polygons "<<polygons_for_new_region<<std::endl;
                PHYSBAM_FATAL_ERROR();}
            if(verbose) LOG::cout<<"Tet "<<tet<<" added a region: "<<polygons_for_new_region<<std::endl;}}
}
//#####################################################################
...
Determine_Duplicate_Tets_And_Duplicate_Particles()
{
    ...
    if(verbose){LOG::cout<<"number of original tets: "<<current_tetrahedralized_volume->mesh.elements.m<<std::endl<<"number of total tets: "<<num_new_tets<<std::endl;}
}
//#####################################################################
    ...
    ARRAY<int>& new_parents,ARRAY<T>& parent_weights) const
{
    ...
    if(boundaries.m==0){
        ...
        int cutting_triangle_simplex=intersection_registry->simplices_on_intersection(orig_intersection)(0);
        TV orig_particle_weights_wrt_tet=Get_Particle_Position_In_Local_Tet_Space(orig_intersection,cutting_triangle_simplex);
        for(int i=0;i<3;i++) parent_weights.Append(orig_particle_weights_wrt_tet(i));
        parent_weights.Append((T)1-orig_particle_weights_wrt_tet.Sum());}
    // face node
    else if(boundaries.m==1){
        int simplex=boundaries(0);const VECTOR<int,3> boundary_nodes=cutting_simplices->simplices(simplex).nodes;
        for(int i=0;i<3;i++) new_parents.Append(duplicate_particles_for_nodes(vertices_for_tet.Find(boundary_nodes(i))));
        const VECTOR<T,2>& weights_on_simplex=intersection_registry->Get_Simplex_Weights_Of_Intersection(orig_intersection,simplex);
        for(int i=0;i<2;i++) parent_weights.Append(weights_on_simplex(i));
        parent_weights.Append((T)1-weights_on_simplex.Sum());}
    // edge node
    else{
        ARRAY<int> shared_nodes;cutting_simplices->Shared_Nodes_On_Simplices(VECTOR<int,2>(boundaries(0),boundaries(1)),shared_nodes);assert(shared_nodes.m==2);
        for(int i=0;i<2;i++) new_parents.Append(duplicate_particles_for_nodes(vertices_for_tet.Find(shared_nodes(i))));
        const VECTOR<T,2>& weights_on_simplex=intersection_registry->Get_Simplex_Weights_Of_Intersection(orig_intersection,boundaries(0));
        const VECTOR<int,3> nodes_on_simplex=cutting_simplices->simplices(boundaries(0)).nodes;
        for(int i=0;i<2;i++){int index=nodes_on_simplex.Find(shared_nodes(i));assert(index>=0);
...
Duplicate_And_Merge_Elements()
{
        ...
        for(int p=0;p<tet_indices_for_this_face.m;p++) for(int q=p+1;q<tet_indices_for_this_face.m;q++){
            ...
            for(int j=0;j<regions_per_tet(tet_index_1).m;j++) for(int k=0;k<regions_per_tet(tet_index_2).m;k++){bool join=false;
                ...
                if(join){int dup_tet_1=new_tets_per_current(tet_index_1)(j);int dup_tet_2=new_tets_per_current(tet_index_2)(k);
                    for(int y=0;y<3;y++){int orig_tet_node=particles_on_current_face(y);int particle_id=cutting_particles.Particle_Id_From_Tet_Node(orig_tet_node);
    ...
    for(int opar=0;opar<uncollapsed_new_particles_per_all_current_particle_ids.m;opar++){
        for(int i=0;i<uncollapsed_new_particles_per_all_current_particle_ids(opar).m;i++) for(int j=i+1;j<uncollapsed_new_particles_per_all_current_particle_ids(opar).m;j++){
            ...
            for(int k=0;k<parents_1.m;k++) if(union_vertices.Find(parents_1(k))!=union_vertices.Find(parents_2(k))){
    ...
    int dup_particle_count=0;for(int i=0;i<union_vertices.parents.m;i++) if(union_vertices.Is_Root(i)) dup_particle_count++;
    new_particles.Preallocate(new_particles.Size()+dup_particle_count);
    previous_particle_index_per_new_particle_index.Resize(new_particles.Size()+dup_particle_count,false,false);
    ...
    new_collapsed_tet_particle_per_old_tet_particle.Resize(current_tetrahedralized_volume->particles.Size());
    for(int i=0;i<union_vertices.parents.m;i++) if(union_vertices.Is_Root(i)){int particle_index=new_particle_indices(i)=new_particles.Add_Element();
        ...
        if(current_particle_id_for_this_new_particle<current_tetrahedralized_volume->particles.Size()){
    ...
    for(int otet=0;otet<new_tets_per_current.m;otet++){const VECTOR<int,4>& original_node=current_embedding_tetrahedrons(otet);
        ...
        for(int i=new_tets_per_current(otet).m-1;i>=0;i--){int dtet_1=new_tets_per_current(otet)(i);
            ...
            VECTOR<int,4> collapsed_dup_particles_1;for(int k=0;k<4;k++){int particle_index=-2;
                ...
                assert(collapsed_dup_particles_1(k)>0 && collapsed_dup_particles_1(k)<new_particles.Size());}
            bool create=true;
            for(int j=i-1;j>=0;j--){int dtet_2=new_tets_per_current(otet)(j);VECTOR<int,4> collapsed_dup_particles_2;
                for(int k=0;k<4;k++){int particle_index=-2;int poind_id=cutting_particles.Particle_Id_From_Tet_Node(original_node(k));
                    ...
                    assert(collapsed_dup_particles_2(k)>0 && collapsed_dup_particles_2(k)<new_particles.Size());}
    ...
    for(int pcdup=0;pcdup<new_parents_per_new_particle.m;pcdup++){
        if(union_vertices.Is_Root(pcdup)){ARRAY<int>& parents=new_parents_per_new_particle(pcdup);ARRAY<int> final_parents(parents.m);
            for(int i=0;i<parents.m;i++){
                final_parents(i)=new_particle_indices(union_vertices.Find(parents(i)));assert(final_parents(i)<next_tetrahedralized_volume->particles.Size());}
...
Compute_World_Space_Position_Of_Uncollapsed_Particle(const int uncollapsed_particle) const
{
    ...
    for(int i=0;i<uncollapsed_parents.m;i++){
        ...
        assert(original_tet_node>=0 && original_tet_node<current_embedding_particles.Size());
...
Create_Boundary_Surface()
{
    ...
    final_duplicated_boundary_mesh.Set_Number_Nodes(duplicate_particles.Size());
    ...
    // set up mesh
    boundary_polygons.Set_Number_Nodes(duplicate_particles.Size());
    ...
    for(int new_polygon=0;new_polygon<boundary_polygons.elements.m;new_polygon++){
        ...
        for(int k=0;k<polygon_particles.m;k++) if(polygon_particles(k).m>=3){
            const ARRAY<int>& particles_for_piece=polygon_particles(k);
            for(int p=0;p<particles_for_piece.m;p++){
                ...
                if(cutting_polygon.flipped) exchange(bary[0],bary[1]);
                fake_positions.Append(VECTOR<T,2>(clamp<T>(bary[0],0,1),clamp<T>(bary[1],0,1)));}
        ...
        for(int k=0;k<triangles.m;k++){
            final_duplicated_boundary_mesh.elements.Append(VECTOR<int,3>(
                real_particle_per_fake_particle(triangles(k)[0]),real_particle_per_fake_particle(triangles(k)[2]),real_particle_per_fake_particle(triangles(k)[1])));}}
}
//#####################################################################
...
Build_New_Cutting_Simplices_And_Intersection_Registry()
{
    ...
    if(verbose){
        LOG::cout<<"Old cuttings simplices"<<std::endl;cutting_simplices->Print();
        LOG::cout<<"New cuttings simplices"<<std::endl;new_cutting_simplices->Print();
        LOG::cout<<"Intersection registry before: "<<std::endl;intersection_registry->Print();
        LOG::cout<<"Intersection registry after: "<<std::endl;new_intersection_registry->Print();
        LOG::cout<<"Old cutting polygons: "<<std::endl;
        for(int i=0;i<current_cutting_polygons.m;i++)
            LOG::cout<<"Polygon index = "<<current_cutting_polygons(i).polygon_index
        ...
        LOG::cout<<"New cutting polygons: "<<std::endl;
        for(int i=0;i<current_cutting_polygons.m;i++)
            LOG::cout<<"Polygon index = "<<new_cutting_polygons(i).polygon_index
        ...
        LOG::cout<<"Regions per tet before: "<<regions_per_tet<<std::endl;
        LOG::cout<<"Regions per tet after: "<<new_regions_per_tet<<std::endl;
        for(int i=0;i<cutting_polygons_per_cutting_simplex.m;i++)
            LOG::cout<<"Cutting polygons for simplex "<<i<<": "<<cutting_polygons_per_cutting_simplex(i)<<std::endl;}
    ...
    HASHTABLE<VECTOR<int,3>,VECTOR<int,3> >& new_sorted_to_original_boundary_nodes) const
{
    ...
    for(int face=0;face<embedding_faces.elements.m;face++){const VECTOR<int,3>& face_vertices=embedding_faces.elements(face);
        // unscramble face vertices to match old boundary that this one came from
        VECTOR<int,3> previous_face_vertices;
        for(int i=0;i<3;i++){int particle_id=current_particle_id_per_collapsed_new_particle(face_vertices(i));
    ...
    ARRAY<ARRAY<int> >& new_cutting_polygons_per_cutting_simplex) const
{
    ...
    for(int otet=0;otet<regions_per_tet.m;otet++){
        ...
        for(int region_index=0;region_index<regions_per_tet(otet).m;region_index++){
            ...
            ARRAY<int>& new_cutting_polygons_in_region=new_regions_per_tet(actual_dtet)(0);
            for(int j=0;j<cutting_polygons_for_region.m;j++){
                const CUTTING_POLYGON& cutting_polygon=current_cutting_polygons(cutting_polygons_for_region(j));
                const ARRAY<ARRAY<int> >& polygon_particles=polygon_mesh.elements(cutting_polygon.polygon_index);
    ...
    INTERSECTION_REGISTRY<T,3>& new_intersection_registry,const HASHTABLE<int,int>& new_child_simplex_to_old_child_simplex,CUTTING_PARTICLES& new_cutting_particles) const
{
    ...
    ARRAY<bool> tet_particle_flags(next_tetrahedralized_volume->particles.Size());
    tet_particle_flags.Fill(false);
    tet_particle_flags.Subset(all_tet_nodes).Fill(true);
    // build intersections
    for(int cp=0;cp<new_cutting_polygons.m;cp++){const CUTTING_POLYGON& cutting_polygon=new_cutting_polygons(cp);
        ...
        for(int i=0;i<particles_on_polygon.m;i++) for(int j=0;j<particles_on_polygon(i).m;j++){
            ...
            for(int k=0;k<old_simplices_on_old_particle.m;k++){int old_simplex=old_simplices_on_old_particle(k);
                ...
                if(cutting_simplices->simplices(old_simplex).type==CUTTING_SIMPLEX<T,3>::GLOBAL_CUT_FACE){
                    ...
                    for(int l=0;l<3;l++){int particle_id=cutting_particles.Particle_Id_From_Tet_Node(old_nodes(l));
                        assert(cutting_particles.particle_ids_types(particle_id)!=CUTTING_PARTICLES::INTERSECTION_ID);
            ...
            if(is_tet_node){
                ...
                for(int k=0;k<4;k++) if(current_particle_id_per_collapsed_new_particle(new_vertics_in_tet(k))==particle_id){found=true;new_tet_node=new_vertics_in_tet(k);}
    ...
    for(int i=0;i<tet_particle_flags.m;i++) if(tet_particle_flags(i)) new_cutting_particles.Add_Tet_Node_Id(i);
    // print
    for(int i=0;i<new_intersection_registry.simplices_on_intersection.m;i++){
        if(verbose) LOG::cout<<"Intersection "<<i<<" has simplices "<<new_intersection_registry.simplices_on_intersection(i)<<std::endl;
...
Get_Particles_On_Simplices(const VECTOR<int,2>& simplices,ARRAY<int>& particles) const
{
    ...
    for(int i=0;i<simplices.m;i++) if(cutting_simplices->simplices(simplices(i)).type!=CUTTING_SIMPLEX<T,3>::LOCAL_CUT_FACE){is_all_cutting_simplices=false;break;}
    ...
    if(is_all_cutting_simplices){VECTOR<int,2> converted_simplices;ARRAY<int> particles_on_cuts;
        for(int i=0;i<simplices.m;i++){converted_simplices(i)=cutting_simplices->simplices(simplices(i)).parent;
        assert((cutting_simplices->simplices(converted_simplices(i)).type==CUTTING_SIMPLEX<T,3>::GLOBAL_CUT_FACE));}
        int tet=cutting_simplices->simplices(simplices(0)).element_owner;
...
Get_Face_Weights_From_Tet(const VECTOR<int,3>& face_vertices,const VECTOR<int,4>& tet_vertices) const
{
    ...
    for(int i=0;i<3;i++){int index=tet_vertices.Find(face_vertices(i));weights(i)=VECTOR<T,3>();if(index<=3) weights(i)(index)=(T)1;}
    return weights;
}
//#####################################################################
...
Get_Triangle_Weights_From_Tet(const VECTOR<TV,3>& triangle_vertex_positions,const VECTOR<int,4>& tet_vertices) const
{
    ...
    for(int i=0;i<3;i++){
        all_weights(i)=TETRAHEDRON<T>::First_Three_Barycentric_Coordinates(triangle_vertex_positions(i),current_embedding_particles.X(tet_vertices(0)),
            current_embedding_particles.X(tet_vertices(0)),current_embedding_particles.X(tet_vertices(1)),current_embedding_particles.X(tet_vertices(2)));}
    return all_weights;
}
//#####################################################################
    ...
    const VECTOR<int,2>& shared_edge,VECTOR<VECTOR<T,2>,3>& all_weights,bool check_if_intersects) const
{
    ...
    edge_node_indices[0]=temp_triangle.nodes.Find(shared_edge[0]);
    ...
    all_weights[triangle_array_index][0]=triangle_coordinates[0];
    all_weights[triangle_array_index][1]=triangle_coordinates[1];
    T segment_weight=segment_coordinates[0];
    for(int i=0;i<3;i++){
        if(i==triangle_array_index) continue;
        all_weights[i]=Get_Weight_From_Segment_On_Face(segment_weight,shared_edge[0],shared_edge[1],cutting_simplices->simplices(simplices[i]).nodes);}
    return true;
}
//#####################################################################
...
Face_Reversed_In_Tetrahedron(const VECTOR<int,4>& tetrahedron_nodes,const VECTOR<int,3>& triangle_nodes,const GEOMETRY_PARTICLES<TV>& particles) const
{
    ...
    return TETRAHEDRON<T>::Signed_Volume(particles.X(triangle_nodes(0)),particles.X(triangle_nodes(1)),particles.X(triangle_nodes(2)),particles.X(other_node))<0;
}
//#####################################################################
...
Get_Particle_Position_In_Local_Tet_Space(const int particle,const int child_simplex) const
{
    ...
    TV particle_weights_wrt_tet=particle_weights_on_simplex(0)*simplex_weights_wrt_tet(0)+
        particle_weights_on_simplex(1)*simplex_weights_wrt_tet(1)+
        ((T)1-particle_weights_on_simplex.Sum())*simplex_weights_wrt_tet(2);
...
World_Coordinates_For_Intersection(int intersection_index,TV& world_coordinates) const
{
    ...
    for(int s=0;s<simplex_indices.m;s++){
    ...
    for(int s=0;s<simplex_indices.m;s++){
        ...
        if(simplex_type==CUTTING_SIMPLEX<T,3>::LOCAL_CUT_FACE){
            ...
            world_coordinates=particles.X(element(0))*local_coordinates(0)+particles.X(element(1))*local_coordinates(1)+
                particles.X(element(2))*local_coordinates(2)+particles.X(element(3))*(1-local_coordinates.Sum());
            return true;}}
    PHYSBAM_FATAL_ERROR();
    return false;
}
...
Draw_Polygon(const int simplex,const bool flipped,const ARRAY<ARRAY<VECTOR<int,2> > >& unconnected_polygonal_regions) const
{
    for(int i=0;i<unconnected_polygonal_regions.m;i++){
        ...
        for(int j=0;j<unconnected_polygonal_regions(i).m;j++){
            ...
            VECTOR<T,2> X1=intersection_registry->Get_Simplex_Weights_Of_Intersection(p[0],simplex);
            VECTOR<T,2> X2=intersection_registry->Get_Simplex_Weights_Of_Intersection(p[1],simplex);
            LOG::cout<<"  "<<X1<<" "<<X2<<" stem head headl cut "<<(flipped?-1:1)<<" arrow "<<std::endl;}
        LOG::cout<<hsb<<" 1 1 sethsbcolor fill"<<"% END RUN"<<std::endl;}
}
//#####################################################################
