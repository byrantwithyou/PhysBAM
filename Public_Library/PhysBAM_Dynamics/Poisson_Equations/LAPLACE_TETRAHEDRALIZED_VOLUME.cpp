...
Solve(SPARSE_MATRIX_FLAT_NXN<T>& A,VECTOR_ND<T>& b,const bool enforce_compatability,const bool recompute_preconditioner)
{
    ...
    VECTOR_ND<T> x(u.m),q,s,r,k,z;for(int t=0;t<u.m;t++)x(t)=u(t);
    pcg.Solve(A,x,b,q,s,r,k,z,tolerance,recompute_preconditioner);
    for(int t=0;t<u.m;t++)u(t)=x(t);
}
//#####################################################################
// Function Find_A
//#####################################################################
...
Find_A(SPARSE_MATRIX_FLAT_NXN<T>& A)
{
    ...
    for(int f=0;f<face_tetrahedrons.m;f++){
    ...
    for(int t=0;t<mesh.elements.m;t++)A.Set_Element(t,t,-row_sum(t));
}
//#####################################################################
// Function Find_A
//#####################################################################
...
Find_b(VECTOR_ND<T>& b)
{
    ...
    b.Resize(volume.m);
    for(int t=0;t<volume.m;t++){assert(volume(t)>0);b(t)=volume(t)*f(t);}
}
//#####################################################################
...
{
    ARRAY<T>& volume=*tetrahedralized_volume.tetrahedron_volumes;
    for(int t=0;t<volume.m;t++)A.Set_Element(t,t,A(t,t)+s*volume(t));
}
//#####################################################################
