...
Resize_Coupled_System_Vector(VECTOR_T& b) const
{
    ...
    if(!leakproof_solve) b.force_coefficients.Resize(solid_forces->Velocity_Dependent_Forces_Size());
    else b.force_coefficients.Resize(FORCE_AGGREGATE_ID());
    if(use_viscous_forces) b.viscous_force_coefficients.Resize(fluid_viscous_forces->Viscous_Forces_Size());
...
Test_Viscosity(const VECTOR_T& V,const VECTOR_T& B) const
{
    ...
    LOG::cout<<"Printing viscous errors"<<std::endl;
    LOG::cout<<temporary_forces<<std::endl;
}
//#####################################################################
    ...
    if(want_fluid){
        ...
        if(fluid_to_solid_interpolation)
            if(FLUID_TO_SOLID_INTERPOLATION_CUT<TV>* cut=dynamic_cast<FLUID_TO_SOLID_INTERPOLATION_CUT<TV>*>(fluid_to_solid_interpolation))
                cut->Dump_Extra_Velocities(temporary_faces);
...
//#####################################################################
template<class TV> void SYMMETRIC_POSITIVE_DEFINITE_COUPLING_SYSTEM<TV>::
Add_Dirichlet_Pressures_To_Velocity(const ARRAY<T,TV_INT>& pressure,ARRAY<T>& fluid_velocity_vector) const
{
...
//#####################################################################
template<class TV> void SYMMETRIC_POSITIVE_DEFINITE_COUPLING_SYSTEM<TV>::
Add_Surface_Tension(ARRAY<T>& fluid_velocity_vector) const
{
    ...
    if(n) LOG::cout<<"asdfasdf "<<mx<<"   "<<av/n<<std::endl;
}
//#####################################################################
// Function Set_Up_RHS
//#####################################################################
    ...
    const ARRAY<T,TV_INT>& p_advected_over_rho_c_squared_dt,const ARRAY<T,TV_INT>& p_advected,const ARRAY<T,TV_INT>& fluid_pressures)
{
    ...
    if(fluid_node){
        ARRAY<T> p_advected_over_rho_c_squared_dt_vector;
...
//#####################################################################
template<class TV> void SYMMETRIC_POSITIVE_DEFINITE_COUPLING_SYSTEM<TV>::
Gather(const KRYLOV_VECTOR_BASE<T>& bV,ARRAY<T>& fluid_velocity,GENERALIZED_VELOCITY<TV>& structure_velocity) const
{
    ...
    if(!leakproof_solve && solid_node){
        ...
        solid_interpolation->Transpose_Times_Add(V.lambda,structure_velocity);}
    if(fluid_node){
...
//#####################################################################
template<class TV> void SYMMETRIC_POSITIVE_DEFINITE_COUPLING_SYSTEM<TV>::
Scatter(const ARRAY<T>& fluid_velocity,const GENERALIZED_VELOCITY<TV>& structure_velocity,KRYLOV_VECTOR_BASE<T>& bF) const
{
...
//#####################################################################
template<class TV> void SYMMETRIC_POSITIVE_DEFINITE_COUPLING_SYSTEM<TV>::
Inverse_Mass(ARRAY<T>& fluid_velocity,GENERALIZED_VELOCITY<TV>& structure_velocity) const
{
    if(!leakproof_solve && solid_node){
...
Apply_Massless_Structure_Force_To_Fluid(ARRAY<T>& fluid_velocity,T time) const
{
    ...
    solid_forces->solid_body_collection.solid_force_collection.Add_Velocity_Independent_Forces(temporary_solids_velocity.V.array,temporary_solids_velocity.rigid_V.array,time);
...
Multiply(const KRYLOV_VECTOR_BASE<T>& bV,KRYLOV_VECTOR_BASE<T>& bF) const
{
    ...
    for(int i=0;i<one_over_rho_c_squared_flat.m;i++) F.pressure(i)+=one_over_rho_c_squared_flat(i)*V.pressure(i);
}
//#####################################################################
// Function Project
//#####################################################################
...
Apply_Preconditioner(const KRYLOV_VECTOR_BASE<T>& bV,KRYLOV_VECTOR_BASE<T>& bR) const  // solve MR=V
{
    ...
    if(use_full_ic){
        for(int i=0;i<full_precondition_in.m;i++)
...
Inner_Product(const KRYLOV_VECTOR_BASE<T>& bV1,const KRYLOV_VECTOR_BASE<T>& bV2) const
{
    ...
    T tmp=0;
    return mpi_solid_fluid?(mpi_solid_fluid->Reduce_Add((T)ret,tmp),tmp):ret;
}
//#####################################################################
...
Linf_Norm(const VECTOR_T& R) const
{
    ...
    LOG::cout<<"Max lambda error at constraint "<<max_lambda_error<<" = "<<coupling_convergence_norm<<std::endl;
    LOG::cout<<"Max forces error = "<<forces_convergence_norm<<std::endl;
    LOG::cout<<"Max viscous forces error = "<<viscous_forces_convergence_norm<<std::endl;
...
Test_Incompressibility(const ARRAY<T,FACE_INDEX<TV::dimension> >& fluid_velocity,const ARRAY<T>& constrained_fluids_velocity) const
{
    ...
    LOG::cout<<"divergence: ["<<mn/vol<<" "<<mx/vol<<"]     vol weighted: ["<<mn<<" "<<mx<<"]"<<std::endl;
}
//#####################################################################
...
Check_Constraints(const GENERALIZED_VELOCITY<TV>& solids_rhs,const ARRAY<T,FACE_INDEX<TV::dimension> >& fluids_rhs,const ARRAY<T>& constrained_rhs) const
{
    ...
    LOG::cout<<coupling_faces<<std::endl;
    ...
    LOG::cout<<"Incompressibility constraints"<<std::endl;
    LOG::cout<<pressure/index_map.grid.Cell_Size()<<std::endl;
}
//#####################################################################
...
Compute_Full_Preconditioner()
{
    ...
    for(int i=0;i<one_over_rho_c_squared_flat.m;i++) full_matrix(i,i)+=one_over_rho_c_squared_flat(i);
...
//#####################################################################
template<class TV> void SYMMETRIC_POSITIVE_DEFINITE_COUPLING_SYSTEM<TV>::
Exchange_Pressure(ARRAY<T>& pressure) const
{
...
//#####################################################################
// Function Exchange_Coupled_Vector
//#####################################################################
template<class TV> void SYMMETRIC_POSITIVE_DEFINITE_COUPLING_SYSTEM<TV>::
Exchange_Coupled_Vector(VECTOR_T& V) const
{
    // TODO: Exchange enough information to do:
    //    solid node: solid_forces->Transpose_Times, solid_interpolation->Transpose_Times
    //    fluid node: fluid_interpolation->Transpose_Times, fluid_gradient->Times, fluid_viscous_forces->Transpose_Times
    Exchange_Pressure(V.pressure);
}
//#####################################################################
// Function Exchange_Velocities
//#####################################################################
template<class TV> void SYMMETRIC_POSITIVE_DEFINITE_COUPLING_SYSTEM<TV>::
Exchange_Velocities(ARRAY<T>& fluid_velocity,GENERALIZED_VELOCITY<TV>& structure_velocity) const
{
    // TODO: Exchange enough information to do:
    //    solid node: solid_forces->Times
    //    fluid node: fluid_interpolation->Times, fluid_gradient->Transpose_Times, fluid_viscous_forces->Times, solid_interpolation->Times
    if(mpi_solid_fluid) PHYSBAM_FATAL_ERROR("Implement this fully before using.");
