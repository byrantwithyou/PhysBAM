...
template<class T> struct FLUID_TO_SOLID_INTERPOLATION_CUT_DISPATCH<VECTOR<T,2> >
{
    ...
    static void Compute_Dirichlet_Cells(T_THIS& this_)
    {
        ...
        for(ce.i=0;ce.i<curve.mesh.elements.m;ce.i++){
            ...
            RANGE<TV_INT> box(index_map.grid.Index(segment.X.x));
            box.Enlarge_To_Include_Point(index_map.grid.Index(segment.X.y));
...
Remove_Degeneracy()
{
    ...
    for(int i=0;i<list.m;i++,pa=pa->next){
        ...
        LOG::cout<<"DEGENERACY FIX: ";
        if(x->next->e.i==z->e.i){x->next=z;LOG::cout<<"a";}
        if(pa->next->e.i==pa->e.i){pa->next=pa->next->next;LOG::cout<<"b";}
        pa->e.b=1;
        z->e.a=0;
        for(x=pa->next;x!=z;x=x->next){x->cell=pa->cell;LOG::cout<<"c";}
        LOG::cout<<std::endl;}

    for(LOOP_ENTRY<TV>* p=pa;;p=p->next){
        while(p->next->e.b<1e-6){
            LOG::cout<<"CLIP "<<p->next->e.a<<"  "<<p->next->e.b<<std::endl;
            p->next=p->next->next;
            p->next->e.a=0;}
        if(p->next==pa) break;}
