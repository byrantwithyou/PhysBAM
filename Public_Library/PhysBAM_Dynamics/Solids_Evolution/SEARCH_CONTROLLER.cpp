...
Add_Solid_Drag(const T time,ARRAY<TV> &F,ARRAY<TWIST<TV> > &rigid_F)
{
    for(int i=1;i<=solid_body_collection.solids_forces.m;i++){
        if(dynamic_cast<WIND_DRAG<TV>*>(&*solid_body_collection.solids_forces(i))||dynamic_cast<ETHER_DRAG<T_GRID>*>(&*solid_body_collection.solids_forces(i))){
            ...
            solid_body_collection.solids_forces(i)->Add_Velocity_Dependent_Forces(solid_body_collection.deformable_body_collection.particles.V,solid_body_collection.rigid_body_collection.rigid_body_particle.twist,F,rigid_F,time);}}
}
//#####################################################################
...
Evaluate_Force_To_Stay(T_FACE_ARRAYS_SCALAR& face_velocities,const T dt,const T time,int hack)
{
    ...
    if(hack){solid_body_collection.deformable_body_collection.particles.V*=(T)0;solid_body_collection.rigid_body_collection.rigid_body_particle.twist*=(T)0;solid_body_collection.rigid_body_collection.rigid_body_particle.angular_momentum*=(T)0;}
    else Set_PD_Targets();
    pd_step=true;
    bool fluid_affects_solid=false;
    solid_body_collection.rigid_body_collection.articulated_rigid_body.constrain_pd_directions=false;
