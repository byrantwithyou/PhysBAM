...
void Initialize_Bodies()
{
    ...
    if(test_number==15){
        ...
        rigid_body_collection.rigid_body_particle.frame(ground).t=TV((T).1,0,0);
        rigid_body_collection.rigid_body_particle.frame(ground).r=ROTATION<TV>::From_Rotation_Vector(TV(0,0,(T)pi/8));
