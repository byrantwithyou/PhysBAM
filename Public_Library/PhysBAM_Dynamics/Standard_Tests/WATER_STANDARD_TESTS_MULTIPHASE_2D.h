...
public:
    ...
    WATER_STANDARD_TESTS_MULTIPHASE_2D(SOLIDS_FLUIDS_EXAMPLE_UNIFORM<T_GRID>& example_input,FLUIDS_PARAMETERS_UNIFORM<T_GRID>& fluids_parameters_input,FLUID_COLLECTION<TV>& fluid_collection,
        RIGID_BODY_COLLECTION<TV>& rigid_body_collection_input)
        :WATER_STANDARD_TESTS_MULTIPHASE<T_GRID,WATER_STANDARD_TESTS_2D<T_GRID> >(example_input,fluids_parameters_input,fluid_collection,rigid_body_collection_input)
    {
    }
...
void Initialize_Bodies()
{
    ...
    if(test_number==15){
        ...
        rigid_body_collection.rigid_body_particle.frame(ground).t=TV((T).1,0);
        rigid_body_collection.rigid_body_particle.frame(ground).r=ROTATION<TV>::From_Angle((T)pi/8);
