...
#include <Fluids/PhysBAM_Compressible/Lagrange_Equations/ARTIFICIAL_VISCOSITY_VNR_2D.h>
...
Get_Artificial_Viscosity(EOS<T>& eos,GRID_LAGRANGE_2D<T>& grid,const ARRAY<T,VECTOR<int,2> >& mass,const ARRAY<T,VECTOR<int,2> >& u,const ARRAY<T,VECTOR<int,2> >& v,
                         const ARRAY<T,VECTOR<int,2> >& energy,ARRAY<T,VECTOR<int,2> >& Q1,ARRAY<T,VECTOR<int,2> >& Q2,ARRAY<T,VECTOR<int,2> >& Q3,ARRAY<T,VECTOR<int,2> >& Q4)
{
    int i,j;
    ...
    ARRAY<T,VECTOR<int,2> > NN1_x(0,m-1,0,n-1),NN1_y(0,m-1,0,n-1),NN2_x(0,m-1,0,n-1),NN2_y(0,m-1,0,n-1),NN3_x(0,m-1,0,n-1),NN3_y(0,m-1,0,n-1),NN4_x(0,m-1,0,n-1),NN4_y(0,m-1,0,n-1);
    ...
    for(i=0;i<m-1;i++) for(j=0;j<n-1;j++){
        ...
        if(V1_x(i,j)*NN3_x(i,j)+V1_y(i,j)*NN3_y(i,j) >= 0) Q1(i,j)=0;
            ...
            Q1(i,j)=constant*density_ave*sqr(velocity_jump1(i,j));
            if(limiter){
        ...
        if(V1_x(i,j+1)*NN4_x(i,j)+V1_y(i,j+1)*NN4_y(i,j) >= 0) Q2(i,j)=0;
            ...
            Q2(i,j)=constant*density_ave*sqr(velocity_jump1(i,j+1));
            if(limiter){
        ...
        if(V2_x(i,j)*NN1_x(i,j)+V2_y(i,j)*NN1_y(i,j) >= 0) Q3(i,j)=0;
            ...
            Q3(i,j)=constant*density_ave*sqr(velocity_jump2(i,j));
            if(limiter){
        ...
        if(V2_x(i+1,j)*NN2_x(i,j)+V2_y(i+1,j)*NN2_y(i,j) >= 0) Q4(i,j)=0;
            ...
            Q4(i,j)=constant*density_ave*sqr(velocity_jump2(i+1,j));
            if(limiter){
