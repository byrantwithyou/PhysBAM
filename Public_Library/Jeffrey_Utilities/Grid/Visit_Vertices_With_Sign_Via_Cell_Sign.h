...
#ifndef PHYSBAM_PUBLIC_LIBRARY_JEFFREY_UTILITIES_GRID_VISIT_VERTICES_WITH_SIGN_VIA_CELL_SIGN_HPP
#define PHYSBAM_PUBLIC_LIBRARY_JEFFREY_UTILITIES_GRID_VISIT_VERTICES_WITH_SIGN_VIA_CELL_SIGN_HPP
...
template< int D, class T_SIGN_OF_CELL_INDEX, class T_VERTEX_VISITOR >
inline void
Visit_Vertices_With_Sign_Via_Cell_Sign(
    ...
    T_VERTEX_VISITOR vertex_visitor)
{
    ...
    for(int linear_index = 1; linear_index <= n; ++linear_index) {
        ...
        const int sign = Vertex_Sign_Via_Cell_Sign(cell_multi_index_bound, sign_of_cell_index, multi_index);
...
namespace Detail_Visit_Vertices_With_Sign_Via_Cell_Sign
{

template< int D, class T_SIGN_OF_CELL_INDEX, class T_VERTEX_VISITOR >
struct VISIT_VERTICES_WITH_SIGN_VIA_CELL_SIGN_HELPER;

} // namespace Detail_Visit_Vertices_With_Sign_Via_Cell_Sign

template< int D, class T_SIGN_OF_CELL_INDEX, class T_VERTEX_VISITOR >
inline void
Visit_Vertices_With_Sign_Via_Cell_Sign_MT(
    const unsigned int n_thread,
    const MULTI_INDEX_BOUND<D>& multi_index_bound,
    const T_SIGN_OF_CELL_INDEX& sign_of_cell_index,
    const T_VERTEX_VISITOR& vertex_visitor)
{
    typedef Detail_Visit_Vertices_With_Sign_Via_Cell_Sign::
        VISIT_VERTICES_WITH_SIGN_VIA_CELL_SIGN_HELPER<
            D, T_SIGN_OF_CELL_INDEX, T_VERTEX_VISITOR
        > VISIT_VERTICES_WITH_SIGN_VIA_CELL_SIGN_HELPER_;
    assert(n_thread >= 1);
    For_Each_MT(
        n_thread,
        1, multi_index_bound.Size(),
        VISIT_VERTICES_WITH_SIGN_VIA_CELL_SIGN_HELPER_(
            multi_index_bound, sign_of_cell_index, vertex_visitor
        )
    );
}
...
#endif // #ifndef PHYSBAM_PUBLIC_LIBRARY_JEFFREY_UTILITIES_GRID_VISIT_VERTICES_WITH_SIGN_VIA_CELL_SIGN_HPP
