...
OPENGL_COMPONENT_GRID_BASED_VECTOR_FIELD_2D(const GRID<TV> &grid,const std::string &vector_field_filename)
    : OPENGL_COMPONENT("Grid Based Vector Field 2D"), opengl_grid_based_vector_field(*(new GRID<TV>(grid)), *(new ARRAY<VECTOR<T,2> ,VECTOR<int,2> >)), 
...
template<class T,class RW> RANGE<VECTOR<float,3> > OPENGL_COMPONENT_GRID_BASED_VECTOR_FIELD_2D<T,RW>::
Bounding_Box() const
{
