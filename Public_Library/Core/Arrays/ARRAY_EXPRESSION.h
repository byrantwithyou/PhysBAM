...
Array_Expression_Helper(const ARRAY_BASE<T0,T_ARRAY0,ID>& array0,const ARRAY_BASE<T1,T_ARRAY1,ID>& array1,OP op,
    enable_if_t<!(FIXED_SIZE_VECTOR<T_ARRAY0>::value || FIXED_SIZE_VECTOR<T_ARRAY1>::value),int>)
{
...
template<class T,class T_ARRAY,class ID> auto
operator+(const ARRAY_BASE<T,T_ARRAY,ID>& array,const enable_if_t<!is_same<typename T_ARRAY::SCALAR,T>::value,T>& s)
{return Array_Expression_Helper(array,[=](const T& x){return x+s;});}
...
template<class T,class T_ARRAY,class ID> auto
operator+(const enable_if_t<!is_same<typename T_ARRAY::SCALAR,T>::value,T>& s,const ARRAY_BASE<T,T_ARRAY,ID>& array)
{return Array_Expression_Helper(array,[=](const T& x){return s+x;});}
