...
#include <Compressible/Lagrange_Equations/LAGRANGE_1D.h>
...
{  
    ...
    // get grid lengths
    ARRAY<T,VECTOR<int,1> > L(VECTOR<int,1>(m-1));
    grid.Get_Lengths(L);
    ...
    ARRAY<T,VECTOR<int,1> > Q(VECTOR<int,1>(m-1));
    ...
    // update velocity
    for(i=0;i<m;i++) velocity(i)+=dt*F(i)/M_node(i);

    // boundary conditions - external velocities 
    for(i=0;i<m;i++) if(fixed_velocity(i)) velocity(i)=external_velocity(i);
    
    // update interal energy 
    for(i=0;i<m-1;i++) energy(i)+=dt*H(i)/mass(i);
}
//#####################################################################
// Function CFL
//#####################################################################
...
CFL()
{
    ...
    ARRAY<T,VECTOR<int,1> > S(VECTOR<int,1>(m-1));
    if(material_strength) for(i=0;i<m-1;i++) S(i)=-stiffness*(L(i)/L_not(i)-1);

    ARRAY<T,VECTOR<int,1> > timestep(VECTOR<int,1>(m-1));
    for(i=0;i<m-1;i++){
