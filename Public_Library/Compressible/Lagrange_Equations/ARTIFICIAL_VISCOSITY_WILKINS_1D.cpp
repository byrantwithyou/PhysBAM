...
template<class T> void ARTIFICIAL_VISCOSITY_WILKINS_1D<T>::
Get_Artificial_Viscosity(EOS<T>& eos,GRID_LAGRANGE_1D<T>& grid,const ARRAY<T,VECTOR<int,1> >& mass,const ARRAY<T,VECTOR<int,1> >& velocity,const ARRAY<T,VECTOR<int,1> >& energy,ARRAY<T,VECTOR<int,1> >& Q)
{
    ...
    ARRAY<T,VECTOR<int,1> > length(VECTOR<int,1>(m-1));
    grid.Get_Lengths(length);
    ARRAY<T,VECTOR<int,1> > u_jump(VECTOR<int,1>(m-1));
    for(i=0;i<m-1;i++) u_jump(i)=velocity(i+1)-velocity(i);

    for(i=0;i<m-1;i++){
        if(u_jump(i) >= 0) Q(i)=0;
        else{
            T density=mass(i)/length(i),sound_speed=eos.c(density,energy(i));
            Q(i)=density*(linear_constant*sound_speed*abs(u_jump(i))+quadratic_constant*sqr(u_jump(i))); 
            if(limiter){
