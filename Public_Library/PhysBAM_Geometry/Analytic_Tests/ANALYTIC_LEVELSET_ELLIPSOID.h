...
struct ANALYTIC_LEVELSET_ELLIPSOID:public ANALYTIC_IMPLICIT_SURFACE_LEVELSET<TV>
{
    ...
    ANALYTIC_LEVELSET_ELLIPSOID(TV cc,TV r_in,int c_i,int c_o): ANALYTIC_IMPLICIT_SURFACE_LEVELSET<TV>(c_i,c_o),cen(cc),r(r_in),jacobian() {
       for(int i=0;i<TV::m;i++) jacobian(i,i)=r.Magnitude_Squared()/r(i)/r(i);
    }
