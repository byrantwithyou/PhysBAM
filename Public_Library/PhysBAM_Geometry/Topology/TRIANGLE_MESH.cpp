...
Initialize_Equilateral_Mesh(const int m,const int n)
{
    Clean_Memory();number_nodes=m*n;elements.Exact_Resize(2*(m-1)*(n-1));
    int t=0;for(int i=0;i<m-1;i++)for(int j=0;j<n-1;j++){ // counterclockwise node ordering
        if(j%2){elements(t++).Set(i+m*j,i+1+m*j,i+m*j);elements(t++).Set(i+1+m*j,i+1+m*j,i+m*j);}
        else{elements(t++).Set(i+m*j,i+1+m*j,i+1+m*j);elements(t++).Set(i+m*j,i+1+m*j,i+m*j);}}
}
//#####################################################################
...
Initialize_Torus_Mesh(const int m,const int n)
{
    ...
    int t=0;for(int i=0;i<m;i++) for(int j=0;j<n;j++){ // counterclockwise node ordering
        int i1=i==m-1?1:i+1,j1=j==n-1?1:j+1;
...
Initialize_Herring_Bone_Mesh(const int m,const int n) // construct a regular m-by-n herring bone rectangular mesh
{
    Clean_Memory();number_nodes=m*n;elements.Exact_Resize(2*(m-1)*(n-1));
    int t=0;for(int i=0;i<m-1;i++)for(int j=0;j<n-1;j++){ // counterclockwise node ordering
        if(i%2){elements(t++).Set(i+m*j,i+1+m*j,i+m*j);elements(t++).Set(i+1+m*j,i+1+m*j,i+m*j);}
        else{elements(t++).Set(i+m*j,i+1+m*j,i+1+m*j);elements(t++).Set(i+m*j,i+1+m*j,i+m*j);}}
}
//#####################################################################
...
Initialize_Cylinder_Mesh(const int m,const int n,const bool create_caps)
{
    ...
    for(int j=0;j<n;j++){int j_1=j==n?0:j;
        for(int i=0;i<m-1;i++){elements(t++).Set(j+i*n,j+i*n,j_1+i*n);elements(t++).Set(j+i*n,j_1+i*n,j_1+i*n);}
