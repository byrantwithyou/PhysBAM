...
Initialize_Equilateral_Mesh(const int m,const int n)
{
    Clean_Memory();number_nodes=m*n;elements.Exact_Resize(2*(m-1)*(n-1));
    int t=0;for(int i=0;i<m-1;i++)for(int j=0;j<n-1;j++){ // counterclockwise node ordering
        if(j%2){elements(t++).Set(i+m*j,i+1+m*j,i+1+m*(j+1));elements(t++).Set(i+m*j,i+1+m*(j+1),i+m*(j+1));}
        else{elements(t++).Set(i+m*j,i+1+m*j,i+m*(j+1));elements(t++).Set(i+1+m*j,i+1+m*(j+1),i+m*(j+1));}}
}
//#####################################################################
...
Initialize_Torus_Mesh(const int m,const int n)
{
    ...
    int t=0;for(int i=0;i<m;i++) for(int j=0;j<n;j++){ // counterclockwise node ordering
        ...
        if(j&1){elements(t++).Set(i+m*j,i1+m*j,i1+m*j1);elements(t++).Set(i+m*j,i1+m*j1,i+m*j1);}
        else{elements(t++).Set(i+m*j,i1+m*j,i+m*j1);elements(t++).Set(i1+m*j,i1+m*j1,i+m*j1);}}
}
//#####################################################################
...
Initialize_Circle_Mesh(const int num_radial,const int num_tangential) // construct a circle
{
    ...
    int t=0,n=num_tangential;for(int i=0;i<num_radial-1;i++)for(int j=0;j<n;j++){ // counterclockwise node ordering
        if(j&1){elements(t++).Set(i*n+j,i*n+(j+1)%n,(i+1)*n+(j+1)%n);elements(t++).Set((i+1)*n+j,i*n+j,(i+1)*n+(j+1)%n);}
        else{elements(t++).Set(i*n+j,i*n+(j+1)%n,(i+1)*n+j);elements(t++).Set((i+1)*n+j,i*n+(j+1)%n,(i+1)*n+(j+1)%n);}}
}
//#####################################################################
...
Initialize_Herring_Bone_Mesh(const int m,const int n) // construct a regular m-by-n herring bone rectangular mesh
{
    Clean_Memory();number_nodes=m*n;elements.Exact_Resize(2*(m-1)*(n-1));
    int t=0;for(int i=0;i<m-1;i++)for(int j=0;j<n-1;j++){ // counterclockwise node ordering
        if(i%2){elements(t++).Set(i+m*j,i+1+m*j,i+1+m*(j+1));elements(t++).Set(i+m*j,i+1+m*(j+1),i+m*(j+1));}
        else{elements(t++).Set(i+m*j,i+1+m*j,i+m*(j+1));elements(t++).Set(i+1+m*j,i+1+m*(j+1),i+m*(j+1));}}
}
//#####################################################################
