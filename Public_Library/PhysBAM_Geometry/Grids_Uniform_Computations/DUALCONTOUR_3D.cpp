...
Generate_Topology()
{
    ...
    TV_INT i;
    for(i.x=0;i.x<m;i.x++)for(i.y=1;i.y<n;i.y++)for(i.z=1;i.z<mn;i.z++){ // generate one triangle pair per x edge
    ...
    for(i.x=1;i.x<m;i.x++)for(i.y=0;i.y<n;i.y++)for(i.z=1;i.z<mn;i.z++){ // generate one triangle pair per y edge
    ...
    for(i.x=1;i.x<m;i.x++)for(i.y=1;i.y<n;i.y++)for(i.z=0;i.z<mn;i.z++){ // generate one triangle pair per z edge
...
Generate_Vertices()
{
    ...
    int vertex=-1;
    for(int i=0;i<m;i++)for(int j=0;j<n;j++)for(int ij=0;ij<mn;ij++)if(vertices(i,j,ij)){ // generate vertices where needed
...
Ensure_Vertices_In_Correct_Cells()
{
    int vertex=-1;
    TV_INT i;
    for(i.x=0;i.x<grid.counts.x;i.x++)for(i.y=0;i.y<grid.counts.y;i.y++)for(i.z=0;i.z<grid.counts.z;i.z++) if(vertices(i)){
