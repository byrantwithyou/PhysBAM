...
public:
    ...
    T2 Clamped_To_Array(const T_GRID& grid,const ARRAY<T2,TV_INT>& u,const TV& X,bool& interpolation_valid) const
...
//#####################################################################
    ...
    T2 From_Base_Node(const T_GRID& grid,const ARRAYS_ND_BASE<T2,TV_INT>& u,const TV& X,const TV_INT& cell) const PHYSBAM_OVERRIDE;
    // TODO: this only works for cells, which would ideally be enforced at compile time
    T2 From_Base_Node(const T_GRID& grid,const ARRAYS_ND_BASE<T2,TV_INT>& u,const TV& X,const TV_INT& cell,bool& interpolation_valid,VECTOR<T2,2>* extrema=0) const;
    ...
    VECTOR<T2,2> Extrema_From_Base_Node(const T_GRID& grid,const ARRAYS_ND_BASE<T2,TV_INT>& u_min,const ARRAYS_ND_BASE<T2,TV_INT>& u_max,const TV& X,const TV_INT& cell) const PHYSBAM_OVERRIDE;
protected:
    T2 Trace_And_Get_Value(const T_GRID& grid,const ARRAYS_ND_BASE<T2,TV_INT>& u,const TV& X,const TV_INT& cell,TV_INT* cells,int& valid_mask,const int mask_value=0) const;
    virtual T2 Invalid_Value_Replacement(const T_GRID& grid,const ARRAYS_ND_BASE<T2,TV_INT>& u,const TV& X,const TV_INT& cell,const TV& intersection_point,
        const COLLISION_GEOMETRY_ID body_id,const int aggregate_id,bool& valid,const T ray_t_max=0) const;
    void Extrapolate_To_Invalid_Values(const T_GRID& grid,const ARRAYS_ND_BASE<T2,TV_INT>& u,const TV& X,const TV_INT& cell,T2* values,int& valid_mask) const;
//#####################################################################
};
}
#endif
