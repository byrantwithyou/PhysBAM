...
public:
    ...
    {for(int i=0;i<levelsets->m;i++) delete (*levelsets)(i);delete levelsets;}
    ...
    {for(int i=0;i<levelsets->m;i++) (*levelsets)(i)->Set_Custom_Normal_Interpolation(interpolation_input);}

    void Update_Box() PHYSBAM_OVERRIDE
    {box=RANGE<TV>::Empty_Box();
    for(int i=0;i<levelsets->m;i++){(*levelsets)(i)->Update_Box();box.Enlarge_To_Include_Box((*levelsets)(i)->box);}}
    ...
    {T minimum_edge_length=(T)FLT_MAX;for(int i=0;i<levelsets->m;i++) minimum_edge_length=min((*levelsets)(i)->Minimum_Cell_Size_Within_Box(box),minimum_edge_length);
    ...
    {T phi_value=(T)FLT_MAX;for(int i=0;i<levelsets->m;i++) phi_value=min((*levelsets)(i)->Extended_Phi(location),phi_value);
    return phi_value;}
    ...
    {T step=FLT_MAX;for(int i=0;i<levelsets->m;i++) step=min(step,(*levelsets)(i)->Integration_Step(phi));return step;}
    ...
    bool levelset_value=true;for(int i=0;i<levelsets->m;i++) levelset_value=levelset_value && (*levelsets)(i)->Lazy_Outside(location,contour_value);
    return levelset_value;}

    bool Lazy_Outside_Extended_Levelset(const TV& unclamped_X,const T contour_value=0) const PHYSBAM_OVERRIDE
    {bool levelset_value=true;for(int i=0;i<levelsets->m;i++) levelset_value=levelset_value && (*levelsets)(i)->Lazy_Outside_Extended_Levelset(unclamped_X,contour_value);
    return levelset_value;}
    ...
    {T min_phi=(T)FLT_MAX;for(int i=0;i<levelsets->m;i++) min_phi=min(min_phi,(*levelsets)(i)->Min_Phi());
    return min_phi;}
