    ...
    for(RANGE_ITERATOR<TV::m> it(range);it.Valid();it.Next())
        if(volume_monomials_needed(it.index)){
            ...
            for(int i=0;i<projected_elements.m;i++){
                ...
                integral+=monomial.Quadrature_Over_Primitive(V)*T_FACE::Normal(V)(dir);}
    ...
    for(RANGE_ITERATOR<TV::m> it(range);it.Valid();it.Next())
        if(surface_monomials_needed(it.index)){
            ...
            for(int i=0;i<interface_elements.m;i++){
                ...
                has_element[e]=true;
                precomputed_interface_integrals[e](it.index)+=monomial.Quadrature_Over_Primitive(V);}}
            ...
            if(op.subcell&(1<<block))
                for(int k=0;k<subcell_elements;k++)
                    if(has_element[k]){
                        ...
                        ib->Add_Entry(element_base+k,op.flat_index_diff_ref(subcell_cell),enclose_inside,integral);
                        ib->Add_Entry(element_base+k,op.flat_index_diff_ref(subcell_cell),!enclose_inside,sign2*integral);}}}
}
