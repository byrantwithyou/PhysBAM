...
template<class TV,int static_degree> void SYSTEM_VOLUME_BLOCK_COLOR<TV,static_degree>::
Add_Open_Subcell_Entries(int flat_index,int block,int color)
{
    for(int j=0;j<open_subcell_entries[block].m;j++) Add_Open_Entry(flat_index,color,open_subcell_entries[block](j));
}
