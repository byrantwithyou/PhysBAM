...
Construct_Surface_Meshes(const GRID<TV>& phi_grid,const ARRAY<T,TV_INT>& phi_value,const ARRAY<int,TV_INT>& phi_color)
{
    ...
    MARCHING_CUBES_COLOR<TV>::Get_Elements(phi_grid,interface,boundary,cell_to_element,phi_color,phi_value,phi_grid.counts.Product()*100,true);
}
//#####################################################################
// Function Interpolate_Level_Set_To_Double_Fine_Grid
//#####################################################################
template<class TV_INT,class T> static void Interpolate_Level_Set_To_Double_Fine_Grid_Helper(const RANGE<TV_INT>& range_input,
