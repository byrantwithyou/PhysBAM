...
INTERFACE_STOKES_SYSTEM_NEW(const GRID<TV>& grid_input,GRID<TV>& coarse_grid_input,ARRAY<T,TV_INT>& phi_input,bool periodic_bc_input)
...
Set_Matrix(const VECTOR<T,2>& mu)
{
    ...
    cdi=new CELL_DOMAIN_INTERFACE_NEW<TV>(grid,padding,grid.counts.x/coarse_grid.counts.x,object.mesh.elements.m,periodic_bc); 
    ...
    BASIS_INTEGRATION_UNIFORM_NEW<TV,2> biu(grid,coarse_grid,phi->phi,*cdi);
    ...
    VECTOR<SYSTEM_INTERFACE_BLOCK_HELPER_NEW<TV>,TV::m> helper_qu,helper_rhs_qu;
