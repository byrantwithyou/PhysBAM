...
Copy(const T c,const BASE& bv)
{
    ...
    for(int i=0;i<TV::m;i++)
        for(int s=0;s<2;s++)
            u(i)[s].Copy(c,v.u(i)[s]);
    for(int s=0;s<2;s++) p[s].Copy(c,v.p[s]);
    for(int i=0;i<TV::m;i++) q(i).Copy(c,v.q(i));
}
//#####################################################################
// Function Copy
//#####################################################################
...
Copy(const T c1,const BASE& bv1,const BASE& bv2)
{
    ...
    for(int i=0;i<TV::m;i++)
        for(int s=0;s<2;s++)
            u(i)[s].Copy(c1,v1.u(i)[s],v2.u(i)[s]);
    for(int s=0;s<2;s++) p[s].Copy(c1,v1.p[s],v2.p[s]);
    for(int i=0;i<TV::m;i++) q(i).Copy(c1,v1.q(i),v2.q(i));
}
//#####################################################################
// Function Print
//#####################################################################
...
Raw_Size() const
{
    ...
    for(int i=0;i<TV::m;i++) size+=q(i).m;
    return size;
}
//#####################################################################
// Function Raw_Get
...
Clone_Default() const
{
    ...
    for(int i=0;i<TV::m;i++) v->q(i).Resize(q(i).m);
    return v;
}
//#####################################################################
// Function Resize
...
Resize(const KRYLOV_VECTOR_BASE<T>& v)
{
    ...
    for(int i=0;i<TV::m;i++) q(i).Resize(cs.q(i).m);
}
//#####################################################################
// Function Dot
//#####################################################################
...
//#####################################################################
template<class TV> void INTERFACE_STOKES_SYSTEM_VECTOR<TV>::
Scale(const INTERFACE_STOKES_SYSTEM_VECTOR<TV>& v,const INTERFACE_STOKES_SYSTEM_VECTOR<TV>& c)
{
    for(int i=0;i<TV::m;i++)
        for(int s=0;s<2;s++)
            for(int k=0;k<u(i)[s].m;k++)
