...
int Inside(const TRIANGULATED_AREA<T>& ta,const VECTOR<T,2>& location,const T thickness_over_two)
{
    ...
    if(ta.bounding_box->Outside(location,thickness_over_two)) return -1;
    if(ta.hierarchy->box_hierarchy(ta.hierarchy->root).Outside(location,thickness_over_two)) return -1;
