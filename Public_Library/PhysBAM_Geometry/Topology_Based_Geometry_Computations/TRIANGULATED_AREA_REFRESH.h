...
void Initialize_Square_Mesh_And_Particles(TRIANGULATED_AREA<T>& ta,const GRID<TV>& grid,const bool reverse_triangles)
{
    ...
    ta.particles.Delete_All_Elements();ta.mesh.Initialize_Square_Mesh(m,n,reverse_triangles);ta.particles.Add_Elements(m*n);
...
{
    int particle=0;ta.particles.Delete_All_Elements();
    ta.mesh.Initialize_Circle_Mesh(num_radial,num_tangential);ta.particles.Add_Elements(num_radial*num_tangential);
...
{
    int m=grid.counts.x,n=grid.counts.y,particle=0;
    ta.particles.Delete_All_Elements();ta.mesh.Initialize_Herring_Bone_Mesh(m,n);ta.particles.Add_Elements(m*n);
    for(int j=0;j<n;j++) for(int i=0;i<m;i++) ta.particles.X(particle++)=grid.X(i,j);
}
//#####################################################################
...
{
    int m=grid.counts.x,n=grid.counts.y,particle=0;
    ta.particles.Delete_All_Elements();ta.mesh.Initialize_Equilateral_Mesh(m,n);ta.particles.Add_Elements(m*n);
...
void Compute_Nodal_Areas(TRIANGULATED_AREA<T>& ta,bool save_triangle_areas)
{
    ...
    ta.nodal_areas->Resize(ta.particles.Size());
