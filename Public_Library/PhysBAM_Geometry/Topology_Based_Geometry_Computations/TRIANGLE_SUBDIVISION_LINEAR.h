...
void Apply_Linear_Subdivision(TRIANGLE_SUBDIVISION& ts,ARRAY_VIEW<const TV> base_values,ARRAY_VIEW<TV> subdivided_values)
{
    ...
    for(int k=0;k<ts.triangle_mesh.segment_mesh->elements.m;k++)
        subdivided_values(ts.start_index_for_new_nodes-1+k)=(T).5*(base_values(ts.triangle_mesh.segment_mesh->elements(k)(0))+base_values(ts.triangle_mesh.segment_mesh->elements(k)(1)));
}
}
}
#endif
