...
int FLOOD_FILL_3D::
Flood_Fill(ARRAYS_ND_BASE<int,TV_INT>& colors,const ARRAYS_ND_BASE<bool,VECTOR<int,3> >& edge_is_blocked_x,const ARRAYS_ND_BASE<bool,VECTOR<int,3> >& edge_is_blocked_y,
    const ARRAYS_ND_BASE<bool,VECTOR<int,3> >& edge_is_blocked_z,ARRAY<bool>* color_touches_uncolorable_node)
{
    TV_INT seed_node;
    int fill_color=0;
...
void FLOOD_FILL_3D::
Fill_Single_Cell_Regions(ARRAYS_ND_BASE<int,TV_INT>& colors,const ARRAYS_ND_BASE<bool,VECTOR<int,3> >& edge_is_blocked_x,const ARRAYS_ND_BASE<bool,VECTOR<int,3> >& edge_is_blocked_y,
    const ARRAYS_ND_BASE<bool,VECTOR<int,3> >& edge_is_blocked_z,int& fill_color)
{
    TV_INT i;
    const TV_INT &a(colors.domain.min_corner),&b(colors.domain.max_corner);
    for(i.x=a.x;i.x<b.x;i.x++) for(i.y=a.y;i.y<b.y;i.y++) for(i.z=a.z;i.z<b.z;i.z++) if(colors(i)==-1){
...
Find_Uncolored_Node(const ARRAYS_ND_BASE<int,TV_INT>& colors,TV_INT& node_index)
{
    ...
    for(i.x=a.x;i.x<b.x;i.x++) for(i.y=a.y;i.y<b.y;i.y++) for(i.z=a.z;i.z<b.z;i.z++){
        ...
        if(colors(i)==-1){last_uncolored_node=node_index=i;return true;}}
    return false;
}
void FLOOD_FILL_3D::
Flood_Fill_From_Seed_Node(ARRAYS_ND_BASE<int,TV_INT>& colors,const int fill_color,const ARRAYS_ND_BASE<bool,VECTOR<int,3> >& edge_is_blocked_x,const ARRAYS_ND_BASE<bool,VECTOR<int,3> >& edge_is_blocked_y,
    const ARRAYS_ND_BASE<bool,VECTOR<int,3> >& edge_is_blocked_z,bool& touches_uncolorable_node,const TV_INT& seed_node)
{
...
void FLOOD_FILL_3D::
Flood_Fill_Node(ARRAYS_ND_BASE<int,TV_INT>& colors,const int fill_color,const ARRAYS_ND_BASE<bool,VECTOR<int,3> >& edge_is_blocked_x,const ARRAYS_ND_BASE<bool,VECTOR<int,3> >& edge_is_blocked_y,
    const ARRAYS_ND_BASE<bool,VECTOR<int,3> >& edge_is_blocked_z,bool& touches_uncolorable_node,STACK<TV_INT>& flood_fill_stack,const TV_INT& node)
{
    ...
    if(node.y<colors.domain.max_corner.y-1&&!edge_is_blocked_y(node.x,node.y+1,node.z)&&colors(node.x,node.y+1,node.z)<0) flood_fill_stack.Push(TV_INT(node.x,node.y+1,node.z));
    ...
    if(node.z<colors.domain.max_corner.z-1&&!edge_is_blocked_z(node.x,node.y,node.z+1)&&colors(node.x,node.y,node.z+1)<0) flood_fill_stack.Push(TV_INT(node.x,node.y,node.z+1));
}
void FLOOD_FILL_3D::
Identify_Colors_Touching_Boundary(const int number_of_colors,const ARRAYS_ND_BASE<int,TV_INT>& colors,const ARRAYS_ND_BASE<bool,VECTOR<int,3> >& edge_is_blocked_x,
    const ARRAYS_ND_BASE<bool,VECTOR<int,3> >& edge_is_blocked_y,const ARRAYS_ND_BASE<bool,VECTOR<int,3> >& edge_is_blocked_z,ARRAY<bool>& color_touches_boundary)
{
...
void FLOOD_FILL_3D::
Identify_Colors_Touching_Color(const int color,const int number_of_colors,const ARRAYS_ND_BASE<int,TV_INT>& colors,const ARRAYS_ND_BASE<bool,VECTOR<int,3> >& edge_is_blocked_x,
    const ARRAYS_ND_BASE<bool,VECTOR<int,3> >& edge_is_blocked_y,const ARRAYS_ND_BASE<bool,VECTOR<int,3> >& edge_is_blocked_z,ARRAY<bool>& color_touches_color)
{
    ...
    for(int i=colors.domain.min_corner.x ;i<colors.domain.max_corner.x;i++) for(int j=colors.domain.min_corner.y;j<colors.domain.max_corner.y;j++)
        for(int k=colors.domain.min_corner.z;k<colors.domain.max_corner.z;k++) if(colors(i,j,k)==color){
            ...
            if(i<colors.domain.max_corner.x-1&&!edge_is_blocked_x(i+1,j,k)&&colors(i+1,j,k)>=0) color_touches_color(colors(i+1,j,k))=true;
            if(j>colors.domain.min_corner.y&&!edge_is_blocked_y(i,j,k)&&colors(i,j-1,k)>=0) color_touches_color(colors(i,j-1,k))=true;
            if(j<colors.domain.max_corner.y-1&&!edge_is_blocked_y(i,j+1,k)&&colors(i,j+1,k)>=0) color_touches_color(colors(i,j+1,k))=true;
            if(k>colors.domain.min_corner.z&&!edge_is_blocked_z(i,j,k)&&colors(i,j,k-1)>=0) color_touches_color(colors(i,j,k-1))=true;
            if(k<colors.domain.max_corner.z-1&&!edge_is_blocked_z(i,j,k+1)&&colors(i,j,k+1)>=0) color_touches_color(colors(i,j,k+1))=true;}
}
bool FLOOD_FILL_3D::
Path_Between_Nodes(const RANGE<TV_INT>& domain,const TV_INT& start_node,const TV_INT& end_node,
    const ARRAYS_ND_BASE<bool,VECTOR<int,3> >& edge_is_blocked_x,const ARRAYS_ND_BASE<bool,VECTOR<int,3> >& edge_is_blocked_y,const ARRAYS_ND_BASE<bool,VECTOR<int,3> >& edge_is_blocked_z,ARRAY<TV_INT>* path)
{
