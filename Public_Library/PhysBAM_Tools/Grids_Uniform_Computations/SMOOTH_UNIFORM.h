...
void Smooth(T_ARRAYS_T2& d,const int steps,const typename GRID_ARRAYS_POLICY<T_GRID>::ARRAYS_SCALAR* phi){
    ...
    for(int step=0;step<steps;step++){
        ...
        for(int axis=0;axis<T_GRID::dimension;axis++)for(int axis_side=0;axis_side<2;axis_side++){
            int side=2*(axis-1)+axis_side;TV_INT ghost_offset=(axis_side==0?-1:1)*TV_INT::Axis_Vector(axis);
        ...
        if(!phi) for(NODE_ITERATOR iterator(grid,number_of_ghost_cells);iterator.Valid();iterator.Next()){TV_INT node=iterator.Node_Index();
            ...
            for(int n=0;n<T_GRID::number_of_neighbors_per_node;n++)sum+=d_ghost(T_GRID::Node_Neighbor(node,n));
