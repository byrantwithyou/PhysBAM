...
//#####################################################################
// Namespace MATLAB_OUTPUT_NEW 
//#####################################################################
#ifndef __MATLAB_OUTPUT_NEW__
#define __MATLAB_OUTPUT_NEW__
...
template<class T>
struct MATLAB_SPARSE_MATRIX_ENTRY
{
    int r,c;
    T x;
    bool operator<(const MATLAB_SPARSE_MATRIX_ENTRY<T>& o) const;
};

template<class T>
class MATLAB_OUTPUT_NEW{
public:
    ...
    std::ofstream out;
private:
    ARRAY<MATLAB_SPARSE_MATRIX_ENTRY<T> > internal;
