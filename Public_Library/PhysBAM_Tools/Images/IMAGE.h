...
public:
    ...
    {for(int i=1;i<=image.m/2;i++)for(int j=0;j<image.n;j++)exchange(image(i,j),image(image.m-i+1,j));}

    template<int d> static void Flip_Y(ARRAY<VECTOR<T,d> ,VECTOR<int,2> >& image)
    {for(int j=1;j<=image.n/2;j++)for(int i=0;i<image.m;i++)exchange(image(i,j),image(i,image.n-j+1));}
