...
Write(const std::string& filename,const ARRAY<VECTOR<T,d> ,VECTOR<int,2> >& image)
{
    ...
    else if (d==4) for(int i=0;i<image.counts.x;i++) for(int j=image.counts.y-1;j>=0;j--){pixels[t].r=(float)image(i,j)[0];pixels[t].g=(float)image(i,j)[1];pixels[t].b=(float)image(i,j)[2];pixels[t].a=(float)image(i,j)[3];t++;}
