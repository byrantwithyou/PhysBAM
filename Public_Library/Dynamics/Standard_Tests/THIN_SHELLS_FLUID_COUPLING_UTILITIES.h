...
//#####################################################################
    ...
    static void Add_Rigid_Body_Walls(SOLIDS_FLUIDS_EXAMPLE_UNIFORM<VECTOR<T,2> >& example,const T coefficient_of_restitution=(T).5,const T coefficient_of_friction=(T).5,
        ARRAY<int>* walls_added=0);
    static void Add_Rigid_Body_Walls(SOLIDS_FLUIDS_EXAMPLE_UNIFORM<VECTOR<T,3> >& example,const T coefficient_of_restitution=(T).5,const T coefficient_of_friction=(T).5,
        ARRAY<int>* walls_added=0);
    ...
    template<class TV> static void Set_Parameters_From_Parameter_List(SOLIDS_FLUIDS_EXAMPLE_UNIFORM<TV>& example,PARAMETER_LIST& parameter_list);
//#####################################################################
};
}
#endif
