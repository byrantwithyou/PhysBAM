...
void Initialize(const int test_number_input,const int resolution,const int restart_frame)
{
    ...
    if(!example.user_output_directory)
        example.viewer_dir.output_directory=LOG::sprintf("Standard_Tests_Multiphase/Test_%d__Resolution_%d_%d_%d",test_number,(grid.counts.x-1),(grid.counts.y-1),(grid.counts.z-1));
...
void Initialize_Bodies()
{
    ...
    if(test_number==15){
        int ground=rigid_body_collection.Add_Rigid_Body(example.data_directory+"/Rigid_Bodies/ground",(T).01,true,true,false);
