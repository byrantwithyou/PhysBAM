...
Initialize(const int test_number_input,const int resolution)
{
    ...
    if(!example.user_output_directory)
        example.viewer_dir.output_directory=LOG::sprintf("Standard_Tests_Smoke/Test_%d__Resolution_%d_%d_%d",test_number,(grid.counts.x-1),(grid.counts.y-1),(grid.counts.z-1));
...
{
    if(test_number==4){
        ARRAY<T,FACE_INDEX<3> > face_velocities_ghost(*fluids_parameters.grid,fluids_parameters.number_of_ghost_cells,no_init);
