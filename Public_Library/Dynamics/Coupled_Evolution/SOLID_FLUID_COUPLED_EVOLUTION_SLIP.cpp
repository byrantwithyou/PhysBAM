...
{
    disable_thinshell=false; // TODO: incompressible can handle thinshell, so revert this later.
...
{
    Solve(fluid_collection.incompressible_fluid_collection.face_velocities,dt,current_velocity_time,current_position_time,velocity_update,false);
        ...
        for(COLLISION_GEOMETRY_ID p(0);p<fluids_parameters.collision_bodies_affecting_fluid->collision_geometry_collection.Size();++p){
            ...
            if(RIGID_COLLISION_GEOMETRY<TV>* rigid_body_wrapper=dynamic_cast<RIGID_COLLISION_GEOMETRY<TV>*>(fluids_parameters.collision_bodies_affecting_fluid->collision_geometry_collection.bodies(p))){
                ...
                if(pressure_impulses_twist.Valid_Index(rigid_particle_index) && pressure_impulses_twist(rigid_particle_index).linear.Magnitude() > body.fracture_threshold){ // Valid_Index because we might be trying to fracture objects added in this substep.
                    LOG::cout<<"Maximum magnitude threshold exceeded: "<<pressure_impulses_twist(rigid_particle_index).linear.Magnitude()<<std::endl;
...
//#####################################################################
template<class TV> void SOLID_FLUID_COUPLED_EVOLUTION_SLIP<TV>::
Apply_Second_Order_Cut_Cell_Method(const T_ARRAYS_INT& cell_index_to_divergence_matrix_index,const T_FACE_ARRAYS_INT& face_index_to_matrix_index,ARRAY<T>& b)
{
/*
    ...
    for(CELL_ITERATOR<TV> iterator(grid);iterator.Valid();iterator.Next()){TV_INT cell_index=iterator.Cell_Index();
        ...
        for(int axis=0;axis<TV::dimension;axis++){
            ...
            if((grid.Inside_Domain(first_cell_index) && !psi_D(first_cell_index) && !psi_N.Component(axis)(first_face_index)) || 
                ...
                LOG::cout<<"Warning: Exposed dirichlet cell "<<cell_index<<std::endl;}}}
}
//#####################################################################
