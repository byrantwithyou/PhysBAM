...
PLS_FC_EXAMPLE(const STREAM_TYPE stream_type_input)
    ...
    time_steps_per_frame(1),use_preconditioner(true),max_iter(100000),dump_matrix(false),sparse_dump_matrix(false),
    wrap(true),use_advection(true),use_reduced_advection(true),omit_solve(false),number_of_colors(1),
    use_discontinuous_velocity(false),use_p_null_mode(false),use_level_set_method(false),use_pls(false),
    dump_largest_eigenvector(false),save_pressure(false),use_polymer_stress(false),num_multigrid_levels(2),
    use_multigrid(false),grid(TV_INT(),RANGE<TV>::Unit_Box(),true),
