...
OPENGL_COMPONENT_RIGID_BODY_COLLECTION_3D(RIGID_BODY_COLLECTION<TV>& rigid_body_collection,const std::string& basedir_input,bool use_display_lists)
    ...
    rigid_body_collection(rigid_body_collection),articulated_rigid_body(0),current_selection(0),need_destroy_rigid_body_collection(false)
{
    rigid_geometry_collection=&rigid_body_collection.rigid_geometry_collection;
    Initialize();
}
...
Display(const int in_color) const
{
    ...
    if(draw_joint_frames==0) for(int i=0;i<joint_frames.m;i++)(OPENGL_AXES<T>(joint_frames(i),axes_box)).Display();
    else if(draw_joint_frames==1) for(int i=1;i<joint_frames.m;i+=2)(OPENGL_AXES<T>(joint_frames(i),axes_box)).Display();
    else if(draw_joint_frames==2) for(int i=0;i<joint_frames.m;i+=2)(OPENGL_AXES<T>(joint_frames(i),axes_box)).Display();
