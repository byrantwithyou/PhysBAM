...
{
    for(typename GRID<TV>::CELL_ITERATOR iterator(grid);iterator.Valid();iterator.Next()) opengl_refinement_mac_velocity_fields(iterator.Cell_Index())->Scale_Vector_Size(1.1);
}

template<class T,class RW> void OPENGL_COMPONENT_REFINEMENT_MAC_VELOCITY_FIELD_2D<T,RW>::
Decrease_Vector_Size()
{
    for(typename GRID<TV>::CELL_ITERATOR iterator(grid);iterator.Valid();iterator.Next()) opengl_refinement_mac_velocity_fields(iterator.Cell_Index())->Scale_Vector_Size(1/1.1);
}
