...
private:
    ...
    {TV current;if(highlighted) current=qNow;else current=qDown;
     ...
     OpenGL_Line(qDown+ray.endpoint,current+ray.Point(ray.t_max)+ray.direction*scale*ray.t_max,vertices);
     OpenGL_Draw_Arrays(GL_LINES,3,vertices);
     TV center=current+ray.Point(ray.t_max)+ray.direction*scale*ray.t_max;
     T distance=(ray.Point(ray.t_max)-ray.Point(FACTOR*ray.t_max)).Magnitude()*scale;
