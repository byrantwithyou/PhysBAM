...
template<class T,class T2> 
void Display_Helper(const OPENGL_BINTREE_FACE_SCALAR_FIELD<T,T2>& self,const int in_color,const T2& value,const VECTOR<T,1>& location,ARRAY<typename OPENGL_POLICY<T>::T_GL>& vertices,ARRAY<GLfloat>& colors) {
    OpenGL_Color(self.color_map->Lookup(value).rgba,colors);OpenGL_Vertex(location.Insert((T)value,2),vertices);
}

void Display_Helper(const OPENGL_BINTREE_FACE_SCALAR_FIELD<float,bool>& self,const int in_color,const bool& value,const VECTOR<float,1>& location,ARRAY<GLfloat>& vertices,ARRAY<GLfloat>& colors) {
    if(value){OpenGL_Color(self.color_map->Lookup(value).rgba,colors);OpenGL_Vertex(location,vertices);}
}

void Display_Helper(const OPENGL_BINTREE_FACE_SCALAR_FIELD<double,bool>& self,const int in_color,const bool& value,const VECTOR<double,1>& location,ARRAY<OPENGL_POLICY<double>::T_GL>& vertices,ARRAY<GLfloat>& colors) {
    if(value){OpenGL_Color(self.color_map->Lookup(value).rgba,colors);OpenGL_Vertex(location,vertices);}
}
};
...
Display(const int in_color) const
{
    ...
    for(DYADIC_GRID_ITERATOR_FACE<BINTREE_GRID<T> > iterator(grid,grid.Map_All_Faces());iterator.Valid();iterator.Next()){
        Display_Helper(*this,in_color,value(iterator.Face_Index()),iterator.Location(),vertices,colors);}
    if(draw_points) OpenGL_Draw_Arrays(GL_POINTS,2,vertices,colors); else OpenGL_Draw_Arrays(GL_LINE_STRIP,2,vertices,colors);
    glPopAttrib();
}
//#####################################################################
// Bounding_Box
