...
public:
    ...
    void Solve(SPARSE_MATRIX_FLAT_MXN<T>& A_matrix,ARRAY<T>& x,ARRAY<T>& b,ARRAY<KRYLOV_VECTOR_BASE<T>*>& vectors,const T tolerance=1e-7,const bool recompute_preconditioner=true)
    {
        PHYSBAM_FATAL_ERROR();
    }
...
//#####################################################################
    ...
    void Solve_In_Parts(DOMAIN_ITERATOR_THREADED_ALPHA<PCG_SPARSE_THREADED<TV>,TV>& threaded_iterator,const ARRAY<int,TV_INT>& domain_index,const ARRAY<INTERVAL<int> >& all_interior_indices,const ARRAY<ARRAY<INTERVAL<int> > >& all_ghost_indices,SPARSE_MATRIX_FLAT_MXN<T>& A,ARRAY<T>& x,ARRAY<T>& b,const T tolerance);
    ...
    void Solve_Part_Two(RANGE<TV_INT>& domain,const ARRAY<int,TV_INT>& domain_index,const ARRAY<INTERVAL<int> >& all_interior_indices,const ARRAY<ARRAY<INTERVAL<int> > >& all_ghost_indices,SPARSE_MATRIX_FLAT_MXN<T>& A,ARRAY<T>& x,ARRAY<ARRAY_VIEW<T> >& b_interior,ARRAY<ARRAY_VIEW<T> >& temp_interior);
    void Solve_Part_Three(RANGE<TV_INT>& domain,const ARRAY<int,TV_INT>& domain_index,const ARRAY<INTERVAL<int> >& all_interior_indices,SPARSE_MATRIX_FLAT_MXN<T>& A,ARRAY<SPARSE_MATRIX_FLAT_MXN<T>*>& C);
    void Solve_Part_Four(RANGE<TV_INT>& domain,const ARRAY<int,TV_INT>& domain_index,const ARRAY<INTERVAL<int> >& all_interior_indices,ARRAY<ARRAY_VIEW<T> >& z_interior,ARRAY<ARRAY_VIEW<T> >& b_interior,ARRAY<ARRAY_VIEW<T> >& temp_interior,ARRAY<SPARSE_MATRIX_FLAT_MXN<T>*>& C);
    ...
    void Solve_Part_Six(RANGE<TV_INT>& domain,const ARRAY<int,TV_INT>& domain_index,const ARRAY<INTERVAL<int> >& all_interior_indices,const ARRAY<ARRAY<INTERVAL<int> > >& all_ghost_indices,const SPARSE_MATRIX_FLAT_MXN<T>& A);
    ...
    void Solve_In_Parts(SPARSE_MATRIX_FLAT_MXN<T>& A,ARRAY<T>& x,ARRAY<T>& b,const T tolerance);
    ...
    void Threaded_Part_One(SPARSE_MATRIX_FLAT_MXN<T>& A,ARRAY<T>& x,ARRAY<T>& b,const T sum,int start_index,int end_index);
    void Threaded_Part_Two(ARRAY<T>& z,T rho,T rho_old,int iteration,int start_index,int end_index);
    void Threaded_Part_Three(SPARSE_MATRIX_FLAT_MXN<T>& A,int start_index,int end_index);
