...
#include <Rigids/Rigid_Bodies/RIGID_BODY.h> // TODO: remove once MUSCLE.cpp exists (windows workaround)
...
{
    for(int i=0;i<joint_mesh.Num_Joints();i++) if(joint_mesh.Joints(i)->joint_function) joint_mesh.Joints(i)->joint_function->Compute_Desired_PD_Velocity(dt,time);
    if(use_muscle_actuators){
        ...
        muscles_crossing_joints.Resize(joint_mesh.Num_Joints());
...
Solve_Velocities_for_PD(const T time,const T dt,bool test_system,bool print_matrix)
{
    ...
    for(int iteration=0;iteration<actuation_iterations;iteration++) for(int k=0;k<poststabilization_iterations;k++)
        for(int i=0;i<joint_mesh.Num_Joints();i++) Post_Stabilization_With_Actuation(joint_mesh.Joints(i)->id_number);
}
//####################################################################################
