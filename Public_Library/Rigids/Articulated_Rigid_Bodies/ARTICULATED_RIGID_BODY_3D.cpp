...
Solve_For_Muscle_Control(MATRIX_MXN<T>& A,const ARRAY<T>& b,ARRAY<T>& x,const T dt)
{
    ...
    if(enforce_nonnegative_activations){
        ...
        if(verbose){
            LOG::cout<<"*** MUSCLE SYSTEM AFTER TRANSFORM: "<<std::endl;
        ...
        if(verbose){
            ...
            else LOG::cout<<"===> LP result x:"<<std::endl<<x_lp<<std::endl;
            LOG::cout<<"===> QP result x:"<<std::endl<<x<<std::endl;}
        LOG::cout<<"===> LP: muscle activation sum is "<<(D*x_lp).Magnitude()<<std::endl;
        LOG::cout<<"===> QP: muscle activation sum is "<<(D*x).Magnitude()<<std::endl;}
    else{
        ...
        if(!getenv("OLD_LS")){
            ...
            if(debug){
                ...
                LOG::cout<<b<<std::endl;
                LOG::cout<<"*** MUSCLE SYSTEM AFTER TRANSFORM: "<<std::endl;
                LOG::cout<<transformed_A<<std::endl;
                LOG::cout<<transformed_b<<std::endl;
                LOG::cout<<permute<<std::endl;}
            ...
            if(debug){
                ...
                LOG::cout<<"==> U "<<std::endl<<U<<std::endl;
                LOG::cout<<"==> G "<<std::endl<<G<<std::endl;
                LOG::cout<<"==> b1 "<<std::endl<<b1<<std::endl;
                LOG::cout<<"==> b2 "<<std::endl<<b2<<std::endl;
                LOG::cout<<"==> Z "<<std::endl<<Z<<std::endl;
                LOG::cout<<"==> Z_rhs "<<std::endl<<Z_rhs<<std::endl;
                LOG::cout<<"OLD METHOD X:"<<std::endl<<old_method_x<<std::endl;
                LOG::cout<<"RESIDUAL: "<<(A*old_method_x-b).Magnitude()<<std::endl;
...
Solve_Minimum_Norm_Solution_For_Linear_Constraints(MATRIX_MXN<T>& A,const ARRAY<T>& b,ARRAY<T>& x,const T zero_row_tolerance,const bool verbose)
{
    assert(A.m==A.n);
    if(verbose){
        LOG::cout<<"*** SYSTEM BEFORE TRANSFORM: "<<std::endl;
        LOG::cout<<A<<std::endl<<b<<std::endl;}
    ...
    if(verbose){
        LOG::cout<<"*** SYSTEM AFTER TRANSFORM: "<<std::endl;
        LOG::cout<<A_after_QR<<std::endl<<b_after_QR<<std::endl<<permute<<std::endl;}
...
Output_Articulation_Points(const STREAM_TYPE stream_type,const std::string& output_directory,const int frame) const
{
    ...
    for(int i=0;i<joint_mesh.Num_Joints();i++){
        ...
        Write_Binary(output,ap1,parent.Frame()*joint.F_pj(),ap2,child.Frame()*joint.F_pj());}
}
//####################################################################################
namespace PhysBAM{
