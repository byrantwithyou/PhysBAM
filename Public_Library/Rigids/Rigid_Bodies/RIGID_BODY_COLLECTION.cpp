...
Add_Velocity_Independent_Forces(ARRAY_VIEW<TWIST<TV> > rigid_F_full,const T time) const
{
    for(int k=0;k<rigids_forces.m;k++)
        rigids_forces(k)->Add_Velocity_Independent_Forces(rigid_F_full,time);
}
//#####################################################################
...
{
    for(int k=0;k<rigids_forces.m;k++)
        rigids_forces(k)->Add_Velocity_Dependent_Forces(rigid_V_full,rigid_F_full,time);
}
//#####################################################################
...
{
    std::string hashname=LOG::sprintf("%s@%.6f",filename.c_str(),scaling_factor); // mangle hash name
...
Read(const STREAM_TYPE stream_type,const std::string& directory,const int frame,ARRAY<int>* needs_init,ARRAY<int>* needs_destroy)
{
    ...
    FILE_UTILITIES::Read_From_File(stream_type,LOG::sprintf("%s/%d/rigid_body_particles",directory.c_str(),frame),rigid_body_particles);
...
Write(const STREAM_TYPE stream_type,const std::string& directory,const int frame) const
{
    ...
    FILE_UTILITIES::Write_To_File(stream_type,LOG::sprintf("%s/%d/rigid_body_particles",directory.c_str(),frame),rigid_body_particles);
