...
struct GRADIENT_MAT
{
    ...
    GRADIENT_MAT<T_MAT,decltype(VEC_NEG::Type(VEC()))> operator- () const {GRADIENT_MAT<T_MAT,decltype(VEC_NEG::Type(VEC()))> r;VEC_NEG()(r.x,x);return r;}
    GRADIENT_MAT<T_MAT,decltype(VEC_SCALE::Type(VEC(),T()))> operator* (T a) const {GRADIENT_MAT<T_MAT,decltype(VEC_SCALE::Type(VEC(),T()))> r;VEC_SCALE()(r.x,x,a);return r;}
    GRADIENT_MAT<T_MAT,decltype(VEC_SCALE_DIV::Type(VEC(),T()))> operator/ (T a) const {GRADIENT_MAT<T_MAT,decltype(VEC_SCALE_DIV::Type(VEC(),T()))> r;VEC_SCALE_DIV()(r.x,x,a);return r;}

    template<class VEC1,class T_MAT2>
    GRADIENT_MAT<T_MAT,decltype(VEC_ADD::Type(VEC(),VEC1()))> operator+ (const GRADIENT_MAT<T_MAT2,VEC1>& z) const {GRADIENT_MAT<T_MAT,decltype(VEC_ADD::Type(VEC(),VEC1()))> r;VEC_ADD()(r.x,x,z.x);return r;}

    template<class VEC1,class T_MAT2>
    GRADIENT_MAT<T_MAT,decltype(VEC_SUB::Type(VEC(),VEC1()))> operator- (const GRADIENT_MAT<T_MAT2,VEC1>& z) const {GRADIENT_MAT<T_MAT,decltype(VEC_SUB::Type(VEC(),VEC1()))> r;VEC_SUB()(r.x,x,z.x);return r;}
};
...
template<class T_MAT,class VEC>
auto operator* (typename T_MAT::SCALAR a,const GRADIENT_MAT<T_MAT,VEC>& u) -> decltype(u*a) {return u*a;}

template<class TV,class T,int d,class VEC> auto
Tensor_Product_1(const GRADIENT_VEC<TV,VEC>& v,const VECTOR<T,d>& u)
    -> GRADIENT_MAT<MATRIX<T,TV::m,d>,decltype(TENSOR_PRODUCT_1_VM_V::Type(VEC(),u))>
...
Tensor_Product_0(const GRADIENT_VEC<TV,VEC>& v,const VECTOR<T,d>& u)
    -> GRADIENT_MAT<MATRIX<T,TV::m,d>,decltype(TENSOR_PRODUCT_0_VM_V::Type(VEC(),u))>
...
Twice_Symmetric_Part_01(const GRADIENT_MAT<T_MAT,MAT>& v)
    -> GRADIENT_MAT<T_MAT,decltype(TWICE_SYMMETRIC_PART_01_VT::Type(v.x))>
...
Contract_01(const GRADIENT_MAT<T_MAT,MAT>& v)
    -> GRADIENT<typename T_MAT::SCALAR,decltype(CONTRACT_01_VT::Type(v.x))>
...
Transposed_01(const GRADIENT_MAT<T_MAT,MAT>& v)
    -> GRADIENT_MAT<decltype(T_MAT().Transposed()),decltype(TRANSPOSED_01_VT::Type(v.x))>
{GRADIENT_MAT<decltype(T_MAT().Transposed()),decltype(TRANSPOSED_01_VT::Type(v.x))> r;TWICE_SYMMETRIC_PART_01_VT()(r.x,v.x);return r;}

template<class T_MAT,class MAT,class T_MAT2> auto
Contract_00(const GRADIENT_MAT<T_MAT,MAT>& v,const T_MAT2& m)
    -> GRADIENT_MAT<decltype(Transpose_Times(T_MAT(),T_MAT2())),decltype(CONTRACT_00_VT_M::Type(v.x,m))>
...
Contract_10(const GRADIENT_MAT<T_MAT,MAT>& v,const T_MAT2& m)
    -> GRADIENT_MAT<decltype(Transpose_Times(T_MAT(),T_MAT2())),decltype(CONTRACT_10_VT_M::Type(v.x,m))>
...
Contract_01(const GRADIENT_MAT<T_MAT,MAT>& v,const T_MAT2& m)
    -> GRADIENT_MAT<decltype(Transpose_Times(T_MAT(),T_MAT2())),decltype(CONTRACT_01_VT_M::Type(v.x,m))>
...
Contract_11(const GRADIENT_MAT<T_MAT,MAT>& v,const T_MAT2& m)
    -> GRADIENT_MAT<decltype(Transpose_Times(T_MAT(),T_MAT2())),decltype(CONTRACT_11_VT_M::Type(v.x,m))>
...
Double_Contract_00_11(const GRADIENT_MAT<T_MAT,MAT>& v,const T_MAT2& m)
    -> GRADIENT<typename T_MAT::SCALAR,decltype(DOUBLE_CONTRACT_00_11_VT_M::Type(v.x,m))>
