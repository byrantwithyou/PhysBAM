...
DATATYPE_HELPER<PAIR<VECTOR<int,2>,VECTOR<T,2> > >::Datatype()
{
    static MPI::Datatype datatype=MPI::DATATYPE_NULL;
    if(datatype==MPI::DATATYPE_NULL){
        static PAIR<VECTOR<int,2>,VECTOR<T,2> > zero_entry;
        static int lengths[3]={2,2,1};
...
template<class T,int d> MPI::Datatype Scalar_Block_Datatype()
{
    ...
    static MPI::Datatype datatype=MPI::DATATYPE_NULL;
    if(datatype==MPI::DATATYPE_NULL){
        datatype=MPI_UTILITIES::Datatype<T>().Create_contiguous(d);
        datatype.Commit();}
    return datatype;
}
//#####################################################################
...
int Pack_Size(const SPARSE_MATRIX_PARTITION& data,const MPI::Comm& comm)
{
    ...
    for(int s=0;s<data.number_of_sides;s++)
        size+=Pack_Size(data.boundary_indices(s),comm);
    return size;
}
...
{
    Unpack(data.number_of_sides,data.interior_indices,data.ghost_indices,data.neighbor_ranks,buffer,position,comm);
    data.neighbors.Resize(data.number_of_sides);
    data.boundary_indices.Resize(data.number_of_sides);
    for(int s=0;s<data.number_of_sides;s++) Unpack(data.boundary_indices(s),buffer,position,comm);
}
//#####################################################################
...
{
    int size=Pack_Size<int>(comm);
    for(int i=0;i<data.m;i++) size+=Pack_Size(data(i),comm);
    return size;
}
