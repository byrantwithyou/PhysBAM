...
constexpr inline int integer_log(unsigned int v) // this works for any v, but it is slower
{int c=0;
...
constexpr inline bool power_of_two(const unsigned int v)
{return v>0 && (v&(v-1))==0;}

constexpr inline unsigned int next_power_of_two(unsigned int v)
{STATIC_ASSERT(sizeof(v)==4);v--;
v|=v>>1;v|=v>>2;v|=v>>4;v|=v>>8;v|=v>>16;return v+1;}

constexpr inline int rightmost_bit(unsigned int v)
{return v&(unsigned int)-(int)v;} // TODO: VS workaround

constexpr inline int integer_log_exact(const unsigned int v) // this only works if v is a power of 2
