    ...
    ARRAY<VECTOR<T,3> >& weights,ARRAY<VECTOR<T,3> >& particle_distances)
{
    ...
    for(int i=0;i<tetrahedron_candidates.m;i++){TETRAHEDRON_COLLISION_BODY<T>& collision_body=*tetrahedron_candidates(i);
        ...
        TV distance=-particles.X(particle_index);for(int j=0;j<TV::m;j++) distance+=particles.X(element(j))*surface_weights(j);
...
Update_Rigid_Deformable_Collision_Pair(RIGID_BODY<TV>& rigid_body,const int particle_index,const T dt,const T time,ARRAY<TV>& X_save,ARRAY<TV>& V_save,
    ARRAY<FRAME<TV> >& rigid_frame_save,ARRAY<typename TV::SPIN>& rigid_angular_momentum_difference,ARRAY<TV>& rigid_velocity_difference)
{
    DEFORMABLE_BODY_COLLECTION<TV>& deformable_body_collection=solid_body_collection.deformable_body_collection;
    DEFORMABLE_PARTICLES<TV>& particles=deformable_body_collection.particles;
    ...
    for(int i=0;i<solids_parameters.rigid_body_collision_parameters.collision_iterations && need_another_iteration;i++){need_another_iteration=false;
        ...
        if(!kinematic_rigid_bodies_only && solids_parameters.rigid_body_collision_parameters.perform_collisions){
            ...
            rigid_body_collisions.Get_Bounding_Box_Collision_Pairs(dt,time,pairs,i==solids_parameters.rigid_body_collision_parameters.collision_iterations-1,
                i==0,solids_parameters.rigid_body_collision_parameters.collision_bounding_box_thickness);
...
//#####################################################################
template<class TV> bool RIGID_DEFORMABLE_COLLISIONS<TV>::
Process_Deformable_Contact_With_Kinematic_Rigid_Body(RIGID_BODY<TV>& rigid_body,const T dt,const T time,ARRAY<FRAME<TV> >& rigid_frame_save,ARRAY<TV>& X_save)
{
    ...
    Apply_Rigid_Deformable_Contact_Projection(X_save,V,rigid_body.Twist(),precompute);
...
//#####################################################################
template<class TV> bool RIGID_DEFORMABLE_COLLISIONS<TV>::
Update_Rigid_Deformable_Contact_Pair(RIGID_BODY<TV>& rigid_body,const int particle_index,const T dt,const T time,const T epsilon_scale,ARRAY<TV>& X_save,
    ARRAY<FRAME<TV> >& rigid_frame_save,const T collision_body_thickness,const bool process_contact_unconditionally)
{
    DEFORMABLE_BODY_COLLECTION<TV>& deformable_body_collection=solid_body_collection.deformable_body_collection;
    DEFORMABLE_PARTICLES<TV>& particles=deformable_body_collection.particles;
                ...
                if(solids_parameters.deformable_object_collision_parameters.perform_collision_body_collisions && do_dr)
                    for(int i=0;i<rigid_bodies_in_level.m;i++){RIGID_BODY<TV>& rigid_body=solid_body_collection.rigid_body_collection.Rigid_Body(rigid_bodies_in_level(i));
                        ...
                        for(int k=0;k<particles.m;k++)
                            if(Update_Rigid_Deformable_Contact_Pair(rigid_body,particles(k),dt,time,epsilon_scale,X_save,rigid_frame_save,collision_body_thickness))
                                need_another_level_iteration=need_another_iteration=true;}}}
    ...
    if(do_dr) if(solids_parameters.deformable_object_collision_parameters.perform_collision_body_collisions)
        for(int i=0;i<solid_body_collection.rigid_body_collection.static_and_kinematic_rigid_bodies.m;i++){
            ...
            if(use_saved_pairs){
                ...
                for(int k=0;k<particles.m;k++){
                    Update_Rigid_Deformable_Contact_Pair(rigid_body,particles(k),dt,time,1,X_save,rigid_frame_save,collision_body_thickness);}}}
...
//#####################################################################
template<class TV> void RIGID_DEFORMABLE_COLLISIONS<TV>::
Set_Collision_Velocities(ARRAY_VIEW<TV> V,ARRAY_VIEW<TWIST<TV> > twist,ARRAY<TV>& X_save,ARRAY<FRAME<TV> >& rigid_frame_save,
    ARRAY<TWIST<TV> >& rigid_velocity_save,ARRAY<typename TV::SPIN>& rigid_angular_momentum_save,ARRAY<TV>& V_save)
{
    ...
    for(int body=0;body<precompute_contact_projections.m;body++){
        ...
        for(int i=0;i<precompute.particles.m;i++){const int p=precompute.particles(i);
            ...
            if(solids_parameters.use_trapezoidal_rule_for_velocities){
                ...
                V_rel_target=(T).5*(RIGID_BODY<TV>::Pointwise_Object_Velocity(twist_n,rigid_frame(rigid_p).t,X(p))-V_save(p));}
    ...
    for(int i=0;i<solids_parameters.rigid_body_collision_parameters.contact_project_iterations;i++) for(int body=0;body<precompute_boundary_list.m;body++)
        /*if(1 || i==0 || !precompute_boundary_list(body)->rigid_body.Has_Infinite_Inertia())*/{ // only process static/kinematic bodies once
...
Project_Contact_Pairs(ARRAY_VIEW<TV> V,ARRAY_VIEW<TWIST<TV> > twist)
{
    ...
    for(int k=0;k<iterations;k++) for(int i=0;i<precompute_contact_projections.m;i++) if(k==0 || !precompute_contact_projections(i)->rigid_body.Has_Infinite_Inertia()){
    ...
    for(int k=0;k<iterations;k++) for(int i=precompute_contact_projections.m-1;i>=0;i--) if(k==iterations-1 || !precompute_contact_projections(i)->rigid_body.Has_Infinite_Inertia()){
        PRECOMPUTE_CONTACT_PROJECTION& precompute=*precompute_contact_projections(i);
        Apply_Rigid_Deformable_Contact_Projection(deformable_body_collection.particles.X,V,twist(precompute.rigid_body.particle_index),precompute);}
}
//#####################################################################
...
Initialize_Rigid_Deformable_Contact_Projection(PRECOMPUTE_CONTACT_PROJECTION& precompute,ARRAY_VIEW<const TV> X)
{
    ...
    MATRIX<T,TV::m> L;MATRIX<T,T_SPIN::m,TV::m> rL;MATRIX<T,T_SPIN::m> rLr;
    for(int i=0;i<precompute.particles.m;i++){const int p=precompute.particles(i);
        precompute.N(i)=precompute.rigid_body.implicit_object->Extended_Normal(X(p));
        const T one_over_NT_K_N=deformable_body_collection.particles.mass(p);precompute.r(i)=X(p)-precompute.rigid_body.Frame().t;precompute.N_over_NT_K_N(i)=precompute.N(i)*one_over_NT_K_N;
        if(has_infinite_inertia) continue;
        precompute.rN(i)=TV::Cross_Product(precompute.r(i),precompute.N(i));
        L+=MATRIX<T,TV::m>::Outer_Product(precompute.N(i),precompute.N_over_NT_K_N(i));
        rL+=MATRIX<T,T_SPIN::m,TV::m>::Outer_Product(precompute.rN(i),precompute.N_over_NT_K_N(i));
        rLr+=MATRIX<T,T_SPIN::m>::Outer_Product(precompute.rN(i),precompute.rN(i)*one_over_NT_K_N);}
    ...
    if(!has_infinite_inertia){
        ...
        precompute.A.Set_Submatrix(TV::m,0,rL);
        precompute.A.Set_Submatrix(0,TV::m,rL.Transposed());
        precompute.A.Set_Submatrix(TV::m,TV::m,rLr+inertia);
...
//#####################################################################
template<class TV> bool RIGID_DEFORMABLE_COLLISIONS<TV>::
Apply_Rigid_Deformable_Contact_Projection(ARRAY_VIEW<const TV> X,ARRAY_VIEW<TV> V,TWIST<TV>& twist,PRECOMPUTE_CONTACT_PROJECTION& precompute)
{
    DEFORMABLE_BODY_COLLECTION<TV>& deformable_body_collection=solid_body_collection.deformable_body_collection;
    ...
    for(int i=0;i<precompute.particles.m;i++){const int p=precompute.particles(i);
        const TV V_rel=twist.linear+TV::Cross_Product(twist.angular,X(p)-precompute.rigid_body.Frame().t)-V(p);
    ...
    if(!has_infinite_inertia){
        VECTOR<T,TV::m+T_SPIN::m> b(Lv,rLv);
        VECTOR<T,TV::m+T_SPIN::m> m_inverse_j_full;
...
Push_Out_From_Rigid_Body(RIGID_BODY<TV>& rigid_body,ARRAY<RIGID_BODY_PARTICLE_INTERSECTION<TV> >& particle_intersections,const T move_fraction)
{
    ...
    if(!parent_rigid_body.Has_Infinite_Inertia()){
        ...
        for(int i=0;i<particle_interactions.m;i++){int p=particle_interactions(i);
            T mass=particles.mass(p);TV radius=particles.X(p)-parent_rigid_body.Frame().t;
            m+=mass;ms[0]+=mass*particle_distances(i);mrs[0]+=mass*TV::Cross_Product(radius,particle_distances(i));
            MATRIX<T,T_SPIN::m,TV::m> ri=MATRIX<T,T_SPIN::m,TV::m>::Cross_Product_Matrix(radius);
        ...
        for(int i=0;i<rigid_body_interactions.m;i++){
            ...
            if(index){centroid+=rigid_body_collision_locations(i)-parent_rigid_body.Frame().t;number_of_static_bodies++;}
        ...
        if(equation_type==0){A=M[0];b=VECTOR<T,TV::m+T_SPIN::m>(ms[0],mrs[0]);}
            ...
            MATRIX<T,T_SPIN::m,TV::m+T_SPIN::m> Z;MATRIX<T,TV::m,TV::m+T_SPIN::m> Y;
    ...
    for(int i=0;i<particle_interactions.m;i++){int p=particle_interactions(i);
        Apply_Displacement_To_Particle(p,-particle_distances(i)+velocity+TV::Cross_Product(angular_velocity,particles.X(p)-parent_rigid_body.Frame().t));}
...
Push_Out_From_Particle(const int particle)
{
    ...
    for(int i=0;i<bodies.m;i++) if(!rigid_body_collection.Rigid_Body(bodies(i)).Has_Infinite_Inertia()){
        ...
        T_SPIN angular_velocity=parent_rigid_body.World_Space_Inertia_Tensor_Inverse()*TV::Cross_Product(particle_location-parent_rigid_body.Frame().t,impulse);
        rigid_body.Frame().t+=impulse/parent_rigid_body.Mass();
        rigid_body.Frame().r=ROTATION<TV>::From_Rotation_Vector(angular_velocity)*rigid_body.Frame().r;rigid_body.Frame().r.Normalize();
        rigid_body_collisions.skip_collision_check.Set_Last_Moved(rigid_body.particle_index);
        parent_rigid_body.Update_Angular_Velocity();}
...
//#####################################################################
// Function Pull_In_Rigid_Deformable_Collision_Pair
//#####################################################################
template<class TV> TV RIGID_DEFORMABLE_COLLISIONS<TV>::
Pull_In_Rigid_Deformable_Collision_Pair(RIGID_BODY<TV>& rigid_body,const int particle_index,const T dt,const TV& original_relative_velocity,const bool check_succeeded_only,const bool apply_impulse)
{
    ...
    depth-=solids_parameters.rigid_body_collision_parameters.collision_body_thickness;
    TV collision_normal=rigid_body.Implicit_Geometry_Normal(particles.X(particle_index));
    TV continued_motion_direction=original_relative_velocity.Normalized();
    TV body_V=rigid_body.Pointwise_Object_Velocity(particles.X(particle_index));
    // need collision original and final relative velocity - assume final is zero?
    //T final_normal_relative_velocity=TV::Dot_Product(particles.V(particle_index)-body_V,collision_normal); // final
    //T original_normal_relative_velocity= 
    TV applied_collision_velocity=original_relative_velocity;
    T normal_applied_collision_velocity=TV::Dot_Product(applied_collision_velocity,collision_normal);
    T normal_applied_position_change=normal_applied_collision_velocity*dt;
    T current_relative_normal_velocity=TV::Dot_Product(particles.V(particle_index)-body_V,collision_normal); // any correction we've already done
    // depth is how far we want to go; depth/dt is how much velocity we need to do it
    // NOTE: for a kinematic we can just base it on the particle, but for dynamic both bodies ought to move together, and not an equal amount
    // Need to put it back as an impulse - targeting a relative velocity is sufficient, I guess?
    T target_normal_relative_velocity=-depth-current_relative_normal_velocity;
    TV impulse;
    if(check_succeeded_only){
        if(target_normal_relative_velocity>-normal_applied_position_change-current_relative_normal_velocity)
            particles_contacting_rigid_body.Get_Or_Insert(rigid_body.particle_index).Set(particle_index);}
    else{
