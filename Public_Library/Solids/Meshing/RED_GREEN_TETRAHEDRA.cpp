    ...
    for(int i=0;i<segment_list.m;i++){
        ...
        if(Find_Edge_And_Test_If_Green(node1,node2,&s,&level,&tet)){
            ...
            if((node1 == a && node2 == segment_midpoints(bc)) || (node2 == a && node1 == segment_midpoints(bc)) || (node1 == b && node2 == segment_midpoints(ca)) || 
                ...
                if(segment_midpoints(ab)<0) Add_Midpoint(ab,level-1,parent_index);
                if(segment_midpoints(bc)<0) Add_Midpoint(bc,level-1,parent_index);
                ...
                if(segment_midpoints(ca)<0) Add_Midpoint(ca,level-1,parent_index);
                if(segment_midpoints(cd)<0) Add_Midpoint(cd,level-1,parent_index);
                if(segment_midpoints(ad)<0) Add_Midpoint(ad,level-1,parent_index);}
...
Find_Edge_And_Test_If_Green(const int node1,const int node2,int* segment_output,int* level_output,int* tet_output)
{
    ...
    PHYSBAM_FATAL_ERROR();
  found_tet:
    if(level_output) *level_output=level;
    if(tet_output) *tet_output=tet;
...
Refine_If_Necessary(const int level,const int tet)
{
    ...
    while(number_children < 8 && (*children(level))(tet)(number_children)>=0) number_children++;
    switch(number_midpoints){
        case 1:
            if(number_children == 2) return;
            break;
        case 2:
            if(number_children == 4) return;
            break;
        case 3:
            if(number_children == 4 && !(midpoints(0)>=0 && midpoints(5)>=0) &&
                !(midpoints(1)>=0 && midpoints(3)>=0) && !(midpoints(2)>=0 && midpoints(4)>=0))
                return;
            break;
        case 6:
            if(number_children==8) return;
            break;
        default: ;}
    ...
    if(!Red(level,tet)){ // Green case - most likely refine parent unless we get lucky and can upgrade Green from 2 to 4
        ...
        int number_children=0;
        while(number_children < 8 && (*children(level-1))(p)(number_children)>=0) number_children++;
        if(number_children < 4){ // try to update Green from 2 to 4
            Refine_If_Necessary(level-1,p); // re-refine the parent
            int new_number_children=0;
            while(new_number_children < 8 && (*children(level-1))(p)(new_number_children)>=0) new_number_children++;
    ...
    if(number_midpoints > 3){Regularly_Refine_Tet(level,tet);
        return;} // too many to go green
    ...
    if(number_midpoints > 1){
        if(midpoints(0)<0 && ((midpoints(1)>=0 && midpoints(2)>=0) || (midpoints(3)>=0 && midpoints(4)>=0))){
            midpoints(0)=Add_Midpoint((*meshes(level)->element_edges)(tet)(0),level,tet);
            number_midpoints++;}
        else if(midpoints(1)<0 && ((midpoints(2)>=0 && midpoints(0)>=0) || (midpoints(4)>=0 && midpoints(5)>=0))){
            midpoints(1)=Add_Midpoint((*meshes(level)->element_edges)(tet)(1),level,tet);
            number_midpoints++;}
        else if(midpoints(2)<0 && ((midpoints(0)>=0 && midpoints(1)>=0) || (midpoints(3)>=0 && midpoints(5)>=0))){
            midpoints(2)=Add_Midpoint((*meshes(level)->element_edges)(tet)(2),level,tet);
            number_midpoints++;}
        else if(midpoints(3)<0 && ((midpoints(4)>=0 && midpoints(0)>=0) || (midpoints(5)>=0 && midpoints(2)>=0))){
            midpoints(3)=Add_Midpoint((*meshes(level)->element_edges)(tet)(3),level,tet);
            number_midpoints++;}
        else if(midpoints(4)<0 && ((midpoints(0)>=0 && midpoints(3)>=0) || (midpoints(5)>=0 && midpoints(1)>=0))){
            midpoints(4)=Add_Midpoint((*meshes(level)->element_edges)(tet)(4),level,tet);
            number_midpoints++;}
        else if(midpoints(5)<0 && ((midpoints(2)>=0 && midpoints(3)>=0) || (midpoints(1)>=0 && midpoints(4)>=0))){
            midpoints(5)=Add_Midpoint((*meshes(level)->element_edges)(tet)(5),level,tet);
            number_midpoints++;}
    ...
    free_tet_indices.Preallocate(8);
    free_edge_indices.Preallocate(24);
    ...
    int i,j,k,l;
    meshes(level)->elements(tet).Get(i,j,k,l);
    ...
    if(midpoints(0)){ // if edge ij is split
        ...
        if(subedges(3)<0) Add_Segment(free_edge_indices,j,midpoints(0));
    ...
    if(midpoints(1)){ // if edge jk is split
        if(subedges(4)<0) Add_Segment(free_edge_indices,j,midpoints(1));
        if(subedges(6)<0) Add_Segment(free_edge_indices,k,midpoints(1));
    ...
    if(midpoints(2)){ // if edge ki is split
        ...
        if(subedges(7)<0) Add_Segment(free_edge_indices,k,midpoints(2));
    ...
    if(midpoints(3)){ // if edge il is split
        if(subedges(2)<0) Add_Segment(free_edge_indices,i,midpoints(3));
        if(subedges(9)<0) Add_Segment(free_edge_indices,l,midpoints(3));
    ...
    if(midpoints(4)){ // if edge jl is split
        if(subedges(5)<0) Add_Segment(free_edge_indices,j,midpoints(4));
        if(subedges(10)<0) Add_Segment(free_edge_indices,l,midpoints(4));
    ...
    if(midpoints(5)){ // if edge kl is split
        if(subedges(8)<0) Add_Segment(free_edge_indices,k,midpoints(5));
        if(subedges(11)<0) Add_Segment(free_edge_indices,l,midpoints(5));
    ...
    if(midpoints(0)>=0 && midpoints(1)>=0 && midpoints(2)>=0){ // if face ijk is subdivided but the other faces aren't
        if(subedges(12)<0) Add_Segment(free_edge_indices,midpoints(0),midpoints(1));
        if(subedges(13)<0) Add_Segment(free_edge_indices,midpoints(1),midpoints(2));
        if(subedges(14)<0) Add_Segment(free_edge_indices,midpoints(2),midpoints(0));
        Add_Tetrahedron(free_tet_indices,level+1,i,midpoints(0),midpoints(2),l,tet);
        Add_Tetrahedron(free_tet_indices,level+1,j,midpoints(1),midpoints(0),l,tet);
        Add_Tetrahedron(free_tet_indices,level+1,k,midpoints(2),midpoints(1),l,tet);
        Add_Tetrahedron(free_tet_indices,level+1,midpoints(0),midpoints(1),midpoints(2),l,tet);}
    else if(midpoints(0)>=0 && midpoints(3)>=0 && midpoints(4)>=0){ // if face ilj is subdivided but the other faces aren't
        if(subedges(15)<0) Add_Segment(free_edge_indices,midpoints(0),midpoints(4));
        if(subedges(16)<0) Add_Segment(free_edge_indices,midpoints(4),midpoints(3));
        if(subedges(17)<0) Add_Segment(free_edge_indices,midpoints(3),midpoints(0));
        Add_Tetrahedron(free_tet_indices,level+1,i,midpoints(3),midpoints(0),k,tet);
        Add_Tetrahedron(free_tet_indices,level+1,l,midpoints(4),midpoints(3),k,tet);
        Add_Tetrahedron(free_tet_indices,level+1,j,midpoints(0),midpoints(4),k,tet);
        Add_Tetrahedron(free_tet_indices,level+1,midpoints(0),midpoints(3),midpoints(4),k,tet);}
    else if(midpoints(2)>=0 && midpoints(3)>=0 && midpoints(5)>=0){ // if face ikl is subdivided but the other faces aren't
        if(subedges(18)<0) Add_Segment(free_edge_indices,midpoints(2),midpoints(5));
        if(subedges(19)<0) Add_Segment(free_edge_indices,midpoints(5),midpoints(3));
        if(subedges(20)<0) Add_Segment(free_edge_indices,midpoints(3),midpoints(2));
        Add_Tetrahedron(free_tet_indices,level+1,i,midpoints(2),midpoints(3),j,tet);
        Add_Tetrahedron(free_tet_indices,level+1,midpoints(3),midpoints(2),midpoints(5),j,tet);
        Add_Tetrahedron(free_tet_indices,level+1,midpoints(5),midpoints(2),k,j,tet);
        Add_Tetrahedron(free_tet_indices,level+1,l,midpoints(3),midpoints(5),j,tet);}
    else if(midpoints(1)>=0 && midpoints(4)>=0 && midpoints(5)>=0){ // if face jlk is subdivided but the other faces aren't
        if(subedges(21)<0) Add_Segment(free_edge_indices,midpoints(1),midpoints(5));
        if(subedges(22)<0) Add_Segment(free_edge_indices,midpoints(5),midpoints(4));
        if(subedges(23)<0) Add_Segment(free_edge_indices,midpoints(4),midpoints(1));
        Add_Tetrahedron(free_tet_indices,level+1,midpoints(1),j,midpoints(4),i,tet);
        Add_Tetrahedron(free_tet_indices,level+1,midpoints(1),midpoints(5),k,i,tet);
        Add_Tetrahedron(free_tet_indices,level+1,midpoints(1),midpoints(4),midpoints(5),i,tet);
        Add_Tetrahedron(free_tet_indices,level+1,midpoints(5),midpoints(4),l,i,tet);}
    else if(midpoints(0)>=0 && midpoints(5)>=0){ // if edges ij and kl are split
        Add_Segment(free_edge_indices,midpoints(0),midpoints(5));
        Add_Tetrahedron(free_tet_indices,level+1,i,midpoints(0),k,midpoints(5),tet);
        Add_Tetrahedron(free_tet_indices,level+1,k,midpoints(0),j,midpoints(5),tet);
        Add_Tetrahedron(free_tet_indices,level+1,i,l,midpoints(0),midpoints(5),tet);
        Add_Tetrahedron(free_tet_indices,level+1,midpoints(0),l,j,midpoints(5),tet);}
    else if(midpoints(1)>=0 && midpoints(3)>=0){ // if edges jk and il are split
        Add_Segment(free_edge_indices,midpoints(1),midpoints(3));
        Add_Tetrahedron(free_tet_indices,level+1,i,j,midpoints(1),midpoints(3),tet);
        Add_Tetrahedron(free_tet_indices,level+1,i,midpoints(1),k,midpoints(3),tet);
        Add_Tetrahedron(free_tet_indices,level+1,j,l,midpoints(1),midpoints(3),tet);
        Add_Tetrahedron(free_tet_indices,level+1,midpoints(1),l,k,midpoints(3),tet);}
    else if(midpoints(2)>=0 && midpoints(4)>=0){ // if edges ki and jl are split
        Add_Segment(free_edge_indices,midpoints(2),midpoints(4));
        Add_Tetrahedron(free_tet_indices,level+1,j,i,midpoints(4),midpoints(2),tet);
        Add_Tetrahedron(free_tet_indices,level+1,midpoints(4),i,l,midpoints(2),tet);
        Add_Tetrahedron(free_tet_indices,level+1,k,j,midpoints(4),midpoints(2),tet);
        Add_Tetrahedron(free_tet_indices,level+1,k,midpoints(4),l,midpoints(2),tet);}
    else if(midpoints(0)){ // if just edge ij is split
        Add_Tetrahedron(free_tet_indices,level+1,i,midpoints(0),k,l,tet);
        Add_Tetrahedron(free_tet_indices,level+1,midpoints(0),j,k,l,tet);}
    else if(midpoints(1)){ // if just edge jk is split
        Add_Tetrahedron(free_tet_indices,level+1,i,j,midpoints(1),l,tet);
        Add_Tetrahedron(free_tet_indices,level+1,i,midpoints(1),k,l,tet);}
    else if(midpoints(2)){ // if just edge ki is split
        Add_Tetrahedron(free_tet_indices,level+1,j,k,midpoints(2),l,tet);
        Add_Tetrahedron(free_tet_indices,level+1,j,midpoints(2),i,l,tet);}
    else if(midpoints(3)){ // if just edge il is split
        Add_Tetrahedron(free_tet_indices,level+1,j,i,midpoints(3),k,tet);
        Add_Tetrahedron(free_tet_indices,level+1,j,midpoints(3),l,k,tet);}
    else if(midpoints(4)){ // if just edge jl is split
        Add_Tetrahedron(free_tet_indices,level+1,i,midpoints(4),j,k,tet);
        Add_Tetrahedron(free_tet_indices,level+1,i,l,midpoints(4),k,tet);}
    else if(midpoints(5)){ // if just edge kl is split
        Add_Tetrahedron(free_tet_indices,level+1,i,k,midpoints(5),j,tet);
        Add_Tetrahedron(free_tet_indices,level+1,i,midpoints(5),l,j,tet);}
}
//#####################################################################
...
Regularly_Refine_Tet(const int level,const int tet)
{
    ...
    if(refinement_target_segments.Contains(element_edges(tet)(2)) && refinement_target_segments.Contains(element_edges(tet)(4))){first_midpoint_index=2;
        second_midpoint_index=4;}
    else if(refinement_target_segments.Contains(element_edges(tet)(1)) && refinement_target_segments.Contains(element_edges(tet)(3))){first_midpoint_index=1;
        second_midpoint_index=3;}
    else{first_midpoint_index=0;
        second_midpoint_index=5;}
    ...
    if(subedges(2)<0) Add_Segment(free_edge_indices,i,il);
    if(subedges(3)<0) Add_Segment(free_edge_indices,j,ij);
    if(subedges(4)<0) Add_Segment(free_edge_indices,j,jk);
    if(subedges(5)<0) Add_Segment(free_edge_indices,j,jl);
    if(subedges(6)<0) Add_Segment(free_edge_indices,k,jk);
    if(subedges(7)<0) Add_Segment(free_edge_indices,k,ki);
    if(subedges(8)<0) Add_Segment(free_edge_indices,k,kl);
    if(subedges(9)<0) Add_Segment(free_edge_indices,l,il);
    if(subedges(10)<0) Add_Segment(free_edge_indices,l,jl);
    if(subedges(11)<0) Add_Segment(free_edge_indices,l,kl);
    if(subedges(12)<0) Add_Segment(free_edge_indices,ij,jk);
    if(subedges(13)<0) Add_Segment(free_edge_indices,jk,ki);
    if(subedges(14)<0) Add_Segment(free_edge_indices,ki,ij);
    if(subedges(15)<0) Add_Segment(free_edge_indices,ij,jl);
    if(subedges(16)<0) Add_Segment(free_edge_indices,jl,il);
    if(subedges(17)<0) Add_Segment(free_edge_indices,il,ij);
    if(subedges(18)<0) Add_Segment(free_edge_indices,ki,kl);
    if(subedges(19)<0) Add_Segment(free_edge_indices,kl,il);
    if(subedges(20)<0) Add_Segment(free_edge_indices,il,ki);
    if(subedges(21)<0) Add_Segment(free_edge_indices,jk,kl);
    if(subedges(22)<0) Add_Segment(free_edge_indices,kl,jl);
    if(subedges(23)<0) Add_Segment(free_edge_indices,jl,jk);
    ...
    Add_Tetrahedron(free_tet_indices,level+1,i,ij,ki,il,tet);
    Add_Tetrahedron(free_tet_indices,level+1,ij,j,jk,jl,tet);
    Add_Tetrahedron(free_tet_indices,level+1,ki,jk,k,kl,tet);
    Add_Tetrahedron(free_tet_indices,level+1,il,jl,kl,l,tet);

    if(first_midpoint_index==0){
        Add_Tetrahedron(free_tet_indices,level+1,kl,ij,il,ki,tet);
        Add_Tetrahedron(free_tet_indices,level+1,kl,ij,jk,jl,tet);
        Add_Tetrahedron(free_tet_indices,level+1,kl,ij,ki,jk,tet);
        Add_Tetrahedron(free_tet_indices,level+1,kl,ij,jl,il,tet);}
    else if(first_midpoint_index==1){
        Add_Tetrahedron(free_tet_indices,level+1,jk,il,ij,jl,tet);
        Add_Tetrahedron(free_tet_indices,level+1,jk,il,ki,ij,tet);
        Add_Tetrahedron(free_tet_indices,level+1,jk,il,kl,ki,tet);
        Add_Tetrahedron(free_tet_indices,level+1,jk,il,jl,kl,tet);}
    else{
        Add_Tetrahedron(free_tet_indices,level+1,jk,ki,jl,kl,tet);
        Add_Tetrahedron(free_tet_indices,level+1,ij,ki,jl,jk,tet);
        Add_Tetrahedron(free_tet_indices,level+1,il,ki,jl,ij,tet);
        Add_Tetrahedron(free_tet_indices,level+1,kl,ki,jl,il,tet);}
}
//#####################################################################
...
Get_Existing_Subindices(const int level,const int tet,VECTOR<int,6>& midpoints,VECTOR<int,24>& subedges)
{
    ...
    int ij=midpoints(0),jk=midpoints(1),ki=midpoints(2),il=midpoints(3),jl=midpoints(4),kl=midpoints(5);
    if(ij>=0){
        subedges(0)=segment_mesh.Segment(i,ij);
        subedges(3)=segment_mesh.Segment(j,ij);
        if(jk>=0) subedges(12)=segment_mesh.Segment(ij,jk);
        if(ki) subedges(14)=segment_mesh.Segment(ki,ij);
        if(il>=0) subedges(17)=segment_mesh.Segment(il,ij);
        if(jl) subedges(15)=segment_mesh.Segment(ij,jl);}
    if(jk>=0){
        subedges(4)=segment_mesh.Segment(j,jk);
        subedges(6)=segment_mesh.Segment(k,jk);
        if(ki>=0) subedges(13)=segment_mesh.Segment(jk,ki);
        if(jl) subedges(23)=segment_mesh.Segment(jl,jk);
        if(kl) subedges(21)=segment_mesh.Segment(jk,kl);}
    if(ki>=0){
        subedges(1)=segment_mesh.Segment(i,ki);
        subedges(7)=segment_mesh.Segment(k,ki);
        if(il>=0) subedges(20)=segment_mesh.Segment(il,ki);
        if(kl) subedges(18)=segment_mesh.Segment(ki,kl);}
    if(il>=0){
        subedges(2)=segment_mesh.Segment(i,il);
        subedges(9)=segment_mesh.Segment(l,il);
        if(jl>=0) subedges(16)=segment_mesh.Segment(jl,il);
        if(kl) subedges(19)=segment_mesh.Segment(kl,il);}
    if(jl>=0){
        subedges(5)=segment_mesh.Segment(j,jl);
        subedges(10)=segment_mesh.Segment(l,jl);
        if(kl>=0) subedges(22)=segment_mesh.Segment(kl,jl);}
    if(kl>=0){subedges(8)=segment_mesh.Segment(k,kl);
        subedges(11)=segment_mesh.Segment(l,kl);}    
}
//#####################################################################
...
Ensure_Level_Exists(const int level)
{
    ...
    children.Append(new ARRAY<VECTOR<int,8> >());
    index_in_stack.Append(new ARRAY<int>);
    leaf_number.Append(new ARRAY<int>);
}
//#####################################################################
...
Delete_Children(const int level,const int tet,ARRAY<int>& deleted_tet_indices,ARRAY<int>& deleted_edge_indices)
{
    ...
    int p;
    for(p=0;p<8&&(*children(level))(tet)(p)>=0;p++){
        deleted_tet_indices.Append((*children(level))(tet)(p));(*children(level))(tet)(p)=-1;}
        ...
        if((q==ij&&r==kl) || (q==jk&&r==il) || (q==ki&&r==jl) || (q==il&&r==jk) || (q==jl&&r==ki) || (q==kl&&r==ij)) 
            ...
            for(int s=0;s<(*meshes(level+1)->incident_elements)(q).m;s++){ // check incident tets for this edge
                ...
                if((*parent(level+1))(t)!=tet){ // only look at tets with a different parent (i.e. not children that should be deleted)
                    int a,b,c,d;
                    meshes(level+1)->elements(t).Get(a,b,c,d);
    ...
    for(p=0;p<deleted_tet_indices.m;p++) for(int q=0;q<4;q++){ // remove deleted tets from incident_tets
            ...
            (*meshes(level+1)->incident_elements)(node).Find(deleted_tet_indices(p),index);
            PHYSBAM_ASSERT(index>=0);
...
Add_Midpoint(const int segment,const int level,const int tet)
{
    ...
    int new_node=object.particles.Add_Element();
    ...
    for(i=0;i<(*meshes(level)->incident_elements)(node1).m;i++){
        ...
        if(s != tet && (*index_in_stack(level))(s)<0){
            int a,b,c,d;
            meshes(level)->elements(s).Get(a,b,c,d);
            assert(a == node1 || b == node1 || c == node1 || d == node1);
    ...
    if(level > 1 && node1 < meshes(level-1)->number_nodes) for(i=0;i<(*meshes(level-1)->incident_elements)(node1).m;i++){
            ...
            if((*index_in_stack(level-1))(s)<0){
                int a,b,c,d;
                meshes(level-1)->elements(s).Get(a,b,c,d);
                if(node2 == a || node2 == b || node2 == c || node2 == d){ // not incident on edge
...
Add_Tetrahedron(ARRAY<int>& free_tet_indices,const int level,const int i,const int j,const int k,const int l,const int parent_index)
{
    ...
    if(free_tet_indices.m == 0){
        ...
        tet_mesh.element_edges->Resize(index+1);
    ...
    int a;
    for(a=0;a<4;a++) (*tet_mesh.incident_elements)(tet_mesh.elements(index)(a)).Append(index);
    // add tet to parent's list of children
    for(a=0;a<8;a++) if((*children(level-1))(parent_index)(a)<0){(*children(level-1))(parent_index)(a)=index;
            break;}
}
//#####################################################################
...
Rebuild_Object()
{
    ...
    int level,tet;
    for(level=0;level<meshes.m;level++) for(tet=0;tet<meshes(level)->elements.m;tet++) if(Leaf(level,tet)) number_of_leaves++;
    ...
    for(level=0;level<meshes.m;level++) for(tet=0;tet<meshes(level)->elements.m;tet++) 
                                            if(Leaf(level,tet)){
                                                ...
                                                leaf_levels_and_indices(index_into_tets).Set(level,tet);(*leaf_number(level))(tet)=index_into_tets;
                                                index_into_tets++;}
...
Coarsen_Green_Refinements(TETRAHEDRON_MESH& final_mesh,ARRAY<int>& t_junctions,ARRAY<VECTOR<int,2> >& t_junction_parents) const
{
    ...
    for(int t=0;t<object.mesh.elements.m;t++){
        int level,tet;
        leaf_levels_and_indices(t).Get(level,tet);
        if(Red(level,tet)) final_mesh.elements.Append(object.mesh.elements(t));
            ...
            for(int i=0;i<4;i++){
                int child=(*children(level-1))(parent_tet)(i);
                if(child<0) break;
                parent_already_coarsened((*leaf_number(level))(child))=true;}}}
    ARRAY<bool> occurs_in_final_mesh(object.particles.Size());
    final_mesh.Mark_Nodes_Referenced(occurs_in_final_mesh,true);
    ARRAY<bool> is_t_junction(object.particles.Size());
    t_junctions.Clean_Memory();
    t_junction_parents.Clean_Memory();
    for(int t=0;t<coarse_parents.m;t++){
        int level,tet;
        coarse_parents(t).Get(level,tet);
        for(int e=0;e<6;e++){
            ...
            if(midpoint && occurs_in_final_mesh(midpoint) && !is_t_junction(midpoint)){
                ...
                t_junctions.Append(midpoint);
                t_junction_parents.Append(segment_mesh.elements(s));}}}
    ...
    bool allow_red_coarsening,ARRAY<bool>* node_is_uncoarsenable) const
{
    ...
    for(int level=0;level<meshes.m;level++){tetrahedron_kept(level).Resize(meshes(level)->elements.m);
        tetrahedron_kept(level).Fill(true);}
    for(int t=0;t<mesh.elements.m;t++) if(!subset(t)){
            int tet,level;
            leaf_levels_and_indices(t).Get(level,tet);
            tetrahedron_kept(level)(tet)=false;
            Unmark_Parents(tetrahedron_kept,level,tet);}
    if(!allow_red_coarsening) for(int level=meshes.m;level>=2;level--) for(int tet=0;
                                                                           tet<meshes(level)->elements.m;
                                                                           tet++)
    ...
    if(node_is_uncoarsenable) for(int level=meshes.m;level>=2;level--) for(int tet=0;
                                                                           tet<meshes(level)->elements.m;
                                                                           tet++)
    ...
    final_mesh.Clean_Memory();
    for(int level=0;level<meshes.m;level++) for(int tet=0;
                                                tet<meshes(level)->elements.m;
                                                tet++) if(tetrahedron_kept(level)(tet)){
                Unmark_Children(tetrahedron_kept,level,tet);
                final_mesh.elements.Append(meshes(level)->elements(tet));}
    ...
    PHYSBAM_ASSERT(object.mesh.incident_elements);
    final_mesh.Initialize_Incident_Elements();
    ...
    for(int node=0;node<mesh.number_nodes;node++){
        ...
        for(int t=0;t<(*mesh.incident_elements)(node).m;t++){
            ...
            int tet,level;
            leaf_levels_and_indices(incident_tet).Get(level,tet);
            adjusted_incident_elements++;
            if(Red(level,tet)) continue;
            ...
            for(int i=0;siblings(i);i++) if(!subset((*leaf_number(level))(siblings(i)))){collapse_siblings=false;
                    break;}
    ...
    for(int s=0;s<segment_midpoints.m;s++) if(segment_midpoints(s)>=0 && is_t_junction(segment_midpoints(s))){
            t_junctions.Append(segment_midpoints(s));
            t_junction_parents.Append(segment_mesh.elements(s));}
}
//#####################################################################
...
{
    while(parent(level)){tet=(*parent(level--))(tet);
        tetrahedron_kept(level)(tet)=false;}
}
//#####################################################################
...
Unmark_Children(ARRAY<ARRAY<bool> >& tetrahedron_kept,int level,int tet) const
{
    if(children(level)) for(int i=0;i<8;i++){
            int child=(*children(level))(tet)(i);
            if(child<0) break;
            tetrahedron_kept(level+1)(child)=false;
            Unmark_Children(tetrahedron_kept,level+1,child);}
}
//#####################################################################
...
Remove_Simplex_List(const ARRAY<int>& tetrahedron_list,ARRAY<HASHTABLE<int,int> >* level_simplex_maps)
{
    ...
    for(int i=0;i<tetrahedron_list.m;i++){
        int level,tet;
        leaf_levels_and_indices(tetrahedron_list(i)).Get(level,tet);
    ...
    for(int level=0;level<meshes.m;level++){
        ...
        for(int i=0;i<level_tetrahedron_list(level).m;i++) for(int j=0;j<3;j++){int node=meshes(level)->elements(level_tetrahedron_list(level)(i))(j);
                int index=-1;(*meshes(level)->incident_elements)(node).Find(level_tetrahedron_list(level)(i),index);
                PHYSBAM_ASSERT(index>=0);
        ...
        if(level>0) for(int i=0;i<children(level-1)->m;i++){for(int j=0;j<4;j++) simplex_map.Get((*children(level-1))(i)(j),(*children(level-1))(i)(j));
                ...
                for(int i=0;i<number_of_red_children;i++) if(child_list(i)==-1) for(int j=i+1;j<number_of_red_children;j++) if(child_list(j)>=0){exchange(child_list(j),child_list(i));
                                break;}}
...
Initialize()
{
    ...
    int t;
    for(t=0;t<meshes(0)->elements.m;t++){leaf_levels_and_indices(t)(0)=0;
        leaf_levels_and_indices(t)(1)=t;}
