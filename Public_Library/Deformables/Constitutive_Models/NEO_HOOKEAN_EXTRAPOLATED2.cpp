...
P_From_Strain_Helper(const DIAGONAL_MATRIX<T,2>& F,const T scale,const int simplex) const
{
    ...
    {//[ -(la*(a - s2)^2*(2*a - 2*s1))/a, -(la*(a - s1)^2*(2*a - 2*s2))/a, 0, 0]
        ...
        result.x.x = base.Ex(a,y) + 2*k*dx-constant_lambda*(a-y)*(a-y)*(T)2*(a-x)/a;
        result.x.y = base.Ey(a,y) + base.Exy(a,y)*dx - constant_lambda*(a-x)*(a-x)*(T)2*(a-y)/a;
        return scale*result;
    }
    else if ((dx >= 0) && (dy < 0))
    {
        DIAGONAL_MATRIX<T,2> result;
        result.x.x = base.Ex(x,a) + base.Exy(x,a)*dy -constant_lambda*(a-y)*(a-y)*(T)2*(a-x)/a;
        result.x.y = base.Ey(x,a) + 2*k*dy - constant_lambda*(a-x)*(a-x)*(T)2*(a-y)/a;
        return scale*result;
    }
    else // ((dx < 0) && (dy < 0))
    {
        DIAGONAL_MATRIX<T,2> result;
        result.x.x = base.Ex(a,a) + base.Exy(a,a)*dy + 2*k*dx -constant_lambda*(a-y)*(a-y)*(T)2*(a-x)/a;
        result.x.y = base.Ey(a,a) + base.Exy(a,a)*dx + 2*k*dy - constant_lambda*(a-x)*(a-x)*(T)2*(a-y)/a;
        return scale*result;
    }
}
//#####################################################################
...
Isotropic_Stress_Derivative_Helper(const DIAGONAL_MATRIX<T,2>& F,DIAGONALIZED_ISOTROPIC_STRESS_DERIVATIVE<T,2>& dP_dF,const int triangle) const
{
    ...
    { /* (2*la*(a - s2)^2)/a, (4*la*(a - s1)*(a - s2))/a,                                                     0,                                                     0]
       [ (4*la*(a - s1)*(a - s2))/a,        (2*la*(a - s1)^2)/a,*/
        ...
        dP_dF.x0000=2*k + (T)2*constant_lambda*(a-y)*(a-y)/a;
        dP_dF.x1111=base.Eyy(a,y)+base.Exyy(a,y)*dx + (T)2*constant_lambda*(a-x)*(a-x)/a;
        dP_dF.x1100=base.Exy(a,y) + (4*constant_lambda*(a - x)*(a -y))/a;
        ...
        dP_dF.x1001=(-Ey*x+Ex*y)/(xpy*xmy) -((T)2*constant_lambda*(a - x)*(a - y)*(x - a + y))/(a*(x + y));
        dP_dF.x1010=(-Ey*y+Ex*x)/(xpy*xmy)  -((T)2*constant_lambda*(a - x)*(a - y))/(x + y); 
    }
    else if ((dx >= 0) && (dy < 0))
    {
        dP_dF.x0000=base.Exx(x,a)+base.Exxy(x,a)*dy+ (T)2*constant_lambda*(a-y)*(a-y)/a;
        dP_dF.x1111=2*k+ (T)2*constant_lambda*(a-x)*(a-x)/a;
        dP_dF.x1100=base.Exy(x,a)+ (4*constant_lambda*(a - x)*(a -y))/a;
    ...
    else // ((dx < 0) && (dy < 0))
    {
        dP_dF.x0000=2*k+ (T)2*constant_lambda*(a-y)*(a-y)/a;
        dP_dF.x1111=2*k+ (T)2*constant_lambda*(a-x)*(a-x)/a;
        dP_dF.x1100=base.Exy(a,a)+ (4*constant_lambda*(a - x)*(a -y))/a;

        T xpy = x+y; if (fabs(xpy)<panic_threshold) xpy=xpy<0?-panic_threshold:panic_threshold;
        T cmn = (base.Ex(a,a)-base.Exy(a,a)*a-2*k*a)/xpy;

        dP_dF.x1001=-cmn-base.Exy(a,a) -((T)2*constant_lambda*(a - x)*(a - y)*(x - a + y))/(a*(x + y));
        dP_dF.x1010=cmn+2*k-((T)2*constant_lambda*(a - x)*(a - y))/(x + y); 
    }
