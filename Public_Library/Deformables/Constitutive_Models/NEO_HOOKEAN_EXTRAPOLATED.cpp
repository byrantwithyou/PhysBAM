...
Isotropic_Stress_Derivative_Helper(const DIAGONAL_MATRIX<T,2>& F,DIAGONALIZED_ISOTROPIC_STRESS_DERIVATIVE<VECTOR<T,2> >& dP_dF,const int id) const
{
    ...
    { /* (2*la*(a - s2)^2)/a, (4*la*(a - s1)*(a - s2))/a,                                                     0,                                                     0]
       [ (4*la*(a - s1)*(a - s2))/a,        (2*la*(a - s1)^2)/a,*/
        ...
        dP_dF.H(0,0)=2*k;
        dP_dF.H(1,1)=base.Eyy(a,y)+base.Exyy(a,y)*dx;
        dP_dF.H(1,0)=base.Exy(a,y);
        ...
        dP_dF.C(0)=(-Ey*x+Ex*y)/(xpy*xmy);
        dP_dF.B(0)=(-Ey*y+Ex*x)/(xpy*xmy); 
    }
    else if((dx >= 0) && (dy < 0))
    {
        dP_dF.H(0,0)=base.Exx(x,a)+base.Exxy(x,a)*dy;
        dP_dF.H(1,1)=2*k;
        dP_dF.H(1,0)=base.Exy(x,a);
    ...
    else // ((dx < 0) && (dy < 0))
    {
        dP_dF.H(0,0)=2*k;
        dP_dF.H(1,1)=2*k;
        dP_dF.H(1,0)=base.Exy(a,a);

        T xpy = x+y; if(fabs(xpy)<panic_threshold) xpy=xpy<0?-panic_threshold:panic_threshold;
        T cmn = (base.Ex(a,a)-base.Exy(a,a)*a-2*k*a)/xpy;

        dP_dF.C(0)=-cmn-base.Exy(a,a);
        dP_dF.B(0)=cmn+2*k; 
    }
