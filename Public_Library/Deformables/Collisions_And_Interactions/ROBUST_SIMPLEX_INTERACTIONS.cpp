...
#include <Deformables/Collisions_And_Interactions/ROBUST_SIMPLEX_INTERACTIONS.h>
...
Triple_Product(const VECTOR<TV,3>& locations)
{
    ...
    for(int i=0;i<6;i++) if(triple_products.Get(permute_three(locations,i),result)){if(!permutation_of_three_is_even(i)) exchange(result.x,result.y);return result;}
    T product;
...
Intersection(const VECTOR<TV,4>& tetrahedron,const VECTOR<TV,3>& triangle,bool *is_robust_input)
{
    ...
    for(int i=0;i<2;i++) for(int j=i+1;j<3;j++) for(int k=0;k<4;k++){
...
{
    VECTOR<T,2> result; // (non_negative_sum,negative_sum)
    for(int i=0;i<2;i++) if(cross_products.Get(permute_two(locations,i),result)){if(!permutation_of_two_is_even(i)) exchange(result.x,result.y);return result;}
    T product;
...
{
    PAIR<T,bool> result;
    for(int i=0;i<6;i++) if(signed_areas_times_two.Get(permute_three(locations,i),result)){
        if(!permutation_of_three_is_even(i)) result.x=-result.x;return result;}
    VECTOR<T,2> determinant=Cross_Product(VECTOR<TV,2>(locations[1],locations[2]))+Cross_Product(VECTOR<TV,2>(locations[2],locations[0]))+Cross_Product(VECTOR<TV,2>(locations[0],locations[1]));
    result.x=TV::Cross_Product(locations[1]-locations[0],locations[2]-locations[0]).x;
    if(abs(determinant.x-determinant.y)<tolerance*(determinant.x+determinant.y)) result.y=false;
    else{result.y=true;
        bool positive1=(determinant.x-determinant.y>0),positive2=(result.x>0);
        if(positive1^positive2) PHYSBAM_FATAL_ERROR();}
