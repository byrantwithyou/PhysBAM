...
// Constructor
//#####################################################################
template<class TV> MOONEY_RIVLIN_B_SPLINE_CURVATURE_FORCE<TV>::
MOONEY_RIVLIN_B_SPLINE_CURVATURE_FORCE(DEFORMABLE_PARTICLES<TV>& particles,const B_SPLINE<TV,3>& spline_input,
    ...
    :DEFORMABLES_FORCES<TV>(particles),spline(spline_input),epsilon(epsilon),
    stiffness(stiffness)
{
...
//#####################################################################
template<class TV> MOONEY_RIVLIN_B_SPLINE_CURVATURE_FORCE<TV>::
~MOONEY_RIVLIN_B_SPLINE_CURVATURE_FORCE()
{
}
//#####################################################################
...
//#####################################################################
template<class TV> typename TV::SCALAR MOONEY_RIVLIN_B_SPLINE_CURVATURE_FORCE<TV>::
...
Update_Position_Based_State(const T time,const bool is_position_update,const bool update_hessian)
{
    ...
    if(data.m!=spline.knots.m-5){
        ...
        for(int i=2;i<spline.knots.m-3;i++){
            ...
            for(int j=0;j<gauss_order;j++){
                ...
                T m_hat=cube(m_bar);
                TV N=rot*phi_p;
                T length_p=m_bar*phi_p.Dot(phi_pp);
                T xi=N.Dot(phi_pp);
                T kappa=m_hat*xi;
                data(i-2).kappa0(j)=kappa;
                data(i-2).length0(j)=length;
                data(i-2).length_p0(j)=length_p;}}}
    ...
    for(int i=2;i<spline.knots.m-3;i++){
        ...
        for(int j=0;j<gauss_order;j++){
            ...
            auto length_p=m_bar*phi_p.Dot(phi_pp);
            auto kappa=m_hat*xi;
            auto lambda=m_bar*dat.length0(j);
            auto lambda_p=(dat.length_p0(j)*length-dat.length0(j)*length_p)*sqr(m_bar);
            VECTOR<T,3> simpson_weight(epsilon/3,4*epsilon/3,epsilon/3);
            for(int k=-1;k<=1;k++){
                T v=epsilon*k;
...
template class MOONEY_RIVLIN_B_SPLINE_CURVATURE_FORCE<VECTOR<float,2> >;
template class MOONEY_RIVLIN_B_SPLINE_CURVATURE_FORCE<VECTOR<double,2> >;
}
