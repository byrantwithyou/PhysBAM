...
public:
    ...
    {
        if(!plastic_yield) plastic_yield=new ARRAY<T>;
        plastic_yield->Resize(bending_quadruples.m,init_all,plastic_yield_input);
        if(!plastic_hardening) plastic_hardening=new ARRAY<T>;
        plastic_hardening->Resize(bending_quadruples.m,init_all,plastic_hardening_input);
