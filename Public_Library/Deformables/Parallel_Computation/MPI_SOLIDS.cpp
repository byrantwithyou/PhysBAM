...
#include <Deformables/Parallel_Computation/MPI_REPULSION_UTILITIES.h>
#endif
    ...
    :deformable_body_collection(0),current_tag(0)
{
    ...
    LOG::cout<<"number of ranks = "<<number_of_ranks<<std::endl;
...
Gather_Data(ARRAY_VIEW<T_DATA> array) const
{
    ...
    if(rank==0){
        ...
        for(PARTITION_ID p(0);p<particles_of_partition.m;p++){
            ...
            if(p>PARTITION_ID(0)) offsets(p)=offsets(p-1)+counts(p-1);}
        ...
        ARRAY<ARRAY<PAIR<VECTOR<int,2>,VECTOR<typename TV::SCALAR,2> > >,PARTITION_ID>& pairs_received)
{
    ...
    for(PARTITION_ID dummy_partition(0);dummy_partition<pairs_received.Size();dummy_partition++){ // n-1 dummy indices
        MPI::Status probe_status;comm->Probe(MPI::ANY_SOURCE,tag,probe_status);
        ARRAY<char> buffer(probe_status.Get_count(MPI::PACKED));int position=0;
        comm->Recv(buffer.Get_Array_Pointer(),buffer.m,MPI::PACKED,probe_status.Get_source(),probe_status.Get_tag());
...
Broadcast_Helper(const ARRAY<MPI_PACKAGE*>& packages) const
{
    ...
    union_packages.Append(MPI_PACKAGE::Union(packages_to_union));
    union_packages(0).Broadcast(*comm);
                                                                     ...
                                                                     T_ARRAY_PAIR& my_boundary_pairs,T_ARRAY_PAIR& my_internal_pairs)
{
    ...
    if(mpi_solids.rank==0){
        ...
        for(PARTITION_ID processor(0);processor<mpi_solids.particles_of_partition.Size();processor++){
            ...
            requests.Append(mpi_solids.comm->Isend(&buffers(processor)(0),position,MPI::PACKED,mpi_solids.Partition_To_Rank(processor),tag));}
        send_particles=send_matrix(PARTITION_ID(0));receive_particles=receive_matrix(PARTITION_ID(0));
        my_boundary_pairs=pairs.Subset(processor_pair_indices(PARTITION_ID(0)));my_internal_pairs=pairs.Subset(internal_pairs(PARTITION_ID(0)));
        MPI_UTILITIES::Wait_All(requests);}
    else{
...
{
    LOG::cout<<"======================================================================"<<std::endl;
    LOG::cout<<"MPI_SOLIDS Diagnostics - Rank="<<rank<<", number_of_processes="<<Number_Of_Processors()<<std::endl;
    LOG::cout<<"======================================================================"<<std::endl;
    LOG::cout<<"particles_of_partition.m="<<particles_of_partition.Size()<<std::endl;
    LOG::cout<<"mpi_partition_binding.neighbor_partitions : "<<mpi_partition_binding.neighbor_partitions<<std::endl;
    LOG::cout<<"mpi_partition_binding.ghost_dynamic_particles : "<<mpi_partition_binding.ghost_dynamic_particles<<std::endl;
    LOG::cout<<"mpi_partition_binding.boundary_dynamic_particles : "<<mpi_partition_binding.boundary_dynamic_particles<<std::endl;
    LOG::cout<<"mpi_partition_force.neighbor_partitions : "<<mpi_partition_force.neighbor_partitions<<std::endl;
    LOG::cout<<"mpi_partition_force.ghost_dynamic_particles : "<<mpi_partition_force.ghost_dynamic_particles<<std::endl;
    LOG::cout<<"mpi_partition_force.boundary_dynamic_particles : "<<mpi_partition_force.boundary_dynamic_particles<<std::endl;
}
//#####################################################################
