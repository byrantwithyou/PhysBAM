...
public:
    ...
    ADVECTION_MACCORMACK_UNIFORM(T_NESTED_ADVECTION& nested_advection_input,const ARRAY<bool,TV_INT>* node_mask_input,const ARRAY<bool,TV_INT>* cell_mask_input,const ARRAY<bool,FACE_INDEX<TV::m> >* face_mask_input,THREAD_QUEUE* thread_queue=0);
...
private:
    ...
    void Apply_Clamped_Extrema_Limiter_Face_Threaded(RANGE<TV_INT>& domain,const GRID<TV>& grid,int axis,ARRAY<T,FACE_INDEX<TV::m> >& Z,const ARRAY<T,FACE_INDEX<TV::m> >& Z_forward_ghost,const ARRAY<T,FACE_INDEX<TV::m> >& Z_backward_ghost,const ARRAY<T,FACE_INDEX<TV::m> >& Z_min,const ARRAY<T,FACE_INDEX<TV::m> >& Z_max);
    ...
    void Apply_Reversion_Limiter_Face_Threaded(RANGE<TV_INT>& domain,const GRID<TV>& grid,int axis,ARRAY<T,FACE_INDEX<TV::m> >& Z,const ARRAY<T,FACE_INDEX<TV::m> >& Z_forward_ghost,const ARRAY<T,FACE_INDEX<TV::m> >& Z_backward_ghost,const ARRAY<T,FACE_INDEX<TV::m> >& Z_min,const ARRAY<T,FACE_INDEX<TV::m> >& Z_max);

    void Apply_Second_Order_Update_Face_Threaded(RANGE<TV_INT>& domain,const GRID<TV>& grid,int axis,ARRAY<T,FACE_INDEX<TV::m> >& Z,const ARRAY<T,FACE_INDEX<TV::m> >& Z_forward_ghost,const ARRAY<T,FACE_INDEX<TV::m> >& Z_backward_ghost);
