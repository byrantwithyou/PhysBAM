...
public:
    ...
    FILE_UTILITIES::Read_From_File<RW>(LOG::sprintf("%s/common/%skey",prefix.c_str(),list_name.c_str()),names);
    for(int i=0;i<needs_init.m;i++){ID id=needs_init(i),index=Element_Index(id);
        ...
        FILE_UTILITIES::Read_From_File<RW>(LOG::sprintf("%s/common/%s%d.%s",prefix.c_str(),list_name.c_str(),id,Active_Element(index)->Extension().c_str()),*Active_Element(index));}
    ...
    for(int i=0;i<needs_write.Size();i++){ID id=needs_write(i);int index=Element_Index(id);assert(index>=0);
        ...
        FILE_UTILITIES::Write_To_File<RW>(LOG::sprintf("%s/common/%s%d.%s",prefix.c_str(),list_name.c_str(),id,Active_Element(index)->Extension().c_str()),*Active_Element(index));}
    if(frame==0 || needs_write.Size()) FILE_UTILITIES::Write_To_File<RW>(LOG::sprintf("%s/common/%skey",prefix.c_str(),list_name.c_str()),names);
