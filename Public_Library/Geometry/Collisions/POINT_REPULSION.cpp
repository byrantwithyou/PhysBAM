...
#include <Geometry/Collisions/POINT_REPULSION.h>
...
Move_Points()
{
    ...
    for(int i=0;i<points.m;i++){
        ...
        for(int j=0;j<data.neighbors.m;j++){
    ...
    for(int i=0;i<points.m;i++){points(i).position=new_points(i);points(i).triangle=new_triangles(i);}
...
Update_Stats()
{
    ...
    for(int i=0;i<points.m;i++){
        ...
        if(data.neighbors.m){
            ...
            for(int j=0;j<data.neighbors.m;j++)local_sum+=(data.position-points(data.neighbors(j)).position).Magnitude();
