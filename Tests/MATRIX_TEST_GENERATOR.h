...
{
    bool v;
    TEST_GENERATOR2(const MATRIX_TESTS<T2>& m,int n){v=regular(m,n)&square(m,n,VECTOR<int,a==b && (a==2 || a==3)>());}
...
{
    bool v;
    TEST_GENERATOR(const MATRIX_TESTS<T2>& m,int n){v=TEST_GENERATOR2<T2,int,int,bool,x/b/b+a,x/b%b+a,x%b+a>(m,n).v;v=(TEST_GENERATOR<T2,a,b,x-1>(m,n).v && v);}
};
