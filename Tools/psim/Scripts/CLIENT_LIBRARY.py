...
def Get_Session(d,query_states=None,users=None):
    ...
    for i in sessions.keys():
        ...
        formatted_choices.append((str(session["id"]),"%-8s %-20s %-20s"%(session["username"],statestr,session["label"])))
    if len(formatted_choices)==0:
        print "No sessions found with state %s and users %s"%(repr(query_states),repr(users))
        sys.exit(0)
    (code,session)=d.menu("Choose session", width=60,height=-1,menu_height=0,choices=formatted_choices)
    handle_dialog_code(d,code)
    return int(session)
