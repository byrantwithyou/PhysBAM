...
{
    ANIMATED_VISUALIZATION<T>::Initialize_Components();
    ...
    OPENGL_UNIFORM_SLICE<T>* slice=new OPENGL_UNIFORM_SLICE(opengl_world);slice->Initialize(GRID_3D<float>(grid));
    ...
    Selection_Priority(OPENGL_SELECTION<T>::GRID_NODE_3D)=100;
    Selection_Priority(OPENGL_SELECTION<T>::GRID_CELL_3D)=90;
}
...
Add_Key_Bindings()
{
    ANIMATED_VISUALIZATION<T>::Add_Key_Bindings();
...
Update_OpenGL_Strings()
{
    ...
    if(current_selection && current_selection->type == OPENGL_SELECTION<T>::GRID_CELL_3D)
    {
    ...
    else if(current_selection && current_selection->type == OPENGL_SELECTION<T>::GRID_NODE_3D)
    {
