...
public:
    bool convert_density,convert_velocity;
    ...
    T_GRID grid;FILE_UTILITIES::Read_From_File<RW>(output_directory+"/"+f+"/dyadic_grid",grid);
    if(convert_density) Convert_Data<T_ARRAYS_SCALAR>("dyadic_rho",grid,frame);
    if(convert_velocity) Convert_Data<T_ARRAYS_SCALAR>("dyadic_face_velocities",grid,frame);}
...
int main(int argc,char* argv[])
{
    ...
    if(use_double){
        ...
        PhysBAM_To_Gnuplot<BINTREE_GRID<double>,double>(parse_args,verbosity);
#endif
    }
    else PhysBAM_To_Gnuplot<BINTREE_GRID<float>,float>(parse_args,verbosity);
}
#else
int main(int argc, char* argv[])
{
