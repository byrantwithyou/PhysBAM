% levelset_contour_3d('directory','xyz_constant',slice_number,filenumber)
function levelset_contour_3d(directory,xyz_constant,slice_number,filenumber)
% get dimensions
...
if(xyz_constant=='x') fseek(fid,8*mn*n*(slice_number-1),'bof');phi=fread(fid,[mn,n],'double');
elseif(xyz_constant=='y') phi=zeros(m,mn);fseek(fid,8*mn*(slice_number-1),'bof');
   for i=1:m
      phi(i,:)=fread(fid,mn,'double')';
      fseek(fid,8*mn*(n-1),'cof');
   end
elseif(xyz_constant=='z') fseek(fid,8*(slice_number-1),'bof');phi=fread(fid,[n,m],'double',8*(mn-1));   
end
% draw graph
clf;orient tall;hold on;
if(xyz_constant=='x') 
   contour(phi',0,'b');axis equal;axis([1 mn 1 n]);
   titlewords=sprintf('levelset zy cross section x=%d',slice_number);title(titlewords);   
elseif(xyz_constant=='y') 
   contour(phi,0,'b');axis equal;axis([1 m 1 mn]);
   titlewords=sprintf('levelset xz cross section y=%d',slice_number);title(titlewords);
elseif(xyz_constant=='z') 
   contour(phi,0,'b');axis equal;axis([1 m 1 n]);
