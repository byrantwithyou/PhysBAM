    ...
    *input>>num_surfaces;
    for(int i=1;i<=num_surfaces;i++){
        *input>>surface_name>>scaling_factor>>input_frame;
        ...
        for(int p=1;p<=surface->particles.number;p++) surface->particles.X(p)=align_frame*subtract_frame.Inverse()*input_frame*surface->particles.X(p);
        ...
        for(int p=1;p<=surface->particles.number;p++) merged_surface->particles.Copy_Particle(surface->particles,p,old_number_of_particles+p);
