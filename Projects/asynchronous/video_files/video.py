...
class VIDEO:
    def __init__(self,directory):
        ...
        # range
        self.range_pattern="^(.*\.)([0-9]+?)\..*$"
        self.range_regexp=re.compile(self.range_pattern)
    ...
    def add_directory(self,directory,start=-1,end=-1):
        ...
        files=filter(lambda x:self.in_range(x,start,end),files)
        files.sort()
...
#        os.system(cmd)
    ...
    def in_range(self,filename,start,end):
        range_match=self.range_regexp.match(filename)
        value=int(range_match.group(2))
        if((value>=start) & ((end==-1) | (value<=end))): return True
        else: return False
