...
#ifndef PHYSBAM_PROJECTS_MULTIGRID_EMBEDDED_POISSON_PETSC_SYSTEM_REFERENCE_HPP
#define PHYSBAM_PROJECTS_MULTIGRID_EMBEDDED_POISSON_PETSC_SYSTEM_REFERENCE_HPP
...
struct SYSTEM_REFERENCE
{
    ...
    explicit SYSTEM_REFERENCE(const T_SYSTEM& system);

    T Diag(const int index) const;
    int Stencil_N_Nonzero(const int index) const;

    typedef UNSTRUCTURED_STENCIL_PROXY< UNSTRUCTURED_STENCIL<int,T> > ADD_STENCIL_TO_STENCIL_PROXY_TYPE;
    void Add_Stencil_To(
        const int index,
...
inline
SYSTEM_REFERENCE<T>::
SYSTEM_REFERENCE(const T_SYSTEM& system)
    : p_system(&system),
...
template< class T >
struct SYSTEM_REFERENCE<T>::DISPATCHER
{
...
template< class T >
template< class T_SYSTEM >
struct SYSTEM_REFERENCE<T>::ACCESSOR
{
    static const DISPATCHER* Dispatcher()
    {
        static const DISPATCHER dispatcher = { &Diag, &Stencil_N_Nonzero, &Add_Stencil_To };
        return &dispatcher;
    }
...
#endif // #ifndef PHYSBAM_PROJECTS_MULTIGRID_EMBEDDED_POISSON_PETSC_SYSTEM_REFERENCE_HPP
