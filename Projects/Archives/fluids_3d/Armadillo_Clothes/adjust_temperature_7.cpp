...
// for i in `seq 0 \`cat last_frame\``; do ../adjust_temperature_7 $i; done
...
*/
    ...
    if(y>.08 || frame>110) return t_fix;
        ...
        if(frame<100) return reduction*t_fix;
        else{
            T tau=(T)(frame-100)/20;
            return (tau+(1-tau)*reduction)*t_fix;}}
}
...
template<class T,class RW> void Process(int argc,char* argv[])
{
    ...
    for(int i=1;i<=grid.m;i++)for(int j=1;j<=grid.n;j++)for(int ij=1;ij<=grid.mn;ij++)temperature(i,j,ij)=Adjust_Temperature<T>(temperature(i,j,ij),frame,grid.X(i,j,ij));
