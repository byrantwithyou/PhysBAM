...
void Delete_Particles_Inside_Objects(PARTICLE_LEVELSET_PARTICLES<T,VECTOR<T,3> >& particles,const typename PARTICLE_LEVELSET<T>::PARTICLE_TYPE particle_type,const T time)
{
    ...
    if(particle_type == NEGATIVE || particle_type == REMOVED_NEGATIVE){
        for(int k=particles.array_collection->Size();k>=1;k--) if(rigid_bodies(1)->Implicit_Surface_Lazy_Inside(particles.X(k),-grid.dx)) particles.Delete_Particle(k);}
   else for(int k=particles.array_collection->Size();k>=1;k--) if(rigid_bodies(1)->Implicit_Surface_Lazy_Inside(particles.X(k))) particles.Delete_Particle(k);
}
//#####################################################################
