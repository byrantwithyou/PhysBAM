...
void Average_Node_Velocities_To_Faces()
{
    ...
    if(!set_fluid_boundary_conditions_for_pressure_jump_solve){
        ...
        for(int axis=1;axis<=3;axis++){
            ...
            for(int i=1;i<=face_grid.m;i++) for(int j=1;j<=face_grid.n;j++) for(int ij=1;ij<=face_grid.mn;ij++) if(!psi_N(i,j,ij)){
                ...
                if(only_use_object_velocity_in_mixed_face && mixed_face){
                    for(int c=1;c<=4;c++){VECTOR<int,3> index=GRID_3D<T>::Face_Node_Index(axis,face_index,c);
                    ...
                    for(int c=1;c<=4;c++){velocity+=V(GRID_3D<T>::Face_Node_Index(axis,face_index,c))[axis];}
...
void Write_Output_Files(const int frame) const
{
    ...
    for(int i=1;i<=solids_parameters.rigid_body_parameters.list.Number_Of_Elements();i++){const RIGID_BODY<TV>& rigid_body=*solids_parameters.rigid_body_parameters.list(i);
