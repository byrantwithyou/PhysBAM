...
void Chorin_Project(
    ...
    const T  mu_negative, const T  mu_positive, // viscosity
    ...
    const T_MAC_VECTOR_FIELD& mac_vector_field)
{
    ...
    Project_To_Zero_Divergence_With_Interface(
        ...
        JUMP_P_OF_X_OF_CELL_INDEX_(
            min_x, max_x, dt,
            sigma, mu_positive - mu_negative,
            mac_cell_multi_index_bound,
            phi_prev_of_fine_index,
            kappa_of_fine_index,
            mac_vector_field
        ),
        CONSTANT_FUNCTION<T>(static_cast<T>(0)),
        mac_vector_field
    );
}
...
struct JUMP_P_OF_X_OF_CELL_INDEX
{
    PHYSBAM_DIRECT_INIT_CTOR_DECLARE_PRIVATE_MEMBERS(
        ...
        (( typename T_KAPPA_OF_FINE_INDEX const, kappa_of_fine_index ))
...
public:
    ...
    T operator()(VECTOR<T,D> x) const
    {
        ...
        BOOST_FOREACH( const MULTI_INDEX_TYPE fine_multi_offset, (STATIC_MULTI_INDEX_CUBE<D,0,1>()) ) {
            ...
            const T weight = ((1 - fine_multi_offset) * (1 - x) + fine_multi_offset * x).Product();
