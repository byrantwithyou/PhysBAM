...
public:
    ...
    using BASE::data_directory;using BASE::last_frame;using BASE::viewer_dir;
        ...
        :BASE(stream_type_input,parse_args),width(2),height(4),num_bodies(6),tests(stream_type_input,data_directory,solid_body_collection)
    {
        ...
        if(!this->user_output_directory)
            viewer_dir.output_directory=LOG::sprintf("MPI_Example/Test_%d",test_number);
    }
...
void Break_Levelset()
{
    ...
    Read_From_File(LOG::sprintf("%s/Rigid_Bodies/sphere_66k.phi",data_directory.c_str()),levelset);
    ...
    for(CELL_ITERATOR<TV> iterator(body_grid);iterator.Valid();iterator.Next()){
        TV_INT cell=levelset.grid.Cell(iterator.Location());
