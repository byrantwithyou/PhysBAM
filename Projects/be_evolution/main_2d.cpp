...
void Plot_Contours(int p)
{
    TV orig_X=psim->solid_body_collection.deformable_body_collection.particles.X(p);

    GRID<TV> sample_grid(TV_INT()+500,RANGE<TV>::Centered_Box()*3);
    ARRAY<T,TV_INT> E(sample_grid.Node_Indices());
    for(NODE_ITERATOR<TV> it(sample_grid);it.Valid();it.Next())
    {
        psim->solid_body_collection.deformable_body_collection.particles.X(p)=it.Location();
        psim->solid_body_collection.Update_Position_Based_State(0,true);
        T ke=0,pe=0;
        psim->solid_body_collection.Compute_Energy(0,ke,pe);
        E(it.index)=pe;
    }
    E.array-=E.array.Min();
    E.array/=E.array.Max();
    ...
    {
        E.array-=de;
        Dump_Levelset(sample_grid,E,VECTOR<T,3>(1,0,0));
    }

    psim->solid_body_collection.deformable_body_collection.particles.X(p)=orig_X;
    Add_Debug_Particle(orig_X,VECTOR<T,3>(1,1,1));
}
...
int main(int argc,char* argv[])
{
    ...
    TRIANGULATED_AREA<T>& ta=tests.Create_Mattress(cube_grid);
    simulation.solid_body_collection.Add_Force(Create_Finite_Volume(ta,new COROTATED_FIXED<T,TV::m>(1e6,0.3,0)));
    ...
    sample_particle=random.Get_Uniform_Integer(0,simulation.solid_body_collection.deformable_body_collection.particles.X.m);
