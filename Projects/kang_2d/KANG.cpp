...
//    fluids_parameters.second_order_cut_cell_method=false;
    ...
    if(!rebuild_surface && use_surface){
        SPHERE<TV> object;
        if(test_number==7) object=SPHERE<TV>(TV((T).5*m,(T).5*m),(T).2*m);
        ...
        if(test_number==7 || test_number==11) 
            for(int i=1;i<=temp_structure->particles.X.m;i++)
    ...
    if(use_massless_structure){
        ...
        if(!use_phi){
            if(remesh){
                ...
                ARRAY<TV> copy_X;
                copy_X=front_tracked_structure->particles.X.Prefix(ARRAYS_COMPUTATIONS::Max(front_tracked_structure->mesh.elements.Flattened()));
                ARRAYS_COMPUTATIONS::Fill(X,TV());
...
Update_Time_Varying_Material_Properties(const T time)
{
    if(!time) return;
    if(rebuild_curve) Rebuild_Surface();
    if((test_number==7 || test_number==11 || test_number==12) && !use_phi){
