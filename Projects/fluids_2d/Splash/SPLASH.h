...
void Delete_Particles_Inside_Objects(PARTICLE_LEVELSET_PARTICLES<T,VECTOR_2D<T> >& particles,const typename PARTICLE_LEVELSET<T,VECTOR_2D<T> >::PARTICLE_TYPE particle_type,const T time)
{
    ...
    if(particle_type == PARTICLE_LEVELSET<T,VECTOR_2D<T> >::NEGATIVE || particle_type == PARTICLE_LEVELSET<T,VECTOR_2D<T> >::REMOVED_NEGATIVE){
        for(int k=particles.Size();k>=1;k--) if(solids_parameters.rigid_body_parameters.list(1)->Implicit_Curve_Lazy_Inside_Extended_Levelset(particles.X(k),-fluids_parameters.grid.dx)) particles.Delete_Particle(k);}
    else for(int k=particles.Size();k>=1;k--) if(solids_parameters.rigid_body_parameters.list(1)->Implicit_Curve_Lazy_Inside_Extended_Levelset(particles.X(k))) particles.Delete_Particle(k);
}
//#####################################################################
