...
Fill_Ghost_Cells(GRID_2D<T>& grid,ARRAY<T2,VECTOR<int,2> >& u,ARRAY<T2,VECTOR<int,2> >& u_ghost,const T time)
{
...
Apply_Boundary_Condition(GRID_2D<T>& grid,ARRAY<T2,VECTOR<int,2> >& u,const T time) 
{
    ...
    for(i=1;i<=m;i++) u(i,1)=(T2)0; // new - do not set top to zero u(i,n,ij)=0; 
}  
//#####################################################################
}
#endif
