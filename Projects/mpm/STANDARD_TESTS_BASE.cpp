...
STANDARD_TESTS_BASE(const STREAM_TYPE stream_type_input,PARSE_ARGS& parse_args)
    ...
    dump_collision_objects(false),poisson_disk(*new POISSON_DISK<TV>(1)),
    tests(stream_type_input,solid_body_collection.deformable_body_collection)
{
    T framerate=0;
    ...
    parse_args.Add("-single_particle_cfl",&this->use_single_particle_cfl,"limit dt based on single particle stability");
    parse_args.Add("-cfl_p",&this->cfl_single_particle,"cfl","limit dt based on single particle stability");
    ...
    parse_args.Add("-verbose_cfl",&this->verbose_cfl,"Be verbose about CFL computations.");
...
template TETRAHEDRALIZED_VOLUME<double>& STANDARD_TESTS_BASE<VECTOR<double,3> >::Seed_Lagrangian_Particles<TETRAHEDRALIZED_VOLUME<double> >(TETRAHEDRALIZED_VOLUME<double>&,std::function<VECTOR<double,3> (VECTOR<double,3> const&)>,std::function<MATRIX<double,3,3> (VECTOR<double,3> const&)>,double,bool,bool);
template TETRAHEDRALIZED_VOLUME<float>& STANDARD_TESTS_BASE<VECTOR<float,3> >::Seed_Lagrangian_Particles<TETRAHEDRALIZED_VOLUME<float> >(TETRAHEDRALIZED_VOLUME<float>&,std::function<VECTOR<float,3> (VECTOR<float,3> const&)>,std::function<MATRIX<float,3,3> (VECTOR<float,3> const&)>,float,bool,bool);
}
