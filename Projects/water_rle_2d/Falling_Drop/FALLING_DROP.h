...
template<class T>
class FALLING_DROP:public SOLIDS_FLUIDS_EXAMPLE_RLE<RLE_GRID_2D<T> >
{
...
public:
    ...
    using BASE::particle_levelset;using BASE::short_cells_for_removed_negative_particles;using BASE::incompressible;
    typedef typename RLE_GRID_2D<T>::ITERATOR_CELL ITERATOR_CELL;typedef typename RLE_GRID_2D<T>::ITERATOR_FACE_X ITERATOR_FACE_X;

    FALLING_DROP(const STREAM_TYPE stream_type)
        :SOLIDS_FLUIDS_EXAMPLE_RLE<RLE_GRID_2D<T> >(stream_type)
    {
        grid.Set_Uniform_Grid(GRID<TV>(100+1,100+1,0,1,0,1));
        //grid.Set_Negative_And_Positive_Bandwidths(1000,3);
        ...
        short_cells_for_removed_negative_particles=true;
...
{
    static SPHERE<TV> circle((TV((T).5,(T).7)),(T).2);
    return min(circle.Signed_Distance(X),X.y-(T).21);
    //return X.y-(T).21;
...
void Get_Cell_Should_Be_Long(ARRAY<bool>& cell_should_be_long,const T time) const PHYSBAM_OVERRIDE
{
    ...
    //for(ITERATOR_CELL cell(grid,0);cell;cell++)if(min(cell.dj,cell.j_end-cell.j)>=2){int c=cell.Cell();
    //  if(particle_levelset.phi(c)<0 && phi_object(c)>=grid.positive_bandwidth) cell_should_be_long(c)=false;}
}
//#####################################################################
// Function Initalize_Velocities
//#####################################################################
