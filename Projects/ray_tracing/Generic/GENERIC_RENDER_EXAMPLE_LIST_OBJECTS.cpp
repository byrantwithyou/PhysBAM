...
Apply_Triangulated_Surface_Parameters(RENDERING_TRIANGULATED_SURFACE<T>& rendering_triangulated_surface,PARAMETER_LIST& parameters)
{
    ...
    if(parameters.Get_Parameter("Add_To_Collisions",false)){
        ...
        collisions->Update_Intersection_Acceleration_Structures(true,COLLISION_GEOMETRY<TV>::FLUID_COLLISION_GEOMETRY_NEW_STATE,COLLISION_GEOMETRY<TV>::FLUID_COLLISION_GEOMETRY_OLD_STATE);}
    ...
    if(type=="Rigid_Body_List" || type=="Rigid_Body_Instance"){
            ...
            if(render_object=="Null"){
                ...
                if(!rendering_triangulated_surface){
                    if(!rigid_body_collection.Rigid_Body(id).simplicial_object->mesh.elements.m){
                        IMPLICIT_OBJECT<TV>* implicit_object=rigid_body_collection.Rigid_Body(id).implicit_object->object_space_implicit_object;
        ...
        for(int index=0;index<integer_list.m;index++){
            ...
            if(SEGMENTED_CURVE<TV>* segmented_curve=dynamic_cast<SEGMENTED_CURVE<TV>*>(structure)){
                ...
                LOG::cout<<"object "<<i<<": embedded object"<<std::endl;
        ...
        for(int index=0;index<integer_list.m;index++){
            ...
            /*if(add_to_collisions){
                ...
                //DEFORMABLE_BODY_COLLECTION<TV>& deformable_body_collection=*deformable_body_collections[object_name];
