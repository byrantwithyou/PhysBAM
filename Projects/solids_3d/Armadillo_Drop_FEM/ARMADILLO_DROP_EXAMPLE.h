//#####################################################################
// Copyright 2002, 2003, 2004, Robert Bridson, Ronald Fedkiw, Neil Molino, Zhaosheng Bao.
// This file is part of PhysBAM whose distribution is governed by the license contained in the accompanying file PHYSBAM_COPYRIGHT.txt.
//#####################################################################
// Class ARMADILLO_DROP_EXAMPLE
//##################################################################### 
#ifndef __ARMADILLO_DROP_EXAMPLE__
#define __ARMADILLO_DROP_EXAMPLE__

#include "../CLOTH_EXAMPLE.h"
namespace PhysBAM{

template<class T>
class ARMADILLO_DROP_EXAMPLE:public CLOTH_EXAMPLE<T>
{
public:
    T height;

    ARMADILLO_DROP_EXAMPLE()
        :height(0)
    {
        ...
        youngs_modulus=1e3;poissons_ratio=(T).3;Rayleigh_coefficient=(T).03;
        ...
        output_directory="Armadillo_Drop/output";
    ...
    virtual ~ARMADILLO_DROP_EXAMPLE()
    {}
...
virtual void Get_Initial_Data(TRIANGULATED_SURFACE<T>& triangulated_surface)
{
    ...
    T miny=1e10;
    for(int p=0;p<particles.Size();p++) if(particles.X(p).y<miny) miny=particles.X(p).y;
