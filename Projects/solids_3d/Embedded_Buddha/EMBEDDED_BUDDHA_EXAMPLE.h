...
//#####################################################################
#ifndef __EMBEDDED_BUDDHA_EXAMPLE__
#define __EMBEDDED_BUDDHA_EXAMPLE__
...
template <class T>
class EMBEDDED_BUDDHA_EXAMPLE:public TET_SIM_EMBEDDED_COLLISIONS_EXAMPLE<T>
{
public:
    T initial_height;
...
virtual void Initialize_Embedded_Tetrahedralized_Volume(EMBEDDED_TETRAHEDRALIZED_VOLUME<T>& embedded_tetrahedralized_volume)
{
    ...
    sprintf(filename,"../../Public_Data/Rigid_Bodies/Buddha_rescaled.phi");input.open(filename,std::ios::in|std::ios::binary);
