...
int main(int argc,char* argv[])
{
    ...
    if(opt_incomp) example=new INCOMPRESSIBLE_TESTS<T>(stream_type,parse_args);
    else if(opt_hair_sim_tests) example=new HAIR_SIM_TESTS<T>(stream_type,parse_args);
    else if(opt_hair_strand_tests) example=new HAIR_STRAND_TESTS<T>(stream_type,parse_args);
    else if(opt_mass_weighted_self_collisions) example=new MASS_WEIGHTED_SELF_COLLISIONS<T>(stream_type,parse_args);
    else if(opt_embedded_collisions) example=new EMBEDDED_COLLISIONS_EXAMPLE<T>(stream_type,parse_args);
    else if(opt_rigid_particles) example=new RIGID_PARTICLE_EXAMPLE<T>(stream_type,parse_args);
    else if(opt_rigid_particle_spheres) example=new RIGID_PARTICLE_SPHERES_EXAMPLE<T>(stream_type,parse_args);
    else if(opt_simple_hard_binding) example=new SIMPLE_HARD_BINDING_EXAMPLE<T>(stream_type,parse_args);
    else if(opt_binding_plasticity) example=new BINDING_PLASTICITY_EXAMPLE<T>(stream_type,parse_args);
    else if(opt_binding_springs) example=new BINDING_SPRINGS_TEST<T>(stream_type,parse_args);
    else if(opt_cloth_spheres) example=new CLOTH_SPHERES_EXAMPLE<T>(stream_type,parse_args);
    else if(opt_embedding_test) example=new EMBEDDING_TEST<T>(stream_type,parse_args);
    else if(opt_hair_tests) example=new HAIR_TESTS<T>(stream_type,parse_args);
    else if(opt_fragment_tests) example=new FRAGMENT_TESTS<T>(stream_type,parse_args);
    else if(opt_armadillo_test) example=new ARMADILLO_TEST<T>(stream_type,parse_args);
    else if(opt_body_test) example=new BODY_TEST<T>(stream_type,parse_args);
