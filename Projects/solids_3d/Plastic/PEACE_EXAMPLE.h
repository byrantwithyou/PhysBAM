...
void Get_Initial_Data(TETRAHEDRALIZED_VOLUME<T>& tetrahedralized_volume)
{
    ...
    if(use_control){
        ...
        for(int p=1;p<=particles.array_collection->Size();p++)plastic_goal(p)=Peace_Map(particles.X(p));
    ...
    if(use_control)for(int p=1;p<=particles.array_collection->Size();p++)plastic_goal(p)*=(T).5;
