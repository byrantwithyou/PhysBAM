...
public:
    ...
    ***************/
    ...
    INCOMPRESSIBLE_DROP(const STREAM_TYPE stream_type)
        :BASE(stream_type,1,fluids_parameters.COMPRESSIBLE)
    {
    }
...
void Parse_Options() PHYSBAM_OVERRIDE
{
    ...
    *fluids_parameters.grid=fluids_parameters.grid->Get_MAC_Grid_At_Regular_Positions();
    ...
    //time
    initial_time=0.;last_frame=500;frame_rate=10000;
    ...
    if(timesplit) output_directory=STRING_UTILITIES::string_sprintf("Incompressible_Drop/Test_%d__Resolution_%d_%d_semiimplicit",test_number,(fluids_parameters.grid->counts.x),(fluids_parameters.grid->counts.y));
    else output_directory=STRING_UTILITIES::string_sprintf("Incompressible_Drop/Test_%d__Resolution_%d_%d_explicit",test_number,(fluids_parameters.grid->counts.x),(fluids_parameters.grid->counts.y));
...
void Initialize_Velocities() PHYSBAM_OVERRIDE
{
    if(test_number==1){
        for(UNIFORM_GRID_ITERATOR_FACE<TV> iterator(*fluids_parameters.grid);iterator.Valid();iterator.Next())
            if(iterator.Axis()==1) fluid_collection.incompressible_fluid_collection.face_velocities.Component(1)(iterator.Face_Index())=(T)100;
            else fluid_collection.incompressible_fluid_collection.face_velocities.Component(2)(iterator.Face_Index())=(T)0;
