...
public:
    ...
    {T result=0;for(int i=1;i<=entries.m;i++) if(array.Domain_Indices().Lazy_Inside(entries(i).x+shift)) result+=array(entries(i).x+shift)*entries(i).y;return result;}

    T Contraction_Scaled(const T_ARRAYS& array,const VECTOR<int,d>& shift,const T_ARRAYS& scale_array) const
    {T result=0;for(int i=1;i<=entries.m;i++) result+=array(entries(i).x+shift)*entries(i).y*scale_array(entries(i).x+shift);return result;}

    T operator*(const STENCIL& stencil) const
    {T result=0;for(int i=1;i<=entries.m;i++) result+=stencil(entries(i).x)*entries(i).y;return result;}
    ...
    {STENCIL result(*this);for(int i=1;i<=entries.m;i++) result.entries(i).y*=array(entries(i).x);return result;}
    ...
    void Normalize()
    {T result=0;for(int i=1;i<=entries.m;i++) result+=entries(i).y;
    if (result==0) return;
    for(int i=1;i<=entries.m;i++) entries(i).y/=result;}
    
    void Print_Stencil(std::ostream& output) const
    {for(int i=1;i<=entries.m;i++) output<<entries(i).x<<"->"<<entries(i).y<<";";output<<std::endl;}
