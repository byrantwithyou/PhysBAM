...
ARGS="../mpm_mac 25 -last_frame 10 -frame_dt 1 -clamp -mu 0 -scale_mass 3 -resolution $RES -analyze_u_modes -dump_modes_freq $RES $DT -max_ke 15"
...
for o in ${opt_name[@]} ; do
    sed -e "s/xxx/$o/" vort_dambreak_plot.tex  > $NAME/plot-dambreak-vort-$o.tex
    sed -e "s/xxx/$o/" vel_dambreak_plot.tex  > $NAME/plot-dambreak-vel-$o.tex
done

for i in {pic,apic}-{default,regular} ; do
    for j in {0..10} ; do
        convert -scale 512 $NAME/$i/modes-$(($j*$RES))-x.png `printf $NAME/$i-dambreak-vort-%02d.png $j`
        convert -scale 512 $NAME/$i/modes-$(($j*$RES))-x.png `printf $NAME/$i-dambreak-vort-%02d.eps $j`
    done
done
