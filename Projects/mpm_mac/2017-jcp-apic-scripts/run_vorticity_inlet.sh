...
INLETV=0.2
STOP=5
ARGS="../mpm_mac 28 -frame_dt 1 -mu 0 -scale_mass 3 -T $INLETV -T $STOP -reseed -analyze_energy_vort -clamp"
...
if [ "X$FULL" = "X1" ] ; then
    ...
    for o in `seq 0 $((${#opt[@]}-1))` ; do
        for r in `seq $HI -$SKIP $LO` ; do
            ...
            echo $ARGS $DT $LF $PVORT -no_affine ${opt[$o]} -xpic 2 -resolution $r -o $NAME/xpic2-${opt_name[$o]}-$r
            echo $ARGS $DT $LF $PVORT -no_affine ${opt[$o]} -xpic 3 -resolution $r -o $NAME/xpic3-${opt_name[$o]}-$r
            echo $ARGS $DT $LF $PVORT -no_affine ${opt[$o]} -xpic 5 -resolution $r -o $NAME/xpic5-${opt_name[$o]}-$r
        done
    done | xargs -P 16 -n 1 -d '\n' bash -c
fi

for s in pic apic flip xpic2 xpic3 xpic5 ; do
    for r in `seq $HI -$SKIP $LO` ; do
        for o in `seq 0 $((${#opt[@]}-1))` ; do
...
if false; then
for s in apic pic flip xpic2 xpic3 xpic5 ; do
    o="default"
...
for o in ${opt_name[@]} ; do
    sed -e "s/xxx/$o/; s/rrr/$RES/; s/LLL/Vorticity/; s/DDD/vort/; s/XMAX/10/; s/YMAX/1.8/;" vort_dambreak_plot.tex  > $NAME/plot-inlet-vort-$o.tex
    sed -e "s/xxx/$o/; s/rrr/$RES/; s/LLL/Kinetic energy/; s/DDD/ke/; s/XMAX/10/; s/YMAX/0.01/;" dambreak_plot.tex  > $NAME/plot-inlet-ke-$o.tex
    sed -e "s/xxx/$o/; s/rrr/$RES/; s/LLL/Total energy/; s/DDD/te/; s/XMAX/10/; s/YMAX/0.01/;" te_dambreak_plot.tex  > $NAME/plot-inlet-te-$o.tex
done
