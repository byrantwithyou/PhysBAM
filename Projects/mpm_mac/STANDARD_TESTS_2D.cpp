    ...
    switch(test_number)
    {
        ...
        case 21:{ // vortex shedding test
            ...
            auto write=[this]()
            {
                ...
                Write_To_File(stream_type,LOG::sprintf("%s/v%d/mac_velocities",output_directory.c_str(),id),phases(PHASE_ID()).velocity);
                Write_To_File(stream_type,LOG::sprintf("%s/v%d/mass",output_directory.c_str(),id),phases(PHASE_ID()).mass);
