...
template<class T,class TV>
void Dump_Interface(const INTERFACE_STOKES_SYSTEM_NEW<TV>& iss,const bool arrows)
{
...
template<class T,class TV>
void Dump_System(const INTERFACE_STOKES_SYSTEM_NEW<TV>& iss,ANALYTIC_TEST<TV>& at)
{
...
void Dump_u_p(const INTERFACE_STOKES_SYSTEM_NEW<TV>& iss,const ARRAY<T,FACE_INDEX<TV::m> >& u,const ARRAY<T,TV_INT>& p,const char* title)
{
    char buff[100];
    for(int i=0;i<TV::m;i++){
        Dump_Interface<T,TV>(iss,false);
...
void Integration_Test(int argc,char* argv[],PARSE_ARGS& parse_args)
{
    ...
    parse_args.Add("-mu_o",&opt_mu_o,"viscosity","viscosity outside");
    ...
    test->s=opt_s;
    test->kg=opt_kg;
    test->mu(0)=opt_mu_o*test->kg/(test->s*(d==3?test->m:1));
    test->mu(1)=opt_mu_i*test->kg/(test->s*(d==3?test->m:1));
    test->Initialize();
