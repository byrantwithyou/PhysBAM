...
range_regexp=re.compile(range_pattern)
...
def FileRangeFromPattern(rangepattern):
    match=range_regexp.match(rangepattern)
    prefix,num_min,num_max,suffix=match.groups()
    num_min,num_max=int(num_min),int(num_max)
    value=FileRange((prefix,suffix),None)
    value.extrema=(num_min,num_max)
    value.find_padding()
    return value
...
class FileRange:
    ...
    def all_filenames(self):
        return map(self.filename,range(self.extrema[0],self.extrema[1]))

    def find_padding(self):
        frame=self.extrema[0]
        print self.extrema
        if os.path.exists(self.filename(frame)): return
        else:
            for i in range(1,10):
                self.number_format="%%0%dd"%i
