...
//#####################################################################
#ifndef __BPP_SOLIDS_EXAMPLE__
#define __BPP_SOLIDS_EXAMPLE__
...
class BPP_SOLIDS_EXAMPLE:public SOLIDS_FLUIDS_EXAMPLE_UNIFORM<VECTOR<T_input,2> >
{
    typedef T_input T;
...
public:
    ...
    BPP_SOLIDS_EXAMPLE(const STREAM_TYPE stream_type_input,PARSE_ARGS& parse_args)
        :BASE(stream_type_input,parse_args,0,FLUIDS_PARAMETERS<TV>::NONE),tests(stream_type_input,data_directory,solid_body_collection),semi_implicit(false),test_forces(false),use_extended_neohookean(false),
...
void Primary_Contour(ISOTROPIC_CONSTITUTIVE_MODEL<T,2>& icm)
{
    ...
    for(int i=0;i<image_size;i++)
        for(int j=0;j<image_size;j++){
            ...
            DIAGONALIZED_ISOTROPIC_STRESS_DERIVATIVE<VECTOR<T,2> > disd;
            icm.Isotropic_Stress_Derivative(DIAGONAL_MATRIX<T,2>(x,y),disd,1);
            SYMMETRIC_MATRIX<T,2> H(disd.H(0,0),disd.H(1,0),disd.H(1,1));
