...
public:
    LEVELSET_2D(const int m_input,const int n_input,const T dx_input,const T xmin_input,const T ymin_input):
    grid(m_input,n_input,dx_input,xmin_input,ymin_input),phi((m_input+1)*(n_input+1)){}
    
    T Phi(INDEX_2D& index){return phi(index.Index());}
    GRID_2D<T>& Grid(){return grid;}
