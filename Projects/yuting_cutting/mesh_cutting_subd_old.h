...
public:
    ...
    struct TET_CUTTING {
        ...
        struct POTENTIAL_CENTER{
            ...
            CENTER sum;
            int n;//n = -1 if set
            POTENTIAL_CENTER(){ n = 0;}
            bool Is_Not_Set() { return n != -1; };
            void Append(CENTER new_potential_center) { PHYSBAM_ASSERT(Is_Not_Set()); sum += new_potential_center; n++; }
            void Set() { if(n!=0) value = sum/(T)n; n = -1; }
        }; 
...
//            ARRAY<CENTER> sum;
//            int n;
//            bool Is_Not_Set() { return true; }
//            void Append(CENTER new_potential_center) { sum.Append(new_potential_center); }
//            void Set()
//            { 
//                CENTER r(1,1,1,1); 
//                for (int i = 0; i < sum.m; i++){
//                    if(sum(i).Magnitude_Squared() < r.Magnitude_Squared()){
//                        r = sum(i);
//                    }
//                }
//                value = r;
...
public:
    ...
    //cutting triangle mesh
    ARRAY<TV> cutting_vertices;
...
private:
    ...
    void Split(const int& tet_id, const ARRAY<int>& tri_ids, const TRIANGULATED_SURFACE<T>& cutting_surface, ARRAY<int>& sim_node_from, ARRAY<bool>& sim_tet_split);
