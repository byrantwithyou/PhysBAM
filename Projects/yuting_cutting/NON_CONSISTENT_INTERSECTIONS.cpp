...
Compute_VE_Helper(int p,I2 e,ARRAY_VIEW<TV> Xp,ARRAY_VIEW<TV> Xe,T& gamma)
{
    ...
    T m=u.Magnitude_Squared();
    TV w=P-A;
    T d=(P-A).Cross(P-B).Magnitude();
    std::cout<<"P:"<<P<<", A:"<<A<<", B:"<<B<<", d:"<<d<<std::endl;
    if(d>tau) return false;
    std::cout<<u<<", "<<w<<std::endl;
    T a_hat=u.Dot(w),a_bar=m-a_hat;
    std::cout<<a_hat/m<<", "<<a_bar/m<<std::endl;
...
//    T a_C=(B-A).Cross(P-A).x;
    T a_A=(B-P).Cross(C-P).x;
    T a_B=(C-P).Cross(A-P).x;
    T a_C=(A-P).Cross(B-P).x;
