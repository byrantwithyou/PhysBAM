...
Compute_VE(int p,I2 e)
{
    ...
    TV P=ta.particles.X(p),A=sc.particles.X(e.x),B=sc.particles.X(e.y);
    TV u=B-A;
    TV w=P-A,v=P-B;
    T a_hat=u.Dot(w),a_bar=-u.Dot(v);
    if(a_hat<0 || a_bar<0) return false;
    if (a_hat==0) {
        hash_vv.Set(I2(p,e.x));
    }
    if (a_bar==0) {
        hash_vv.Set(I2(p,e.y));
    }
    
    hash_ve.Set(e.Insert(p,0),a_hat/(a_hat+a_bar));
    return true;
}
//#####################################################################
// Function Compute_EV
...
Compute_EV(I2 e,int p)
{
    ...
    TV A=ta.particles.X(e.x),B=ta.particles.X(e.y),P=sc.particles.X(p);
    TV u=B-A;
    TV w=P-A,v=P-B;
    T a_hat=u.Dot(w),a_bar=-u.Dot(v);
    if(a_hat<0 || a_bar<0) return false;
    bool r = 1;
    if (a_hat==0) {
        hash_vv.Set(I2(e.x,p));
        r=0;
    }
    if (a_bar==0) {
        hash_vv.Set(I2(e.y,p));
        r=0;
    }
    
    if (r) {
        hash_ev.Set(e.Append(p),a_hat/(a_hat+a_bar));
    }
    return true;
}
//#####################################################################
...
//    T a_C=(B-A).Cross(P-A).x;
    ...
    bool r=1;
    if (a_A==0) {
        Compute_EV(I2(f.y,f.z),p);
        r=0;
    }
    if (a_B==0) {
        Compute_EV(I2(f.x,f.z),p);
        r=0;
    }
    if (a_C==0) {
        Compute_EV(I2(f.x,f.y),p);
        r=0;
    }
    if (r) {
...
Compute()
{
    ...
    ta.mesh.Initialize_Segment_Mesh();
...
Compute(T L)
{
    ...
    ts.mesh.Initialize_Segment_Mesh();
    ...
    tv.mesh.Initialize_Segment_Mesh();
