...
{    
    ...
    rigid_body_collection.rigid_body_particle.X(lighthouse)=VECTOR<T,3>((T)25,(T)-2,(T)30);rigid_body_collection.Rigid_Body(lighthouse).Is_Kinematic()=true;
    cove=rigid_body_collection.Add_Rigid_Body(stream_type,data_directory+"/Rigid_Bodies/cove",(T)1,true,true,false);
    rigid_body_collection.rigid_body_particle.X(cove)=VECTOR<T,3>((T)-80,(T)11,(T)40);rigid_body_collection.Rigid_Body(cove).Is_Kinematic()=true;
