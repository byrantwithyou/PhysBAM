...
void Initialize_Particle_Positions_And_Velocities(const int object)
{
    ...
    for(int p=1;p<=particles.array_collection->Size();p++)if(particles.X(p).y>pivot.y) pivot.z=min(pivot.z,particles.X(p).z);
    for(int p=1;p<=particles.array_collection->Size();p++)if(particles.X(p).y>pivot.y) particles.X(p)=pivot+rotation.Rotate(particles.X(p)-pivot);
}
//#####################################################################
