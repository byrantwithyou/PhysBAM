...
void Adjust_Phi_With_Sources(const T time)
{
    ...
    for(int i=1;i<=grid.m;i++) for(int j=1;j<=grid.n;j++) for(int ij=1;ij<=grid.mn;ij++) if(source.Inside(grid.X(i,j,ij))) 
...
void Get_Source_Velocities(const T time)
{
    ...
    for(int i=1;i<=u_grid.m;i++) for(int j=1;j<=u_grid.n;j++) for(int ij=1;ij<=u_grid.mn;ij++) if(source.Inside(u_grid.X(i,j,ij))){psi_N_u(i,j,ij)=true;u(i,j,ij)=source_velocity.x;}
    for(int i=1;i<=v_grid.m;i++) for(int j=1;j<=v_grid.n;j++) for(int ij=1;ij<=v_grid.mn;ij++) if(source.Inside(v_grid.X(i,j,ij))){psi_N_v(i,j,ij)=true;v(i,j,ij)=source_velocity.y;}
    for(int i=1;i<=w_grid.m;i++) for(int j=1;j<=w_grid.n;j++) for(int ij=1;ij<=w_grid.mn;ij++) if(source.Inside(w_grid.X(i,j,ij))){psi_N_w(i,j,ij)=true;w(i,j,ij)=source_velocity.z;}
}
//#####################################################################
