...
int main(int argc,char* argv[])
{
    ...
    else if(opt_lighthouse) example=new LIGHTHOUSE<T>(stream_type,parse_args);
    else if(opt_sheeting) example=new SHEETING<T>(stream_type,parse_args);
    else if(opt_dsd_fire_ball) example=new DSD_FIRE_BALL_EXAMPLE<T>(stream_type,parse_args);
    else if(opt_dsd_no_navier) example=new DSD_NO_NAVIER_STOKES<T>(stream_type,parse_args);
    else if(opt_twophase) example=new TWO_PHASE<T>(stream_type,parse_args);
    else if(opt_efty) example=new FLOW_PAST_EFTYCHIS<T>(stream_type,parse_args);
    else if(opt_waterfall) example=new WATERFALL<T>(stream_type,parse_args);
    ...
    //example=new SIMPLE_FLAME<T>(stream_type,parse_args);
    //example=new VORTEX_TEST<T>(stream_type,parse_args);
    //example=new VORTEX_PARTICLES_STREAM<T>(stream_type,parse_args);
