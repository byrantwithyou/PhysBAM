...
class SURFACE_TENSION:public SOLIDS_FLUIDS_EXAMPLE_3D<RW>
{
public:
    ...
    using SOLIDS_FLUIDS_EXAMPLE_3D<RW>::data_directory;using SOLIDS_FLUIDS_EXAMPLE_3D<RW>::fluids_parameters;using SOLIDS_FLUIDS_EXAMPLE_3D<RW>::Get_Source_Velocities;
    using SOLIDS_FLUIDS_EXAMPLE_3D<RW>::Adjust_Phi_With_Source;using SOLIDS_FLUIDS_EXAMPLE_3D<RW>::Get_Source_Reseed_Mask;
    ...
    SURFACE_TENSION(const int example_input)
        :SOLIDS_FLUIDS_EXAMPLE_3D<RW>(fluids_parameters.WATER),levelset_object(fluids_parameters.grid,phi_object),
        source(VECTOR<T,3>(.5,.9,.5),VECTOR<T,3>(.5,1,.5),.1),source_velocity(0,-.8,0),moving_objects(false),example(example_input)
    {
        ...
        fluids_parameters.scalar_substeps=.97;
        //fluids_parameters.object_friction=1;
        //fluids_parameters.adhesion_coefficient=0;

        if(example==1){
            output_directory="Surface_Tension/output1";
            ...
            fluids_parameters.surface_tension_coefficient=.007;
            fluids_parameters.viscosity=(T)200;}
        else if(example==2){
            output_directory="Surface_Tension/output2";
            //fluids_parameters.grid.Initialize(60,100,60,.4,1.6,0,2,.4,1.6);
            fluids_parameters.grid.Initialize(30,50,30,.4,1.6,0,2,.4,1.6);
            source.Set_Endpoints(VECTOR<T,3>(1,1.9,1),VECTOR<T,3>(1,2,1));
            //source.radius=.1;
...
void Construct_Levelsets_For_Objects(const T time)
{
    ...
    if(!solids_parameters.rigid_body_parameters.list.rigid_bodies.m || initialized) return;
    ...
    for(int i=-2;i<=grid.m+3;i++)for(int j=-2;j<=grid.n+3;j++)for(int ij=-2;ij<=grid.mn+3;ij++){
        ...
        V_object(i,j,ij)=solids_parameters.rigid_body_parameters.list(min_r)->Pointwise_Object_Velocity(X);}
...
{
    Adjust_Phi_With_Source(source,MATRIX<T,4>::Identity_Matrix(),time);
}
//#####################################################################
...
{
    Get_Source_Reseed_Mask(source,MATRIX<T,4>::Identity_Matrix(),cell_centered_mask,true,time);
}
//#####################################################################
...
{
    if(!solids_parameters.rigid_body_parameters.list.rigid_bodies.m) return true;
