...
class TWO_PHASE:public SOLIDS_FLUIDS_EXAMPLE_UNIFORM<GRID<VECTOR<T_input,3> > >
{
    typedef T_input T;
public:
...
void Parse_Options() PHYSBAM_OVERRIDE
{
    ...
    grid.Initialize(10*cells+1,20*cells+1,10*cells+1,(T)-.01,(T).01,(T)-.01,(T).02,(T)-.01,(T).01);
...
void Initialize_Bodies() PHYSBAM_OVERRIDE
{
    if(test_number==2||test_number==3){
        ...
        rigid_body_collection.rigid_body_particle.frame(sphere).t=TV((T).0,(T).005,(T).0);
