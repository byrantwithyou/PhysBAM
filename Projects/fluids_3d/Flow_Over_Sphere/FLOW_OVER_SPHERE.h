...
void Delete_Particles_Inside_Objects(HEAVY_PARTICLES<T,VECTOR<T,3> >& particles,const typename WATER_FREE_SURFACE_3D_EXAMPLE<T,RW>::PARTICLE_TYPE particle_type,const T time)
{
    if(particle_type == NEGATIVE || particle_type == REMOVED_NEGATIVE){
        for(int k=0;k<particles.Size();k++) if(rigid_bodies(1)->Implicit_Surface_Lazy_Inside_Extended_Levelset(particles.X(k),-grid_3d.dx)) particles.Add_To_Deletion_List(k);}
   else for(int k=0;k<particles.Size();k++) if(rigid_bodies(1)->Implicit_Surface_Lazy_Inside_Extended_Levelset(particles.X(k))) particles.Add_To_Deletion_List(k);
