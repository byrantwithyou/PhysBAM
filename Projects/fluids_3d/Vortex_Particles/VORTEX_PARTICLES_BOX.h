...
void Get_Source_Velocities(const T time)
{
    ...
    for(int j=1;j<=fluids_parameters.u_grid.n;j++) for(int ij=1;ij<=fluids_parameters.u_grid.mn;ij++){
...
{
    GRID<TV>& grid=fluids_parameters.grid;
    for(int i=1;i<=grid.m;i++)for(int j=1;j<=grid.n;j++)for(int ij=1;ij<=grid.mn;ij++) fluids_parameters.particle_levelset_evolution.phi(i,j,ij)=grid.y(j)-(T).5;
}
//#####################################################################
