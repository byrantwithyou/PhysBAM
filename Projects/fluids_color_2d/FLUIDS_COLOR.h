...
public:
        ...
        :PLS_FC_EXAMPLE<TV>(stream_type)
    {
        ...
        grid.Initialize(TV_INT::All_Ones_Vector()*scale,RANGE<TV>(TV(),TV::All_Ones_Vector()),true);
    ...
    {
        if(time>3) return;
        for(typename GRID<TV>::CELL_ITERATOR iterator(grid);iterator.Valid();iterator.Next()){Set_Phi_Inside_Source(iterator.Cell_Index(),iterator.Location());}
    }
