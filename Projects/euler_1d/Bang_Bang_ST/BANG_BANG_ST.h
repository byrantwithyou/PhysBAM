        ...
        implicit_rk(false),exact(false)
    {
        ...
        *fluids_parameters.grid=fluids_parameters.grid->Get_MAC_Grid_At_Regular_Positions();
        ...
        if(!this->user_output_directory){
            if(timesplit) viewer_dir.output_directory=LOG::sprintf("Bang_Bang_ST/Test_1__Resolution_%d_semiimplicit",(fluids_parameters.grid->counts.x));
            else viewer_dir.output_directory=LOG::sprintf("Bang_Bang_ST/Test_1__Resolution_%d_explicit",(fluids_parameters.grid->counts.x));
