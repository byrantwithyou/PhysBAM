...
public:
    ...
    PROJECTION_EXAMPLE(const STREAM_TYPE stream_type_input,int number_of_threads=1);
    ...
    {for(typename GRID<TV>::FACE_ITERATOR iterator(mac_grid);iterator.Valid();iterator.Next()) face_velocities(iterator.Full_Index())=(T)iterator.Face_Index()(iterator.Axis())/(T)mac_grid.counts(iterator.Axis());}
