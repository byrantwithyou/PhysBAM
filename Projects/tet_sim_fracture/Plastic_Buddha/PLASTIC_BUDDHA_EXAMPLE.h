...
//#####################################################################
// Class PLASTIC_BUDDHA_EXAMPLE
//#####################################################################
#ifndef __PLASTIC_BUDDHA_EXAMPLE__
#define __PLASTIC_BUDDHA_EXAMPLE__
...
template <class T>
class PLASTIC_BUDDHA_EXAMPLE:public TET_SIM_EMBEDDED_EXAMPLE<T>
{
public:
    T initial_height;
        ...
        grain_boundaries(0),fracture_bias_direction_smoothing_passes(0),arm_pull_velocity(1.0)
    {
        ...
        strcpy(output_directory,"PLASTIC_BUDDHA/output");
        cg_tolerance=1e-4;
    ...
    virtual ~PLASTIC_BUDDHA_EXAMPLE()
    {}
...
void Initialize_Tetrahedralized_Volume(TETRAHEDRALIZED_VOLUME<T>& tetrahedralized_volume)
{
    ...
    sprintf(filename,"../../Public_Data/Tetrahedralized_Volumes/buddha_coarse.tet");input.open(filename,std::ios::in|std::ios::binary);assert(input.is_open());
    ...
    tetrahedralized_volume.Update_Bounding_Box();
     T scale=1;T velocity=0;T miny=1e10;
    ...
    for(int p=0;p<tetrahedralized_volume.particles.Size();p++) if(tetrahedralized_volume.particles.X(p).y<miny) miny=tetrahedralized_volume.particles.X(p).y;
    for(int p=0;p<tetrahedralized_volume.particles.Size();p++){
